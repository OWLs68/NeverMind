...
1. **🔴 B-98 «Залиплий прапорець OWL табло» — ПЕРША ЗАДАЧА НОВОЇ СЕСІЇ** (Роман підтвердив 24.04). Діагностика Романа показала: табло не оновлюється 8+ годин попри `stale message detected, forcing generation` у логах 5+ разів за день. Симптом: функція генерації зветься, прапорець «я вже генерую» не скидається (ймовірно упав проміс без `.catch()`, `finally` не виконався). Файл: `src/owl/board.js`. Фікс: `try/finally` + watchdog-timeout 60 сек. **Блокує V3** — без робочого табла тести проактивності фальшиві.
...
3. **🧪 Test Sprint триває.** Друг Романа вже тестував 22.04 з v368 (не активно — 2 API запити за 6 год, 1+1 запис). Роман хоче виділити **1-2 дні суто для тестів**. Треба **розширити `test-checklist.html`** з поточних 13 секцій до ~30 (блоки: базові системи, Inbox, 8 чатів, вкладки, OWL ініціатива, Кошик/памʼять, стрес-тести, форма звіту). Детальний зміст 8 блоків — у блоці сесії 8bSsE нижче. Порядок дій нової сесії: (1) фікс B-98, (2) розширити test-checklist.html, (3) тестування, (4) V3.

4. **✅ Правило «Довгі списки через HTML з кнопкою Копіювати» записано у `CLAUDE.md`** секція «📏 РОЗМІР ВІДПОВІДЕЙ» (24.04.2026 8bSsE, обрано варіант А). Тригери: список >30 рядків / шпаргалка для тестера / промпт для Gemini / розширений план. Дія: створити HTML за шаблоном `test-checklist.html` з sticky-кнопкою, оранжевим градієнтом, `navigator.clipboard` + fallback. Виняток: Роман явно каже «покажи у чаті».
