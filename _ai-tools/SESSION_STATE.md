...
**5. ГЛОСАРІЙ термінів** → `docs/GLOSSARY.md`. При поясненні термінів Роману використовуй формулювання звідти. Оновлюй коли з'являються нові поняття.

**6. Наступний крок роботи:** уніфікація `src/owl/followups.js` з `shouldOwlSpeak()` — обидва канали (табло + chat follow-ups) мають ходити через один Judge Layer. Деталі → секція "Jarvis Architecture — ДО Supabase" нижче.
...
- `FEATURES_ROADMAP.md` — додано Judge Layer і BroadcastChannel у "зроблено", переформульовано 4.2 (структуровані факти), уточнено 4.19 і 4.20
- `CLAUDE.md` — оновлено секцію про `localStorage.setItem` hack (BroadcastChannel вже поруч)
- **Новий файл `docs/GLOSSARY.md`** — 10 термінів простою мовою з аналогіями: RAG, Embedding, pgvector, Tool Calling, SyncEngine, BroadcastChannel, RLS, Edge Functions, BYOK, Multi-tenant. Читається на старті кожної сесії.
- `SESSION_STATE.md` (цей файл) — новий порядок Jarvis Architecture, підсумок сесії
...
**⚠️ ОНОВЛЕНО 11.04 після обговорення з Gemini 3 Pro + аудиту коду.** Детальне обговорення стратегії, аналіз "велосипед vs стандарт", глосарій термінів — див. `docs/CHANGES.md` запис від 11.04 і `docs/GLOSSARY.md`.
...
0. ✅ **Документація і синхронізація ROADMAP** — ЗРОБЛЕНО 11.04 у цій сесії. Оновлено FEATURES_ROADMAP (Judge Layer → "зроблено", 4.2 переформульовано на структуровані факти, BroadcastChannel → "зроблено"), CLAUDE.md (localStorage.setItem hack), створено `docs/GLOSSARY.md`.
...
- **BYOK vs Підписка** — бізнес-модель OpenAI ключа при переході на Supabase. **Критично** вирішити ДО Supabase, інакше ризик банкрутства при масштабуванні. (Варіанти: юзер вводить свій ключ / підписка $15/міс / гібрид)
