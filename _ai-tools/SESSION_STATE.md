...
**Оновлено:** 2026-07-03 (сесія **26yz5s** — аудит brain-Claude 3 пункти + **B-201 знайдено-закрито** (habits: re-export без локального імені → чат Задач «Мережева помилка») + **ESLint-сторож no-undef у CI** — статично ловить клас E2E#39/B-201 «переніс функцію — забув константи/import»).
...
- Додати `OPENAI_SMOKE_KEY` (окремий ключ, ліміт $5) → перший ai-smoke.
- Тріаж security-Issues #9/#12 (половина знахідок закрита у червні — оновити/закрити).
- iPhone-смоук хвости v3pexs (списки-тап, OWL-групи, чіпи).
