...
**Оновлено:** 2026-07-14 (сесія **26yz5s**, багатоденна 03-14.07 з паузами — 14.07: `/fullaudit` створено→тестовий прогін спалив тижневий ліміт→заблоковано + токен-запобіжник-хук. 11.07: розумніший OWL + модель-рішення. Раніше: аудит brain + B-201 + ESLint + 2 /byyou-потоки).

---

## 🔧 Поточна сесія 26yz5s — Аудит brain: B-201 + ESLint-сторож no-undef (03.07.2026)

### Зроблено (запушено)
1. **Аудит brain 3 пункти:** мітка «клас `\b`-кирилиця закрито» у `lessons.md` + правило «мапа різу зобов'язана містити константи + змінні модуля» у `byyou.md` Фаза 2 крок 1.5 (`ee2f0de`).
2. **🎯 B-201 знайдено і закрито (`f8ba815`):** оцінював ESLint як ловець класу E2E#39 → рівно 1 спрацювання на весь src/ (нуль шуму), і це живий прод-баг з батчу D: `export {processUniversalAction} from ...` у habits.js:968 НЕ створює локального імені → виклик у `_processOne` (чат Задач, текстовий JSON-fallback) падав ReferenceError → юзер бачив «Мережева помилка» (з v1082). Фікс: import+export окремо, проброс для 6 імпортерів збережено.
3. **ESLint-сторож у CI (`460fc5b`):** тільки правило no-undef (звертання до неоголошеного імені), конфіг `eslint.config.mjs` (скоуп src/), крок у `e2e.yml` після контракт-тестів + скрипт `npm run lint`. Альтернативу «розширити check-imports» відхилено: потрібен справжній аналіз областей видимості, регулярки дадуть кривий недо-лінтер. Дірка check-imports (вважає re-export локальним оголошенням) перекрита ESLint'ом.
4. **Розслідування «табло давно не оновлювалось» — НЕ баг:** тихі години сови рахуються за розкладом сну (дефолт сон 23:00 → тиша до 05:00), Роман активний вночі → генерація мовчить саме у його години, вдень PWA закритий. Знайдена вада: у табло вкладок «до 5 ранку» зашито жорстко повз розклад (`proactive.js` tryTabBoardUpdate). **Рішення Романа: лишити як є.** Швидкий обхід без коду: сказати OWL свій реальний ритм.
5. **Потік /byyou «3 дірки + прокачка режиму» (16/16, деплой v1110, ADR-004)** — за дослідженням gstack/best-practice (2 sonnet-агенти + власне читання raw): 🐤 канарейка після кожного деплою (перший прогін #2 зелений; одразу відпрацювала реальний кейс — Pages «try again later» → самокорекція порожнім комітом) · 🤖 AI-смоук 4 живі сценарії (чекає `OPENAI_SMOKE_KEY` від Романа, ліміт $5, інструкція у `_archive/BYYOU_PLANS_archive.md`) · 🔒 замок DO_NOT_TOUCH на boot.js/app.js (`dnt-ack` прохід, тест 5/5) · /byyou: `/qa-explore` хмарний смоук у Фазі 3 + верифікатор знахідок (Фаза 1 крок 3.5) + памʼять агентів (`_ai-tools/agent-memory/`) · **Sonnet 5 політика у CLAUDE.md** (рішення Романа «якість > економія»: sonnet-агентам тепер і верифікація/суддівство).

6. **Ранковий брифінг + 7 портів рев'ю/потоку (2 потоки /byyou, деплой v1114, ADR-005)** — задачі А+Б+В за погодженим планом Романа 04.07, залізне правило «нуль нових ручних команд» (усе — авто-кроки):
   - **А — /brief** (`.claude/commands/brief.md`): read-only ранковий брифінг для Routine (04:00 UTC = 06:00 Амстердам щодня) — стан CI + сторожі + беклог → звіт у чат з готовою byyou-чернеткою. Значення Routine Роман вставляє руками (в архіві плану). Перший смоук знайшов реальне: 2 security-Issues (#9/#12) висіли з травня — /brief тепер їх підсвічує.
   - **Б (alibaba/open-code-review):** P1 асиметричний фактчек знахідок (fail-open, замінив симетричний верифікатор 03.07; `refute-parser.js` 8/8) · P2 сніпет-якорі + `resolve-anchor.js` (порт resolver.go, **нуль regex** — клас \b-кирилиця неможливий; 10/10) · P3 мапа глоб→чекліст-шрами (`_ai-tools/review-rules/` + `glob-match.js` без regex; інжект у Council/audit; 12/12) · P4 оцінка обсягу на брамі + мʼякий поріг 60-74% контексту (ранній handoff; check-context-guard 6/6, жорсткий 75 не чіпано).
   - **В (gstack + best-practice):** P5 авто security-агент для чутливих глобів (`security.md` = CSO-методологія під наш стек, byyou Фаза 4) · P6 JSONL-журнал хуків (`log-event.js`, 6 подій, fail-open; 4/4; лог у .gitignore) · P7 /qa-explore + рівні Quick/Standard/Exhaustive + health-рядок; WebKit у CI підтверджено (був з 16.06).
   - **Модель:** Роман перемкнув сесію на Opus 4.8 посеред потоку (Fable→Opus).

### Метрики
Гілка `claude/new-session-26yz5s`. Деплої v1110 (потік «3 дірки») + v1113/v1114 (потік «брифінг+7 портів»). `src/` НЕ чіпався у жодному /byyou → CACHE не бампали (лише B-201 вранці). **Сторожів у CI: 22 node** (+refute/review-rules/context-guard/anchor/hooks-log/dnt) + ESLint. Нові workflow: canary.yml, ai-smoke.yml. Нові чисті модулі рев'ю (Supabase-ready, без regex): glob-match, resolve-anchor, refute-parser. **Двічі за 2 дні Pages падав «try again later»** — канарейка + самокорекція порожнім комітом відпрацювали обидва рази.

### 🔧 Фінальний день 11.07 (модель Opus, задачі OWL-розум)
7. **Розумніший OWL (`2d5b852`, деплой v1116):** промпт — приклад тону на кожен характер (coach/partner/mentor) + жорсткий ліміт «≤2 речення» (`prompts.js` getOWLPersonality). Пам'ять `nm_facts` — тимчасові факти (category `context`) авто-згасають за 90 днів (не копичаться вічно, клас «болить горло»); постійні (relationships/work/goals) не тонуть зі стелі 30 (`memory.js`, перевірено 4/4). Класифікатор `INBOX_SYSTEM_PROMPT` виявився вже окремим від особистості — не чіпав.
8. **Модель OWL — рішення записано у `docs/SUPABASE_MIGRATION_PLAN.md` п.10** (НЕ впроваджено): зараз gpt-4o-mini/OpenAI/BYO-key ~$1-2/міс. Рішення Романа «ключ буде мій на сервері» → перехід на **Claude Sonnet 5** (гібрид: mini на класифікацію, Sonnet 5 на поради) РАЗОМ із переносом ключа на Edge Function (щоб кордон `callLLM` зробити дводверним за раз, не двічі).
9. **AI-смоук 4→11 сценаріїв (`edca124`):** реальні фрази юзера + варіації маршрутів (3 фінанси-дієслова, 2 списки, звичка) + **негативна перевірка** (емпатія «немає сил» → OWL відповідає, задачу НЕ створює). Прибрано недільний cron — ганяємо за сигналом NM-Claude (після зміни промптів). Ключ і далі за Романом.
10. **Прибрання/коригування:** ідея Романа «Поділитися з OWL» (PWA share зараз + асистент-список з нативним застосунком) → ROADMAP (`85ce9c5`). Прибрано стару прив'язку `claude-fable-5` з `settings.json` (вікно минуло 07.07, сесія на Opus) + `byyou.md` 0.3 оновлено.
11. **`/fullaudit` ЗАБЛОКОВАНО 14.07** — винесено з `.claude/commands/` у `.claude/commands-archived/` (команда фізично не реєструється). Причина: застосунок ще активно будується, повний аудит зараз занадто дорогий (тестовий прогін 11.07 = ~2,64М токенів, вичерпав тижневий/сесійний ліміт за ~40 хв) відносно користі. **Активувати тільки на фінішній прямій/поліровці перед релізом.** Дизайн з фіксами (бюджет-гейт, severity-verify, чанкінг хвилями, fail-open) готовий, лишається у файлі + `_ai-tools/FULLAUDIT_TESTRUN_2026-07-11.md`. Розблокувати: `git mv` назад + прибрати банер.
12. **Токен-запобіжник** — новий хук `.claude/hooks/token-guard.js` (per-task стоп: один Agent-виклик >350К токенів → сесія на паузі до підтвердження Романа). Окремий від бюджет-гейту /fullaudit.

### ⏳ Відкриті хвости (чекають Романа / майбутніх сесій)
- **🔴 РЕАЛЬНИЙ БАГ з аудиту (потрійно підтверджений R=3, чекає фіксу) — системний клас «порожній fuzzy-fallback»:** `delete_habit` (`src/core/execute-action.js:228`), `edit_task` (:168), `edit_habit` (:144) — коли AI дає неіснуючий/галюцинований UUID а name/query undefined, `''.slice(0,6)=''` → `arr.find(x=>x.name...includes(''))`===`arr[0]` → **мовчки видаляє/редагує ПЕРШУ сутність**. `delete_task` захищений guard'ом `nameQ.length>=3` (QDIGl 04.05), решта — НІ. Діє у 7 з 8 чатів (Evening має власний безпечний handler). **Фікс системний:** guard `if(!h) return` / `nameQ.length>=3` у ВСІХ delete_*/edit_* execute-action, не точково. Через `/fix` або `/byyou`. Деталі → `_ai-tools/FULLAUDIT_TESTRUN_2026-07-11.md`.
- **Інші знахідки аудиту (не переверифіковані, вартують погляду):** N+1 ре-рендер бордів (boot.js/habits.js) · delete_project лише в Inbox · me.js:590 innerHTML monthLabel без escapeHtml · tool-filter неіснуючі tools · 32 «under-verified» у звіті прогону.
- **`/fullaudit` ЗАБЛОКОВАНО** (`.claude/commands-archived/`) — активувати ТІЛЬКИ на поліровці перед релізом. Дизайн-фікси по 2 осях (обсяг+темп) підготовлені для Брейна, не впроваджені (скіл спить).
- **🛡️ Токен-запобіжник АКТИВНИЙ** — один Agent-виклик >350К токенів зупиняє сесію (прапорець `.claude/hooks/.token-guard-tripped`, прибрати щоб продовжити).
- Створити ранкову Routine (значення → `_archive/BYYOU_PLANS_archive.md`, потік «брифінг»).
- Додати `OPENAI_SMOKE_KEY` (можна свій наявний ключ; або окремий $5) → перший ai-smoke (варто після зміни промпту OWL 11.07).
- iPhone-смоук: **тон OWL 11.07** (коротший/виразніший) + хвости v3pexs (списки-тап, OWL-групи, чіпи).
- Рішення: «Поділитися з OWL» (share target — невелика сесія) — робити чи ні.
- (Закрито 11.07: security-Issues #9/#12 верифіковано і закрито.)

---

## 🔧 Сесія v3pexs — Токен-баг `\b`-кирилиця у фільтрі + 2 AI-фічі + bareNoun-чіпи (27.06.2026)
...
## 🔧 Сесія gfrvu5 (20-23.06.2026) — архівовано 26yz5s 03.07 → [archive](../_archive/SESSION_STATE_archive.md#-сесія-gfrvu5--режим-byyou-напівавтономний-потік--дог-фуд-20-23062026)
...
**✅ `\b`-кирилиця SWEEP — ЗРОБЛЕНО v3pexs 27.06 (контроль brain):** суцільний grep `src/` — прямого `\b`+кирилиця в активному коді НЕМАЄ (усі збіги — коментарі-лекції або вже фікс через `CYR_BL/BR`). Єдиний `\b` у коді (`inbox-board.js:1251`) — після цифри `(\d{1,2})\b`, валідний (цифра=\w). Сторож `check-cyrillic-boundary` зелений + стереже весь src/ на push → клас закрито, рецидив неможливий тихо. **НЕ перевідкривати.**

**📋 ЧЕРГА (обговорено, чекає роботи):**
- ~~bareNoun node-тест~~ ✅ ЗАКРИТО v3pexs (автономний блок A — clarify-decision.js + 28/28).
...
- Потім — з ROADMAP (Supabase Ворота 2 / EU compliance) або нові ідеї Романа.

**🎯 НАСТУПНИЙ КРОК (після v3pexs):** усе автономне ВИЧЕРПАНО — далі лише задачі з Романом: (1) **send_chips** (структуровані чіпи, потрібен смоук); (2) **Ворота 2** структурний nm-data-changed 30 точок (нагляд за табло); (3) **Supabase Фаза 2** — створення проекту (EU-регіон, акаунт Романа) → схема+RLS+міграція; (4) iPhone-смоук хвостів (нотатки після D4, списки-тап, OWL-групи). Модель: Fable 5 до 07.07 (потім /byyou сам відкотить settings).
