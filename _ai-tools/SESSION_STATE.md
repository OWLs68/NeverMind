...
**📋 ПРІОРИТЕТНИЙ ПОРЯДОК (історичний — bOqdI/BqTWF закрили частину):**

1. ~~**🛠 Створити `/council` скіл через `/prompt-engineer`**~~ — **закрито у bOqdI 02.05.** Council реалізовано як секція `## 🧠 Council` у `CLAUDE.md` (24 рядки) — не скіл, не лог, не метрики 👍/👎. Концепт `_archive/COUNCIL_CONCEPT.md` лишився як ілюстрація overengineering 4 ітерацій з Gemini.
2. ~~**🚨 Архівація LW3j8 + 6ANWm**~~ — **закрито у BqTWF 02.05** (Phase 1 з CLEANUP_PLAN, коміт `b953825`).
3. **🚨 iPhone smoke-test v553+** — БОРГ 14+ сесій. Council секція у CLAUDE.md + 3 фікси evening/health/proactive (`19e112f`/`25e60da`/`8c3fe8d`) + категорії папок нотаток (`Здоровя` без апострофа).
...
**⚠️ ФЛАГНУТО — НЕ ВИРІШЕНО:**
- iPhone smoke-test v553+ — БОРГ 14+ сесій.
