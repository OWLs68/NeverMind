...
## Зараз робимо
✅ Виправлено deploy pipeline v2 — deploy тепер прямо в auto-merge.yml.
...
- B-11/B-12 — **НЕ виправлено з точки зору користувача**. В коді є `padding:28px 48px` (index.html рядки 958 і 1129), але deploy був зламаний → на GitHub Pages стара версія без padding. Потрібно: змінити `48px` → `20px` (саме стільки просив Роман) і задеплоїти.

## Наступне
- B-11/B-12: змінити в index.html рядки 958 і 1129 — `padding:28px 48px calc(...)` → `padding:28px 20px calc(...)`. Оновити sw.js CACHE_NAME. Запушити.
