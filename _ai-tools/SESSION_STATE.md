...
4. **Cleanup аудит** (другий Council 5 агентами + 3 верифікатори впали у rate limit `resets 1:10pm UTC`) — знайшли 33+ протиріч у документації. План з 8 фаз збережено у `_ai-tools/CLEANUP_PLAN_bOqdI.md` (`b33e9ff`) для нового чату. Топ-критичні: 4 битих посилання `_archive/COUNCIL_CONCEPT.md`, пріоритет SESSION_STATE «Створити /council скіл» застарілий, версія v494 → v551, `start.md:24` містить скасоване правило «швидкий діалог», `/obsidian` згадки без файлу.
...
8. **Чернетка `/council` концепції** (`921b7ff`) — `_archive/COUNCIL_CONCEPT.md` (171 рядок) як результат 4 ітерацій з Gemini. **НЕ скіл, не архітектура** — нотатка для нової сесії через `/prompt-engineer`. Покриває: філософію (чесно «частково театр»), термін життя 3-6 місяців, повну спеку v1.1 (5 ролей з #SkipReason, conditional synthesis Голови, mobile-формат з прикладом «темна тема», 👍/👎 флоу), 4 відкриті дірки.
...
- 4 відкриті питання з `_archive/COUNCIL_CONCEPT.md` розділ 4 (тригер localStorage Schema, релевантність пошуку у LOG, хибна тривога No Echo, конкуренція з `/audit`/`/fix`).
