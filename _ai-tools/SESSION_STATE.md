...
**Оновлено:** 2026-04-13 (сесія start-session-gQ2Hl — закриття боргів 3.6-3.13 + Фази 1.1/1.2 + записка для наступного чату про консолідацію плану)
...
## 🔥 ПРІОРИТЕТНА ЗАДАЧА ДЛЯ НАСТУПНОГО ЧАТУ (від 2026-04-13)

**Задача:** консолідувати план проекту в один файл `ROADMAP.md` у корені репо. Зараз план розкиданий по 3+ місцях (`FEATURES_ROADMAP.md`, цей `SESSION_STATE.md`, `CONCEPTS_ACTIVE.md`) — новий чат губиться де шукати.

**Важливо:** перед виконанням **прочитай УВАЖНО** `FEATURES_ROADMAP.md` повністю (600+ рядків). Не роби "на пам'ять" — саме цю помилку я зробив у попередній сесії і Роман спіймав.

**Чому винесли задачу в окремий чат:** попередня сесія (gQ2Hl) була довга (9 коммітів, багато архітектурних обговорень). Створення довгоживого артефакту (ROADMAP) зі втомленим контекстом — ризик помилок. Новий чат стартує чистим через `/start`, читає свіжими очима — якість вище.

**Стан на старт:** робоче дерево чисте (все попередньо закомічено і запушено). Нічого з файлів плану ще не архівовано — ти починаєш з нуля.

### Кроки (дрібно, щоб не зловити API Stream idle timeout при великому Write)

1. **Кістяк `ROADMAP.md`** у корені — короткий Write (~80 рядків) з маркерами-заглушками (`<!-- NEXT_BLOCK_1 -->`, `<!-- DONE_PLACEHOLDER -->` тощо)
2. **Заповнити Next — 7 блоків** послідовно через Edit'и (по одному блоку за раз, щоб кожен Edit був малим)
3. **Заповнити Done** — останні 15-20 закритих пунктів з датами (список нижче у секції "Що зроблено в цій сесії" + з `_archive/FEATURES_ROADMAP.md` таблиця "Що мозок вже вміє")
4. **Ideas + Rejected + After Supabase** — коротко, з посиланням на архів за деталями (не копіюй весь Grok/Gemini/DeepSeek — це 300+ рядків, просто дай посилання)
5. **Архівувати** — `git mv FEATURES_ROADMAP.md _archive/FEATURES_ROADMAP.md`
6. **Архівувати старий SESSION_STATE** — `git mv _ai-tools/SESSION_STATE.md _archive/SESSION_STATE_2026-04-13.md`
7. **Новий компактний `SESSION_STATE.md`** — короткий (~30-40 рядків): версія + гілка + останні 3-5 сесій коротко + посилання на ROADMAP. **БЕЗ цієї секції 🔥 ПРІОРИТЕТНА ЗАДАЧА** — щоб наступний чат після тебе не думав робити знову
8. **Оновити посилання:** `CLAUDE.md` (секція "Карта документації"), `START_HERE.md` (список обов'язкових файлів), `.claude/commands/start.md` (якщо існує). Замінити всі посилання на `FEATURES_ROADMAP.md` → `ROADMAP.md` + додати примітку "Перенесено у ROADMAP.md 2026-04-XX".
9. **Grep** по репо на `FEATURES_ROADMAP` і `SESSION_STATE` щоб не лишилось висячих посилань (окрім `_archive/`)
10. **Один коміт** `docs: consolidate plan into ROADMAP.md` + push

**CACHE_NAME НЕ чіпати** — це чисті документаційні зміни (правило з `CLAUDE.md`).
...
## 🚀 Active — поточний пріоритет (1-2 пункти, оновлюється після кожної сесії)
## 📋 Next — послідовні блоки робіт (7 блоків нижче)
## ✅ Done — закриті пункти з датами (останні 20)
## 💡 Ideas — для обговорення (Grok/DeepSeek/Gemini, посилання на _archive/FEATURES_ROADMAP.md)
## 🚫 Rejected — відхилені з причиною
## 🔒 After Supabase — окремий блок
```

### Послідовність 7 блоків (для секції Next, узгоджено з Романом 2026-04-13)

**Блок 1 — Малі фікси з великою цінністю (швидкі перемоги, ~1 сесія кожен):**
- `set_reminder` фікси — створити картку в стрічці Inbox, автоствор `nm_events` з часом, додати категорію "Нагадування" в `CAT_DOT_BG` / `CAT_META`
- 4.5 Блок табло коли чат відкритий
- G9 Page Visibility fix (не палити API коли вкладка прихована)
- G11 "Дія замість питань" (правило промптів — завжди чіпи з діями)
- G12 Мікро-розмови (ліміт діалогу 2-3 повідомлення на тему)
- 4.40 Auto-silence (ignored_3 → silence_4h)

**Блок 2 — Концепції вкладок (core UX):**
- Аудит стану 5 вкладок — прочитати код + порівняти з `CONCEPTS_ACTIVE.md`, дати статус (30 хв)
- Фінанси — доробка (hero-карта % заощаджень, 3 інсайти, прогноз, коуч-блок OWL, графік тижнів, категорії 🟢🟡🔴 + benchmark 50/30/20)
- Вечір — доробка (кільце продуктивності, настрій 5 емодзі, підсумок OWL, моменти)
- Я — доробка (огляд тижня одне речення від OWL, теплова карта 14-30 днів, патерни OWL раз на тиждень, аналіз)
- Проекти — доробка (4-питання інтерв'ю через Inbox, карта проекту з кроками+труднощами+ризиками+ресурсами, живий супровід, аналітика)
- Здоров'я — доробка (денні шкали енергія/сон/біль, картки)

**Блок 3 — Ключові UX-розширення:**
- Inbox стрічка редизайн (компактні картки з кольоровою крапкою + датові сепаратори + закріплені нагадування зверху)
- Динамічний розпорядок дня (ключі за датами `{'2026-04-10': [...]}` замість `{mon: [...]}`, автододавання блоків з часом, детектор конфліктів)
- G4 Shadowing (конфлікти розкладу — дубль-перевірка до попереднього)
- G5 All-day events = "Фокус дня" (події без часу показуються окремо зверху)
- G6 Дедуплікація нагадувань (linked_event_id)
- Голосовий ввід (Web Speech API, `lang: 'uk-UA'`, іконка мікрофона вже є в UI)

**Блок 4 — Мозок OWL: інструменти AI (потрібно перед покращенням якості):**
- 4.11 `getAIContext` повне покриття (проекти/події/здоров'я/моменти — зараз не всі типи даних у контексті)
- 4.9 Повний набір CRUD tools (delete_note, delete_finance, edit_moment, delete_memory_fact, tools для проектів/здоров'я)
- 4.10 Tool calling на всіх чат-барах (залежить від 4.9 — tab chat bars зараз на текстовому форматі без tools)
- G13 Brain Dump (покращити парсинг суцільного тексту, частково працює)

**Блок 5 — Мозок OWL: якість рішень:**
- 4.4 Relevance Scoring (локальний бал 0-10 чи варто OWL говорити)
- 4.20 + 4.39 Negative Memory + Override ("відчепись" кнопка) — консолідувати в одну фічу
- 4.6 Contextual Personality (режими planning/flow/struggling/celebrating/reflecting/returning)
- G1 Тиша як модифікатор (Judge Layer уточнення — тиша без контенту = 0 балів)
- G2 Burst Detection (антиспам серій — 3+ закриття за 2 хв → OWL мовчить, збирає в чергу)
- G3 Характери → різний поріг Judge Layer (coach=4, partner=2, mentor=3)
- G10 Velocity tracking (темп юзера, тригер velocity-drop +3)
- 4.21 Verify Loop (перевіряти результат дії після виконання)

**Блок 6 — Довершення:**
- 4.7 Мовчазний запис (AI сам зберігає факти без команди — частково через save_memory_fact вже є)
- 4.8 Smart Notification Queue (черга повідомлень OWL з пріоритетом)
- 4.41 Mirror Mode (мікро-чіп 😊/😐/😣 після задач/звичок + аналіз через 3-4 дні)
- 4.43 Conversation Threading Фаза A (локальний `nm_thread_index` за ключовими словами)
- Геймфікація — обговорити деталі → імплементація (назви рівнів, точні числа балів, перед імплементацією обговорити з Романом)

**Блок 7 — Адаптивність Android + desktop (в кінець, за рішенням Романа):**
- Фаза A — Android fit (тестування на реальному Android, ревізія `src/ui/keyboard.js`, заміна hardcoded pixel'ів на `min()/max()/clamp()`)
- Фаза B — desktop media queries (великі екрани, правильні пропорції)

**Після цього — Supabase (окрема велика фаза).**

### Що зроблено в цій сесії (gQ2Hl, 2026-04-13) для секції Done

- `fix(memory)` d61b15b — захист пам'яті від сміття (жорсткий промпт save_memory_fact + код-фільтр у addFact + прибрано фонове вгадування _backgroundExtractFacts)
- `refactor(owl)` 11c41cd — Фаза 1.1 Об'єднання табло Inbox і табло вкладок в один API (`getBoardContext`, `tryBoardUpdate`)
- `feat(ai)` c4112ca — Фаза 1.2 Чат бачить історію табло (3 останні повідомлення з часовими мітками)
- `feat(owl)` c8741fd — Фаза 3.11 Навігаційні чіпи на всі вкладки (підсилено CHIP_PROMPT_RULES)
- `feat(ai)` 0c24676 — Фаза 3.7 Правило емпатії в universal personality (усі чати + табло, 21 маркер замість 8)
- `feat(owl)` 142c816 — Фаза 3.13 OWL запитує про комфортний темп + автоматичний розрахунок 20%
- `feat(owl)` 5785819 — Фаза 3.8 Прокрастинація detector (rescheduleCount при edit_task)
- `feat(owl)` c379bd2 — Фаза 3.6 Smart Boot-up (прострочені задачі + first-open-today тригер + SMART BOOT-UP правила в промпті)
- `feat(owl)` 66b54e0 + a661e70 — Фаза 3.10 Кореляції між вкладками (4 патерни, 3 джерела даних: nm_habit_log2 + nm_health_log + nm_inbox + nm_moments + nm_finance)

### ⚠️ КРИТИЧНО — ПІСЛЯ ПОВНОГО ВИКОНАННЯ ВИДАЛИ ЦЮ СЕКЦІЮ

Коли всі 10 кроків вище зроблені **і закомічено** — у новому компактному `SESSION_STATE.md` який ти створиш на кроці 7 **НЕ копіюй цю секцію "🔥 ПРІОРИТЕТНА ЗАДАЧА"**. Інакше наступний чат після тебе подумає що задачу треба робити знову.

Якщо виконав лише частину кроків (наприклад 4 з 10) — секція залишається для продовження в наступному чаті.
