...
**Оновлено:** 2026-04-20 (сесія **Gg3Fy** — **Шар 1 "Один мозок V2" ЗАВЕРШЕНО: всі 8 чатів на INBOX_TOOLS + єдиний dispatcher. 6 комітів. Потребує iPhone-тесту — B-94/B-95 мають закритися архітектурно.**).

---

## ⚠️ ДЛЯ НОВОГО ЧАТУ — найважливіше

1. **🧪 ПЕРШИМ ДІЛОМ — iPhone-тест Gg3Fy деплою.** Після авто-мержу гілки `claude/start-session-Gg3Fy` у main і білду (~3 хв) зайти у v340+ на телефоні і перевірити шість фраз: "Алергія на пил" / "Завтра прийом у лікаря о 10" / "Болить горло 3 дні" / "Перейди до фінансів" / "Переключись на Ментора" / "Запам'ятай що не їм глютен". Очікування: перші три → відповідні CRUD дії (add_allergy/create_event/add_health_history_entry), четверта-п'ята → UI tools (switch_tab/set_owl_mode), остання → save_memory_fact. Якщо всі 6 спрацюють — **B-94/B-95 закрити у NEVERMIND_BUGS.md**.
...
|---|---|---|---|---|---|
| **Gg3Fy** | 20.04 | ✅ Шар 1 "Один мозок V2" ЗАВЕРШЕНО: створено `ai/tool-dispatcher.js`, мігровано Health/Finance/Projects на INBOX_TOOLS, додано 10 нових tools. 6 комітів. B-94/B-95 мають закритися (потребує iPhone v340+). CACHE_NAME nm-20260420-2040→2159. ROADMAP Шар 3 переписано. | 6 | `claude/start-session-Gg3Fy` | [CHANGES §20.04-Gg3Fy](../docs/CHANGES.md) |
