...
1. **🧪 ПОВТОРНИЙ iPhone-тест B-96 v343+.** Зайти і ввести "Болить горло" при активній картці "Шляпа" або без активної. Очікування: (а) зʼявляється загальна картка "Здоровʼя" з записом "болить горло", (б) НЕ створюється нова картка "Горло" / "Шляпа" тощо, (в) у Нотатках у папці "Здоровʼя" дубля НЕМАЄ (бо разовий симптом). Потім тест: "Вже тиждень болить спина" → очікується і картка Здоровʼя запис, і нотатка у Нотатках папка "Здоровʼя" (бо тривалий). Якщо спрацює — B-96 закрити у NEVERMIND_BUGS.md.
...
| Параметр | Значення |
|---|---|
| **Версія** | v342+ на проді очікується після мержу (v340 iPhone тест пройшов) |
...
| **Гілка** | `claude/start-session-Gg3Fy` (Шар 1 "Один мозок V2" ЗАВЕРШЕНО + B-94/B-95 закриті, B-96 три ітерації фіксу) |
| **CACHE_NAME** | `nm-20260421-0445` |
