...
**Оновлено:** 2026-04-24 (сесія **R5Ejr** — **UI-pass Продуктивності + stale board fix через completedAt + стандарт карток 5/10 у 7 вкладок + 1DAY analysis**).

---

## ⚠️ ДЛЯ НОВОГО ЧАТУ — найважливіше

1. **⚠️ ВЕРИФІКУВАТИ НА iPhone після v386+ деплою (R5Ejr):**
   - **Sticky header Продуктивності** — усі помаранчеві фони прибрано, картки при скролі проходять **повністю видимо** за таблом/чіпами/перемикачем.
   - **Анімація закриття задачі:** тап ✓ → одразу зелена галочка + перекреслений тайтл → чверть секунди → картка плавно сповзає (350мс) → рендер і вона у виконаних зверху.
   - **Виконані сортуються** за часом закриття ↓ (найсвіжіша одразу під активними).
   - **Тач-зона галочки 44×44** (візуально 28×28) — тапи повз квадратик ставлять галочку, не відкривають редагування.
   - **Стандарт карток 5/10 скрізь:** Inbox, Задачі, Звички, Нотатки, Папки, Картки здоров'я, Проекти.

2. **🧪 Test Sprint — все ще пріоритет.** Розширити `test-checklist.html` з 13 секцій до ~30 (8 блоків). Детальний зміст у CHANGES §24.04-8bSsE. Після R5Ejr тестовий обсяг виріс — треба додати секцію UI-карток.

3. **🔴 B-103 ВІДКРИТИЙ** — дублі подій у календарі. 5 call-sites без dedup helper. Фікс: `addEventDedup(ev)` у `calendar.js` → делегують з inbox/habits/tasks/notes/health.

4. **🐛 Відкриті середні баги** (після R5Ejr):
   - **B-100** 🟡 Сова не реагує на «не доставай / відчепись».
   - **B-101** 🟢 Туманне «Щось пішло не так» на мережеві помилки.
   - **B-102** 🟡 Табло ігнорує настрій у чаті (архітектурно V3 Фаза 2).

5. **🆕 CSS-змінні карток** (R5Ejr): `--card-gap`, `--card-pad-y`, `--card-pad-x` у `:root`. Всі нові картки — через ці токени. Єдина точка редагування висоти.

6. **📝 План vs Факт у Вечорі + Місячний/Річний AI-звіт** — записано у ROADMAP/EVENING_PLAN (R5Ejr після аналізу 1DAY). Не брали: тайм-трекінг по годинах, кольорові теги, відволікання (як окремий запис).

7. **🧠 OWL Reasoning V3 — Active після Test Sprint.** 8 фаз до Supabase. Порядок у ROADMAP.md.

8. **CACHE_NAME** актуальне: `nm-20260424-1906` (R5Ejr).

9. **Workflow Романа:** "Роби" → один таск → звіт → пропозиція → чекати. Файли >250 рядків — skeleton+Edit, checkpoint-коміт після кожної фази, ≤25 слів між tool calls. **Довгі списки/шпаргалки/промпти** — через HTML з кнопкою Копіювати.

10. **🚫 НЕ стандартизовано у R5Ejr:** транзакції Фінансів і моменти Вечора — стрічкові списки з `border-bottom`, не картки з фоном. Якщо Роман захоче — окрема задача.
...
| Параметр | Значення |
|---|---|
| **Версія** | Очікується v390+ після мержу R5Ejr (9 комітів) |
| **URL** | owls68.github.io/NeverMind |
| **AI модель** | OpenAI GPT-4o-mini з Tool Calling (56 tools INBOX_TOOLS + post_chat_message) |
| **Гілка** | `claude/start-session-R5Ejr` (9 комітів, запушено) |
| **CACHE_NAME** | `nm-20260424-1906` |
...
## 🔧 Поточна сесія R5Ejr — UI-pass Продуктивності + stale board fix + 1DAY analysis (24.04.2026)
