...
- 🥇 **ПЕРШЕ НАСТУПНОЇ СЕСІЇ (рішення Романа A, після його iPhone-смоуку):** винести чисту логіку рішення `shouldClarify` (bareNoun) з `src/owl/clarify-guard.js` у `src/data/` → стане node-тестованою (як зробили з tool-filter). Зараз покрито ЛИШЕ емуляцією 8/8 — реального тесту нема, регрес у 7 чатах може пройти тихо. Чистий рефактор (поведінка не міняється) + новий `check-*`-сторож. ~15 хв.
...
**⛔ ЗАМОК НА НОВЕ:** не пушити нові фічі поверх неперевіреного — спершу iPhone-смоук усього v3pexs (списки/OWL-групи/чіпи/фінанси/момент). Brain-контроль #3 27.06.
