...
**Оновлено:** 2026-04-29 (сесія **H0DxS** — fix-сесія: тижневий зріз звичок у getAIContext + bump weekly insights cache; правило «ЗОВНІШНІ ФАКТИ» проти галюцинацій AI у всіх 8 чатах; «Відкрий звички» з чату Я тепер реально відкриває підтаб Звички; блок «🦉 OWL знає тебе» видно на Я (білий фон + темніша рамка + тінь); видалено онбординг-модалку при першому вході — поля у Налаштуваннях).
...
| Параметр | Значення |
|---|---|
| **Версія** | v488 (deploy 29.04 22:10 після TdIqO auto-merge); H0DxS очікує v489+ |
| **URL** | owls68.github.io/NeverMind |
| **AI модель** | OpenAI GPT-4o-mini з Tool Calling (57 tools, 5 task-tools з `task_id: string`) |
| **Гілка** | `claude/start-session-H0DxS` (тижневий контекст звичок + правило проти галюцинацій + закриті 2 баги lRnXU + видалено онбординг) |
| **CACHE_NAME** | `nm-20260429-2340` (bump 4 рази за фікс-коміти) |
...
## 🔧 Поточна сесія H0DxS — фікс-сесія: тижневий контекст звичок + правило проти галюцинацій + закриті 2 баги lRnXU + видалено онбординг (29.04.2026)
...
## 🔧 Сесія lRnXU — quick dialogue mode + TESTING_LOG.md + перебудова графіка «Я» (29.04.2026) — _попередня_
