...
**Оновлено:** 2026-04-24 (сесія **8bSsE** — **обговорення + діагностика: новий критичний баг B-98 залиплий прапорець OWL табло, баг логгера B-99 brain-pulse skip без причини, розширена шпаргалка для великого тесту, прохання Романа запам'ятати правило "кнопка копіювати у довгих списках"**).

---

## ⚠️ ДЛЯ НОВОГО ЧАТУ — найважливіше

1. **🔴 B-98 «Залиплий прапорець OWL табло» — НОВИЙ критичний баг.** Діагностика Романа 24.04 показала: табло не оновлюється 8+ годин попри `stale message detected, forcing generation` у логах 5+ разів за день. Симптом: функція генерації зветься, прапорець «я вже генерую» не скидається (ймовірно упав проміс без `.catch()`, `finally` не виконався). Файл: `src/owl/board.js`. Фікс: обгорнути генерацію у `try/finally` + timeout на прапорець.

2. **🟡 B-99 «brain-pulse skip: без причини» — баг логгера.** У логах Романа і друга стрінга `[brain-pulse] skip:` іноді без причини (пуста), іноді з `silent-phase`. Файл: `src/owl/brain-pulse.js`. Фікс: гарантувати reason у кожному `logSkip()`, fallback `unknown`.

3. **🧪 Test Sprint триває.** Друг Романа вже тестував 22.04 з v368 (не активно — 2 API запити за 6 год, 1+1 запис). Роман хоче виділити **1-2 дні суто для тестів** за розширеною шпаргалкою (див. нижче блок 8bSsE «Розширена шпаргалка»). Після Test Sprint → можна відкривати V3.

4. **📋 Прохання Романа до Claude (НЕ ВИКОНАНО, чекає рішення):** коли Claude дає довгі списки/шпаргалки/промпти для Gemini — завжди давати **HTML з кнопкою «Копіювати»** (як `test-checklist.html`), бо з телефону неможливо виділити «кілометрові повідомлення». Роман попросив «запам'ятай або зроби хук». **Варіанти:** (а) правило у CLAUDE.md секція «📏 РОЗМІР ВІДПОВІДЕЙ», (б) новий скіл `/copyable-list`, (в) UserPromptSubmit хук який детектує «список/шпаргалка/промпт для Gemini» → нагадує створити HTML. **Треба підтвердити Романом перед записом.**

5. **🔴 B-97 «Прийом у лікаря відміни» — все ще відкритий.** Агент у чаті Задач не викликає `delete_event`. Діагноз (Gemini 2 ітерації): Context Segmentation Failure. Фікс: OWL Reasoning V3 Фаза 1 (`_reasoning_log`).

6. **🧠 OWL Reasoning V3 — Active після Test Sprint.** 8 фаз до Supabase. Порядок: 0 Usage Meter → 1 `_reasoning_log` → 1.5 Dynamic Tool Loading → 3 лог корекцій → 4 typed cooldowns + is_silent → 2 Brain Pulse USER_STATE → 5 характер → 6 Lazy Profile Builder. Повний опис у ROADMAP.md.

7. **💰 Економіка узгоджена:** підписка $10-12/міс. Поточна вартість $4/міс. Після V3 ~$2/міс → маржа ~80%. Правило: оптимізації ПІСЛЯ даних з Usage Meter (Фаза 0 V3).

8. **📊 Шкала розумності ~20%.** Стеля ROADMAP 45-48%. Оновлювати після кожної V3-фази.

9. **🧩 Архітектурне розʼяснення (8bSsE):** Brain Pulse фізично **не може** працювати у фоні на iPhone PWA (iOS заморожує вкладку коли застосунок закритий). Справжнє фонове мислення — тільки після Supabase + Edge Functions + push-нотифікацій. Роман зрозумів і прийняв.

10. **CACHE_NAME** актуальне: `nm-20260422-0639`. Не змінювався у 8bSsE (без коду).

11. **Workflow Романа:** "Роби" → один таск → звіт → пропозиція → чекати. Файли >250 рядків — skeleton+Edit, checkpoint-коміт після кожної фази, ≤25 слів між tool calls.
...
| Параметр | Значення |
|---|---|
| **Версія** | v370 на проді (після L67Xf). Сесія 8bSsE — без коду, без деплою |
...
| **Гілка** | `claude/start-session-8bSsE` (0 комітів — сесія обговорення) |
...
**Прохання Романа «запамʼятай про кнопку копіювати»:**
Коли Claude дає довгі списки, шпаргалки, промпти для Gemini — Роман не може виділити текст з мобільного Claude Code. Просить або правило, або хук. **Варіанти для запису:**
- **А)** Правило у `CLAUDE.md` секція «📏 РОЗМІР ВІДПОВІДЕЙ»: «Якщо відповідь містить список/шпаргалку/промпт довше 30 рядків → пропонуй HTML-файл з кнопкою "Копіювати"».
- **Б)** Новий скіл `/copyable-list <тема>` — генерує HTML по шаблону `test-checklist.html` (кнопка "Копіювати весь текст" зверху).
- **В)** Хук `UserPromptSubmit` з тригерами «список / шпаргалка / промпт для Gemini / чеклист / копіювати» → нагадує Claude створити HTML.
- **Г)** Комбінація А+В (правило + хук-нагадування).

**Чекає явного рішення Романа.**
