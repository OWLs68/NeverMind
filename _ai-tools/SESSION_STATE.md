...
- Черга gfrvu5 без змін: правило категоризації потоку OWL (чекає «Роби») + фіча списків в Inbox.

### Метрики
- Гілка `claude/new-session-v3pexs`. Коміти `04a3285`→`c34b3c6` (~15, 3 батчі /byyou). CACHE `nm-20260627-1027`. Деплой v1082.
- E2E #27 зелений. Council: flow-мапа 8 чатів + false-positive хантер + архітектор (Sonnet) для #3.
...
**📱 SMOKE v3pexs на iPhone (нове, чекає):** одне слово «Хімчистка» в Inbox/Фінансах/Нотатках → клікабельні чіпи «куди записати?» (тапаються) · «Дякую/Окей» → без чіпів · тап по чіпу зберігає · «купив каву 50» працює як раніше · фінанси показують заголовки дат · момент з місцем → «у Моменти (Вечір)».
