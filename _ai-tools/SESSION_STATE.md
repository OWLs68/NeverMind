...
**Оновлено:** 2026-04-20 (сесія **NRw8G** — **9 багів закрито + Один мозок parity**: чіпи JSON-regex, темна тема галюцинація, модалка Пам'яті свайп, "Запам'ятай" тригер, мертва `log_health`, Здоров'я чат (алергія/симптом/подія), свайп handle 22px→40px. Плюс: `save_memory_fact` додано у Health/Finance/Projects чати.)
...
**4. 🔜 ПІСЛЯ iPhone-тесту — продовження з ROADMAP:**
  - **"Один мозок #2 A продовження"** — перевести Finance/Health/Projects на повний INBOX_TOOLS (не тільки save_memory_fact). Зараз вони text-JSON. 2-3 сесії.
...
- **[Пріоритет 1]** Тест на iPhone v323+ — перевірити 9 багів (повний список у секції "ДЛЯ НОВОГО ЧАТУ" пункт 3).
- **[Пріоритет 2]** "Один мозок #2 A продовження" — Finance/Health/Projects чати на повний INBOX_TOOLS, не тільки save_memory_fact.
- **[Пріоритет 3]** Я 65→100% або Проекти 65→100% з ROADMAP.
