...
**Оновлено:** 2026-06-13 (сесія **v1d9eo** — голос: дослідження (3 агенти, ~50 джерел) → відхилено OpenAI Realtime, обрано КОНВЕЄР (Deepgram STT → теперішній мозок → ElevenLabs стрім); 2 інтерим-фікси (єдиний двигун = вибраний голос; чанк-озвучка для миттєвого першого слова); переписано VOICE-план; календар-баг визнано побічкою голосу (фікс 02038bd)).

---

## 🔧 Поточна сесія v1d9eo — Голос: дослідження + 2 фікси + новий план (13.06.2026)

### Зроблено (коміт `632a10c`, запушено)

**Дослідження (Council 3 Sonnet-агенти + web):** Realtime vs альтернативи vs Supabase-кут. Два незалежні агенти зійшлись: OpenAI Realtime — **погано для iOS-PWA** (Apple вбиває веб-голос у фоні — обходу нема; cost-bomb re-білінг історії ~×100 за 20хв; VAD ламає function-calling + обриває мовця; echo на iOS Safari). **Рішення Романа: КОНВЕЄР** «почув(Deepgram)→теперішній GPT-4o-mini мозок БЕЗ переробки→сказав(ElevenLabs стрім)» — 4-12× дешевше, без WebRTC-засідок, найкраща українська, реюз «одного мозку».

**Brain/Fable Романа незалежно зійшовся з агентами** (конвеєр, не Realtime) → впевненість висока. Інтерим доведено до його специфікації.

**4 інтерим-зміни `voice-output.js` + `index.html`:**
- **«голос не той»** — список у Налаштуваннях = єдине джерело правди (раніше ключ ElevenLabs мовчки перекривав вибір OpenAI зі списку). Додано пункт «ElevenLabs» + одноразова міграція `ttsEngineMigrated`.
...
- **Дозвіл мікрофона на зеленій — СПРОБА + ВІДКАТ.** Додав `ensureMicPermission` (getUserMedia) на зелену кнопку → РЕГРЕСІЯ: на iOS getUserMedia перемикає аудіо-сесію у режим запису → агента НЕ ЧУТИ (TTS 200 але глухо — підтв. діагностикою Романа: `audio/speech 200 1305мс`) + мік пише погано. Відкочено. **Урок:** на iOS-вебі дозвіл мікрофона не спитати без побічки (єдиний спосіб «спитати» = відкрити мік = зламати аудіо-маршрут). Дозвіл лишається за SpeechRecognition (червоний мік) при реальному тапі.
- **🗄️ ГОЛОСОВИЙ РЕЖИМ АРХІВОВАНО** (рішення Романа після тестів — TTS на iOS-вебі ненадійний). `voice-output.js` відключено з `app.js` (import закоментовано, файл збережено), прибрано блок «Голос Агента» з Налаштувань. **Лишився лише нижній мік (`voice-input.js`) для диктування тексту** — працює standalone (без voice mode → pendingSendClick не ставиться → диктує у поле, юзер сам шле). Голос повернеться великою збіркою-конвеєром після Supabase (`docs/VOICE_REALTIME_PLAN.md`).

**E2E + CI (друга половина сесії, через MCP):**
- **E2E переїхав Hetzner → GitHub Actions** (безкоштовно, я перевіряю прогони через MCP). `.github/workflows/e2e.yml` + `playwright.config.js` + `tests/e2e/smoke.spec.js` (2 зелені: boot, навігація 8 вкладок). Інтеракційні тести — Крок 2, батчами з трейсами.
- **Hetzner вимкнено програмно:** `tester-config.json` `enabled:false` + `/start` Крок 1.5 пропускає heartbeat коли disabled (без «тестер мертвий»). Фізичне вимкнення VPS — за Романом у панелі Hetzner.
- **Claude Security Scan полагоджено** (3 шари): порожній diff→чанк (grep-фільтр + skip empty); модель `claude-sonnet-4-5-20250929`→`claude-sonnet-4-6` + прибрано зайвий 1M-beta; **справжній корінь — на акаунті Anthropic закінчились кредити** → graceful-skip при білінг/кредит/auth (CI зелений, без спаму). Рішення Романа: поповнити кредити чи лишити (gitleaks стереже секрети окремо).
- **Auto-merge:** workflow-файли бот змерджити не міг → 4 ручні PR (#13-#16) через MCP. Тепер workflow у main → майбутні non-workflow коміти auto-merge'аться знову.
- Коміти `632a10c`→`d011a7c` + merge-коміти. CACHE: `nm-20260614-1240`.

**Відкрите рішення велика сесія:** TTS OpenAI-tts-only (brain) vs +ElevenLabs опція без акценту (агенти). Занотовано у плані.

**Документ:** `docs/VOICE_REALTIME_PLAN.md` переписано Realtime→Конвеєр (розділ «чому НЕ Realtime» з джерелами, фази, Supabase-кут, інтерим).

### Відкрите / смоук за Романом (на iPhone, з хмари не чути)

- Засмоукати голос: вибір у Налаштуваннях = що чуєш (і «Прослухати», і чат); перше слово швидше.
- **Велика збірка конвеєра** (Deepgram STT + ElevenLabs стрім + ключі/ліміти на Edge) — окрема сесія **після Supabase**.
- **Календар-баг (A):** трасування коду — механізм закриття справний; найімовірніше була побічка завислого голосу (фікс `02038bd`). Перевірити на чистому старті: тап календар у шапці Inbox → закрити (фон/свайп). Якщо ок — закрити; якщо ні — відео-репро.

### Метрики

- Коміт: `632a10c`, гілка `claude/new-session-v1d9eo`, запушено. CACHE_NAME: `nm-20260613-1955`.
- Council: 3 Sonnet (Realtime-факти / red-team+альтернативи / Supabase). Build: node --check + check-imports + check-i18n чисте (esbuild у хмарі нема — bundle збирає CI).
...
- ⏳ **Лишилось у Фазі 1:** (а) **проекти** — 2 точки створення без фабрики → `makeProject` + конверт; (б) **Ворота 2** — структурний `nm-data-changed` `{type,action,id}`. Council: 7/9 слухачів ламаються → strangler-shim (слухачі читають обидва формати → потім флоп). **⚠️ РИЗИК: не верифікувати з хмари** (зламаєш слухача = табло мертве), потрібен iPhone; (в) **структуровані чіпи** `send_chips` (B-194 клас).
- ⚠️ **maintenance конверта на edit'ах** (updated_at/hlc при кожній мутації) = **Фаза 2** (sync), НЕ Фаза 1. Зараз конверт лише на створенні.
