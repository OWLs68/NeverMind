...
| Питання | Файл / секція |
|---|---|
| Правила взаємодії з Романом (пояснення у дужках, розмір, "Роби") | `CLAUDE.md` верх — "Обов'язково в кожному повідомленні", "Розмір відповідей", "Процес роботи" |
| Хто Роман, як комунікувати | `РОМАН_ПРОФІЛЬ.md` |
| Глибина відповіді (читай код перед оцінкою) | `CLAUDE.md` → "Глибина відповіді" |
| Великі файли / checkpoint-коміти / мінімум тексту між tool calls | `CLAUDE.md` → "Робочий процес (надійність проти обривів)" |
...
- `hooks/` — автоматичні нагадування:
  - `rules-reminder.sh` — правила CLAUDE.md кожне 5-те + на тригер-слова
...
**Приклад економії:**
- Було: Роман питає про свайп закриття модалки → читаю весь `CLAUDE.md` (416 рядків) + `docs/DESIGN_SYSTEM.md` (500+ рядків) = 900+.
