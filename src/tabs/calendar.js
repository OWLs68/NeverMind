  ...
  if (timelineEl) {
    if (timedItems.length === 0 && untimedTasks.length === 0 && allDayEvents.length === 0) {
      ...
      timedItems.forEach((item, i) => {
        ...
        // B-150: text color уніфіковано — без синього #3b82f6 для event.
        // Кольоровий індикатор зберігається через крапку (line нижче).
        const color = isDone ? 'rgba(30,16,64,0.3)' : isCurrent ? '#ea580c' : '#1e1040';
        // SVG calendar icon (замість emoji 📅/☑️) — нейтральний stroke,
        // кольорова крапка (нижче) показує тип/статус.
        const iconSvg = isEvent || isTask
          ? `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(30,16,64,0.5)" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:-2px;margin-right:4px"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>${isDone ? '<polyline points="9 14 11 16 15 12"/>' : ''}</svg>`
          : '';
      ...
      if (untimedTasks.length > 0) {
        ...
        untimedTasks.forEach(tk => {
          const prio = (tk.priority === 'critical') ? '🔴 ' : (tk.priority === 'important') ? '🟠 ' : '';
