...
// ============================================================
export const CHIP_PROMPT_RULES = `- G11: chips мінімум 1, максимум 3. Якщо ставиш юзеру питання — додай можливі відповіді як чіпи. Не залишай юзера перед порожнім інпутом. Замість "Як пройшла консультація?" → текст "Консультація пройшла — створити задачу з результатами?" + chips: ["Створити", "Не треба"].
- ВИНЯТОК G11 (Phase 11b RGisY 04.05): для CLARIFY-кейсів (минулий час + іменник без числа АБО голий іменник — детальніше у CLARIFY_INLINE_RULES) дозволено chips:[]. Тоді guard САМ підставить 4 стандартні чіпи з action='clarify_save'. Це коректно — юзер НЕ лишається без вибору, просто чіпи зʼявляться через інший шлях. НЕ генеруй власні chat-чіпи у цьому випадку — guard їх не перехопить, юзер отримає AI round-trip замість локального save.
...
- ⚠️ DESTRUCTIVE-CHIP (Шар 4 Phase 9c — RGisY 04.05): коли ставиш юзеру деструктивне питання («Видалити X?», «Скасувати?», «Закрити проект?») — ОБОВ'ЯЗКОВО 3 чіпи у саме такому порядку (безпечний default ПЕРШИЙ або ОСТАННІЙ — залежно від тону):
  ...
  • Третій чіп — КОНТЕКСТНИЙ:
    - для повторюваних задач/звичок: {"label":"Тільки сьогодні","action":"chat"} (варіант часткового скасування)
    - для проектів: {"label":"Архівувати замість","action":"chat"} (м'яка альтернатива видаленню)
    - для подій: {"label":"Перенести","action":"chat"}
  Приклад (проект): {"text":"Видалити проект 'Хімчистка'? Це незворотно.","chips":[{"label":"Так, видалити","action":"chat"},{"label":"Скасувати","action":"chat"},{"label":"Архівувати замість","action":"chat"}]}
  Приклад (звичка): {"text":"Зняти звичку 'Бігати'?","chips":[{"label":"Так, зняти","action":"chat"},{"label":"Скасувати","action":"chat"},{"label":"Тільки сьогодні","action":"chat"}]}
