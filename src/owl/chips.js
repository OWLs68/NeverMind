...
- chips — варіанти швидкої ВІДПОВІДІ користувача (не заклики до дії!). Масив об'єктів. Кожен має label (до 3 слів) і action:
  ...
  • "chat" — відправляє label у чат як ЗВИЧАЙНЕ повідомлення-уточнення (Пізніше / Розкажи більше / Ні, дякую). НЕ для звітів про виконане — для звітів є окремий action="complete" нижче.
  • "complete" — ЗВІТ про виконане (Phase 2 Шар 6 04.05): юзер підтверджує що ВЖЕ зробив. Локально знаходить задачу/звичку через fuzzy-match по перших 4 літерах слів і закриває БЕЗ AI round-trip. СТРОГО:
       а) ТІЛЬКИ минулий час ("Подав", "Купив", "Поправ") — НІКОЛИ інфінітив і НІКОЛИ наказ
       б) ОБОВ'ЯЗКОВО символ ✔️ в кінці кожного такого чіпа (для UX-впізнаваності + backward-compat)
       в) Пиши ГРАМОТНОЮ українською — НЕ суржиком
       г) Чіп = "так, я це зробив". Приклади: {"label":"Подав декларацію ✔️","action":"complete"}, {"label":"Купив продукти ✔️","action":"complete"}
...
- ⏰ КОНТЕКСТНІ ЧІПИ ЧАСУ (Шар 3 Phase 9c — RGisY 04.05): коли ставиш юзеру питання про ЧАС («Коли?», «О котрій?», «Через скільки часу?», «Коли почалось?») — ОБОВ'ЯЗКОВО додавай 4 чіпи з типовими відповідями замість порожнього інпута:
  • {"label":"Зараз","action":"chat"}
  • {"label":"Через годину","action":"chat"}
...
- 🚫 ANTI-LEAK ЧАСОВИХ ЧІПІВ (LfA6w 07.05 v2): часові чіпи [Зараз/Через годину/Завтра 08:00/Інше] — для **set_reminder** і для health-логування доконаних дій (приклад: «Коли ти останній раз приймав ліки?» → log_medication_dose). НЕ використовуй ці часові чіпи для finance-clarify, task-clarify, note-clarify, project-clarify — там потрібні категорійні чіпи (Витрата/Дохід/Задача/Проект тощо). Запит «80 бензин» НЕ є питанням про час — не давай часові чіпи.
- 💰 FAST-PATH FINANCE (LfA6w 07.05 v2): «ЧИСЛО + ДОКОНАНА ПОВСЯКДЕННА ПОСЛУГА» (вузький список, 95%+ витрата): «80 бензин», «50 кава», «25 таксі», «800 оренда», «300 інтернет», «500 телефон», «120 паркінг», «900 мийка», «60 проїзд» → ВИКЛИКАЙ save_finance НАПРЯМУ як expense. БЕЗ чіпів. Категорія: бензин/таксі/паркінг/проїзд/мийка→Транспорт, кава→Їжа, оренда/комуналка/інтернет→Житло, телефон/підписка→Підписки.
- 💰 АМБІВАЛЕНТНІ ТОВАРИ (LfA6w 07.05 v2): «ЧИСЛО + ТОВАР ЯКИЙ МОЖНА КУПИТИ ЗАРАЗ АБО ПЛАНУВАТИ»: «150 ліки», «60 обід», «90 хліб», «200 продукти», «300 одяг», «80 м'ясо» — це може бути доконана витрата АБО задача «купити». Питання + 3 чіпи:
  • {"label":"Витрата 150 ₴","action":"clarify_save","target":"save_finance","payload":{"_reasoning_log":"чіп юзера","fin_type":"expense","amount":150,"category":"Здоров'я","fin_comment":"ліки"}}
  • {"label":"Задача \"Купити ліки\"","action":"clarify_save","target":"save_task","payload":{"_reasoning_log":"чіп юзера","title":"Купити ліки","text":"Купити ліки на 150 ₴","comment":""}}
  • {"label":"Не зберігати","action":"clarify_save","target":"none","payload":{}}
  • 🎯 SUBCATEGORY (LfA6w 07.05): у контексті є блок «Категорії витрат з підкатегоріями: Транспорт: [Паливо, Парковка, Мийка, Сервіс] · Їжа: [Сніданок, Кафе, Продукти] · ...» — це РЕАЛЬНІ юзерські підкатегорії. ОБОВ'ЯЗКОВО передавай параметр subcategory у save_finance якщо в обраній категорії є підкатегорія що матчить іменник: «80 бензин» → category=Транспорт, **subcategory=Паливо**. «300 заміна масла» → category=Транспорт, **subcategory=Сервіс**. «50 кава» → category=Їжа, **subcategory=Кафе**. «120 хліб» → category=Їжа, **subcategory=Продукти**. ВАЖЛИВО: значення subcategory має бути ТОЧНО зі списку категорії (case-sensitive, з контексту). НЕ вигадуй власних — якщо немає підкатегорії що ідеально матчить, не передавай subcategory взагалі (буде записано тільки на категорію). Працює універсально для ВСІХ категорій включно з юзерськими новоствореними.
- 💰 ЧІПИ FINANCE-CLARIFY (LfA6w 07.05) — ТIЛЬКИ для амбівалентних кейсів «ЧИСЛО + БІЗНЕС-ІМЕННИК» («300 хімчистка», «500 салон», «1500 курс англійської», «800 ательє», «1200 студія», «900 школа танців», «2000 агенція»): може бути витрата АБО проект АБО задача. Питання + 4 чіпи categorical:
  • {"label":"Витрата 300 ₴","action":"clarify_save","target":"save_finance","payload":{"_reasoning_log":"чіп юзера","fin_type":"expense","amount":300,"category":"Покупки","fin_comment":"хімчистка"}}
  • {"label":"Створити проект","action":"clarify_save","target":"create_project","payload":{"_reasoning_log":"чіп юзера","name":"Хімчистка","subtitle":""}}
  • {"label":"Задача \"Сходити в хімчистку\"","action":"clarify_save","target":"save_task","payload":{"_reasoning_log":"чіп юзера","title":"Сходити в хімчистку","text":"Сходити в хімчистку","comment":""}}
  • {"label":"Не зберігати","action":"clarify_save","target":"none","payload":{}}
...
export function handleChipClick(tab, text, action, target, payloadRaw) {
  ...
  // 2a. Явний action='complete' (Phase 2 Шар 6 04.05) — explicit completion
  // chip який локально закриває задачу/звичку. Раніше було тільки через ✔️ у text
  ...
  // 2b. Legacy ✔️-detection (backward-compat для chips створених до Phase 2).
  // Зберігаємо щоб історія чату з старими чіпами `action:'chat' + ✔️ у label`
  // продовжувала працювати після rollback або у нових юзерів які пропустили v10.
  if (text.includes('✔️')) {
    const handled = handleCompletionChip(text, tab);
    if (handled) return;
