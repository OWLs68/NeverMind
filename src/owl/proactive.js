...
${sc ? `РОЗКЛАД ЮЗЕРА: прокидається ${sc.wakeUpStr || '?'}, починає день ${sc.workStartStr || '?'}, завершує роботу ${sc.workEndStr || '?'}, лягає ${sc.bedTimeStr || '?'}. Зараз ${timeStr} → ФАЗА: ${(phase || 'unknown').toUpperCase()}.

🚫 phase=work (юзер НА РОБОТІ — фізично не може виконати фізичні дії на робочому місці) АБО phase=night якщо workEnd>=21 (пізня зміна):
  СЕМАНТИЧНЕ ПРАВИЛО: ЗАБОРОНЕНО пропонувати БУДЬ-ЯКУ дію що вимагає від юзера встати, рухатись, змінити позу, вийти з робочого місця, виконати фізичну активність. Це включає (але не обмежується): біг, зал, спорт, присідання, віджимання, розтяжка, прогулянка, ходьба, йога, зарядка, кардіо, бокс, плавання, "встань і зроби N", "займе всього N хв", "пробіжи 5 хв", "потягнись", "вийди подихати", "зроби 5-хвилинну активність", "мікро-вправа", "зарядись енергією через рух".
  Habit "не виконано сьогодні" про фізичну активність — НЕ нагадуй до workEnd. Скажи нейтрально "Звичка X залишилась на ввечері" АБО просто промовч про неї.
  Дозволено: розумові/тексто-мікро-задачі (записати, подзвонити, відповісти, прочитати), стан задач/бюджету, події з таймером.
✅ phase=evening (після workEnd, до bedTime-1) — можна нагадати про звички/спорт.
✅ phase=morning (між wakeUp і workStart) — м'яке привітання, без тиску.
🌅 phase=dawn (до wakeUp, юзер прокинувся раніше) — мовчи або дуже мʼяке привітання, без вимог.
🌙 phase=night (після bedTime-1) — тільки критичне, дуже коротко. Без нагадувань про вправи / нерухомість.
🤐 phase=silent — взагалі не пиши.
❓ phase=unknown (помилка визначення розкладу — getDayPhase повернула пусто) — пиши ОБЕРЕЖНО, без фізичних дій. Це fallback, краще промовчати ніж нагадати про спорт у некоректний момент.
