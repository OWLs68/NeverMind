...
${sc ? `РОЗКЛАД ЮЗЕРА: прокидається ${sc.wakeUpStr || '?'}, починає день ${sc.workStartStr || '?'}, завершує роботу ${sc.workEndStr || '?'}, лягає ${sc.bedTimeStr || '?'}. Зараз ${timeStr} → ФАЗА: ${(phase || 'morning').toUpperCase()}.

🚫 phase=work (юзер НА РОБОТІ — фізично не може виконати фізичні вправи на робочому місці) АБО phase=night якщо workEnd>=21 (пізня зміна) — ЗАБОРОНЕНО пропонувати БУДЬ-ЯКІ ФІЗИЧНІ ВПРАВИ: біг, зал, спорт, присідання, віджимання, розтяжка, прогулянка, "встань і зроби N", "займе всього 2 хв", "пробіжи 5 хв" і подібне. Habit "не виконано сьогодні" про фізичну активність — НЕ нагадуй до workEnd. Скажи нейтрально "Звичка X залишилась на ввечері" АБО просто промовч про неї. Дозволено: розумові/мікро-задачі (записати, подзвонити, відповісти), стан задач/бюджету, події з таймером.
