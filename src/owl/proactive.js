...
function _getInboxBoardContext() {
  ...
  forgotten.forEach(t => {
    ...
    important.push(`[ЗАБУТА ЗАДАЧА] "${t.title}" [task_${t.id}] висить ${days} днів. М'яко запитай у text content чи ще актуально. ВАЖЛИВО: НЕ викликай complete_task / delete_task у цьому повідомленні — юзер ще не дав відповідь. Згенеруй ТРИ КОНТЕКСТНІ чіпи під саме цю задачу (не шаблонні «Виконано/Видалити»). СТРУКТУРА:
  Чіп 1: action='complete', payload={"task_id":${t.id}}, label = ПIДТВЕРДЖЕННЯ ВИКОНАННЯ У МИНУЛОМУ ЧАСI + ✔️ (юзер каже про себе). Приклади: «Зареєструватися на Upwork» → «Вже зареєструвався ✔️»; «Подати декларацію» → «Подав ✔️»; «Купити продукти» → «Купив ✔️»; «Поприбрати ввечері» → «Вже поприбрав ✔️»; «Помити вікна» → «Помив ✔️». ⚠️ ЗАБОРОНЕНО копіювати назву задачі в інфінітиві («Поприбрати ввечері» — ❌). Якщо не вдається підібрати минулий час — fallback label = «Вже зробив ✔️» (без назви). ОБОВ'ЯЗКОВО ✔️ + минулий час від першої особи.
  Чіп 2: action='chat', label = проміжний прогрес або причина затримки, ≤3 слова, БЕЗ ✔️ (для Upwork → «Збираю документи», для декларації → «Готую папери», для продуктів → «Завтра зайду»).
  Чіп 3: action='chat', label = перенесення без зобов'язання, ≤3 слова, БЕЗ ✔️ («Не зараз», «Наступного тижня», «Перенесу»).
КРИТИЧНО: чіп 1 ОБОВ'ЯЗКОВО має ✔️ І payload з task_id — інакше задача не закриється. Чіпи 2-3 БЕЗ ✔️.`);
  });
  ...
  if (activeTasks.length > 0) {
    ...
    // PJi7l 08.05: задач взагалі немає (не закритих, не активних — порожньо).
    // Без явного сигналу AI пише «жодна задача не закрита» — це звучить як критика.
    normal.push('Задач у юзера немає взагалі — ні активних, ні закритих. НЕ кажи «жодна задача не закрита» або «задачі не виконані» — нема чого виконувати. Запропонуй створити нову задачу або іншу дію.');
  } else {
    normal.push('Всі задачі виконано. Юзер має закриті задачі сьогодні — підкріпи позитивно, не критикуй.');
  }
  ...
  } else if (buildHabits.length === 0 && quitHabits.length === 0) {
    // PJi7l 08.05: звичок взагалі немає. Без сигналу AI пише «жодна звичка не виконана».
    normal.push('Звичок у юзера немає взагалі — ні build, ні quit. НЕ кажи «жодна звичка не виконана» — звичок просто не існує. Запропонуй створити першу або зайнятись чимось іншим.');
  }
