...
- "видали категорію Дозвілля" → delete_finance_category з name='Дозвілля' (транзакції збережуться).
...
export const INBOX_TOOLS = [
  ...
  { type: "function", function: { name: "delete_finance_category", description: "Видалити категорію Фінансів. Юзер каже 'видали категорію X'. Транзакції зберігаються (їх категорія лишиться рядком без візуального кружечка). Якщо юзер хоче об'єднати з іншою — використай merge_finance_categories замість delete.", parameters: { type: "object", properties: { name: { type: "string", description: "Назва категорії для видалення" }, comment: { type: "string" } }, required: ["name", "comment"], additionalProperties: false } } },
  { type: "function", function: { name: "merge_finance_categories", description: "Об'єднати дві категорії Фінансів в одну. Юзер каже 'об'єднай X і Y', 'злий X у Y'. Всі транзакції з 'from' перейдуть у 'to', 'from' буде видалена. Підкатегорії переносяться у 'to'.", parameters: { type: "object", properties: { from_name: { type: "string", description: "Назва категорії яка буде злита у іншу (зникне)" }, to_name: { type: "string", description: "Назва категорії-одержувача (залишиться)" }, comment: { type: "string" } }, required: ["from_name", "to_name", "comment"], additionalProperties: false } } },
