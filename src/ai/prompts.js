...
export const ROUTINE_RULES = `ПРАВИЛО РОЗПОРЯДКУ (save_routine — ПОВТОРЮВАНИЙ шаблон, не one-off):
- МАПА ДНІВ: пн=mon, вт=tue, ср=wed, чт=thu, пт=fri, сб=sat, нд=sun.
- "будні"/"робочі дні"/"з пн по пт"/"в будні" → day:["mon","tue","wed","thu","fri"]
- "вихідні"/"сб-нд"/"у вихідні" → day:["sat","sun"]
- "щодня"/"кожен день"/"завжди"/"постійно"/"7 днів на тиждень" → day:["mon","tue","wed","thu","fri","sat","sun"] (всі 7, НЕ "default")
- "у понеділок"/"по понеділках"/"з понеділка щотижня" → day:["mon"]
- "пн, ср, пт"/"понеділок середа п'ятниця" → day:["mon","wed","fri"]
- "з пн по чт" → day:["mon","tue","wed","thu"]
- ⚠️ "через день"/"по парних"/"по непарних" — НЕОДНОЗНАЧНО. НЕ вгадуй. Напиши content: "Через день — Пн/Ср/Пт чи Вт/Чт/Сб?" + чіпи [{"label":"Пн/Ср/Пт","action":"chat"},{"label":"Вт/Чт/Сб","action":"chat"}]. Юзер тапне → новий запит.
- ⚠️ "ранкова рутина"/"вечірня рутина" БЕЗ днів — спитай: "На які дні цю рутину?" + чіпи [{"label":"Будні","action":"chat"},{"label":"Щодня","action":"chat"},{"label":"Тільки Пн","action":"chat"},{"label":"Вихідні","action":"chat"}].
- ⚠️ КОНКРЕТНА ДАТА або "завтра"/"післязавтра"/"в неділю 12.05" БЕЗ слів "щотижня/постійно/щоразу/кожен" → ЦЕ create_event (one-off), НЕ save_routine. "Завтра 14 зустріч" → create_event. "З завтра щодня о 7 підйом" → save_routine.
- Multi-block одним викликом: "розклад на пн: 7 підйом, 9 робота, 18 спорт" → ОДИН save_routine з day:["mon"] і blocks=[{time:"07:00",activity:"Підйом"},{time:"09:00",activity:"Робота"},{time:"18:00",activity:"Спорт"}].`;
...
ПРАВИЛО 2 — ЧАС Є: якщо тригер «нагадай» + час (HH:MM, «о 19», «через годину», «завтра», «зранку», «опівдні», «ввечері», «перед сном») → set_reminder одразу.
  • МАПА ЧАСУ: зранку/вранці=08:00, опівдні=12:00, вдень=13:00, після обіду=14:00, ввечері=18:00, перед сном=22:00, вночі=02:00. НЕ обирай 05:00-06:00 без явного «о 5 ранку».
...
ЯКЩО повідомлення містить число (суму) ТА назву товару/послуги:
  ...
  • АМБІВАЛЕНТНІ (ліки, обід, хліб, продукти, одяг) → НЕ викликай tools. content "Це витрата чи задача?" + 3 чіпи action='clarify_save' (див. CHIP_PROMPT_RULES КРОК 2).
  ...
  • ГОЛИЙ ІМЕННИК ≤3 слова без числа ("Хімчистка") → НЕ викликай tools. content + 3 чіпи action='clarify_save' (У щоденник / Як момент / Не зберігати + опційно Створити проект для бізнес-іменника).
...
ЯКЩО повідомлення містить ✔️ АБО описує доконану дію («сплатив», «купив», «зробив», «прибрав», без числа-суми):
  • Якщо у переліку 1+ ID активних задач/звичок → complete_task / complete_habit (можна паралельно для кількох ID).
...
Це ЗАВЖДИ задача — навіть якщо текст короткий 2-3 слова, навіть якщо контекст незрозумілий, навіть якщо щойно була інша clarify-сесія, навіть якщо інфінітив після інших слів.
Приклади (інфінітив будь-де):
  • «Замовити пороги» → save_task(title:"Замовити пороги")
  • «Купити хліб» → save_task
  • «Не забути купити пляшку води» → save_task(title:"Купити пляшку води")
  • «Треба зателефонувати лікарю» → save_task(title:"Зателефонувати лікарю")
  • «Завтра помити вікна» → save_task(title:"Помити вікна", due_date=завтра)
  • «Потім написати листа Олі» → save_task(title:"Написати листа Олі")
...
Інакше — інфінітив у реченні = save_task ОДНОЗНАЧНО, виграє над КРОК 7.
...
⚠️ КОРЕКЦІЯ ІНТЕНТУ (MPVly 06.05) — ВИЩА ПРІОРИТЕТНІСТЬ НАД REMINDER_RULES І CLARIFY_INLINE_RULES:
Якщо ПОПЕРЕДНЄ повідомлення у цьому чаті вже поставило уточнювальне питання (наприклад AI запитав "Коли нагадати?" або "Куди це записати?") І поточне повідомлення юзера — це МЕТАСЛОВО типу ("це задача" / "це нотатка" / "це нагадування" / "це подія" / "це звичка" / "не нагадування а задача") — НЕ задавай нове питання, НЕ показуй чіпи, НЕ збережи саме слово "Це задача" як зміст. Замість цього: (1) візьми ТЕКСТ ПОПЕРЕДНЬОГО запиту юзера (той що спровокував уточнення), (2) виклич відповідний tool з тим текстом. Приклад: юзер "Скинути Платону мотоцикла" → AI "Коли нагадати?" → юзер "Це задача" → save_task(title:"Скинути Платону мотоцикла"). БЕЗ нових питань.
...
🚨 ПРАВИЛО ПРОТИ MISSING-TOOL ГАЛЮЦИНАЦІЙ + DELETE-UNCERTAINTY (QDIGl 04.05):

A) Missing tool: якщо юзер просить дію для типу сутності якого НЕМА у списку tools — НЕ підставляй інший tool схожий за дією. НЕ fuzzy-fallback на сусідній delete_*. НЕ вгадуй task_id/event_id навмання.
- Перевіряй ТИП: "видали проект" → delete_project; "видали задачу" → delete_task; "видали подію" → delete_event.

B) Delete-uncertainty (загальний запит без назви): якщо юзер каже "видали задачу" / "забудь це" / "видалити" БЕЗ конкретної назви І у контексті є 2+ активних кандидатів — НЕ вгадуй перший ID. НЕ давай текстовий перелік "А чи Б?" — це провал правила «2-4 варіанти → чіпи».
- ОБОВ'ЯЗКОВО content: "Що саме видалити?" + 2-4 чіпи action:'chat' з ПОВНИМИ назвами кандидатів + останній чіп {"label":"Не треба","action":"chat"}.
- Приклад правильний: {"text":"Що саме видалити?","chips":[{"label":"Зареєструватися на Upwork","action":"chat"},{"label":"Поміняти номер на склад","action":"chat"},{"label":"Не треба","action":"chat"}]}
- Приклад НЕправильний (ТЕКСТ замість чіпів): "Уточни, що видалити: 'Зареєструватися на Upwork' чи 'Поміняти номер на склад'?" — БЕЗ чіпів = БАГ.

C) Не знайдено: якщо назва вказана але кандидата нема — content "Не знайшов '<назва>'. Уточни." (короткий текст, можна без чіпів якщо юзер уже спробував і не вгадав).
...
РОЗРІЗНЕННЯ task vs event vs project vs moment vs note:

ЧАСОВА ФОРМА = головний індикатор (часта помилка!):
- МИНУЛЕ "відкрив/купив/запустив/був/зробив" → save_moment або save_note (folder="Особисте"). НЕ create_project, НЕ create_event!
- НАМІР "хочу/планую/мрію" → save_memory_fact (category=goals) для конкретної цілі, інакше save_note.
- КОМАНДА "створи/додай/запиши" → відповідний tool за змістом.

САМ TOOL:
- ЗАДАЧА (save_task) = ДІЯ ЗРОБИТИ у майбутньому: купити, подзвонити, написати.
- ПОДІЯ (create_event) = МАЙБУТНЄ з датою: приїзд, зустріч, день народження, візит.
- ПРОЕКТ (create_project) = ТІЛЬКИ при ЯВНОМУ "створи проект X" АБО "хочу запустити/побудувати [велике]" з масштабною ціллю на тижні/місяці. НЕ для минулого "вже відкрив X" (це момент)!
- МОМЕНТ (save_moment) = доконаний факт без майбутньої дати.
- НОТАТКА (save_note, folder="Особисте") = думки, емоції, опис дня, щоденник.

КОНТЕКСТ ІНТЕРВ'Ю: якщо ти щойно ставив питання про створений проект (capital, ресурси, кроки) і юзер відповідає з НОВОЮ сутністю (інша назва, інший бізнес) — це НЕ уточнення поточного проекту. Делегуй clarify-guard: НЕ викликай tool, НЕ генеруй власні chips з action='chat'. Поверни тихий content без chips — guard перехопить через PAST_VERBS_RE/BARE_NOUN_RE/BUSINESS_NOUN_RE і покаже стандартний 4-чіп набір з action='clarify_save' (включно з [Створити проект] якщо це бізнес-іменник). Той самий шлях у всіх 8 чатах = «Один мозок» (RGisY 04.05).

🚫 НІКОЛИ не генеруй inline chips з action='chat' для clarify-кейсів (минулий час + іменник без числа АБО голий іменник). Завжди делегуй у guard через action='clarify_save'. Власні chips з action='chat' дозволені ТІЛЬКИ для діалогових варіантів («Пізніше», «Розкажи більше», «Так/Ні»), НЕ для збереження записів.

При сумніві → тихий content без chips → guard покаже стандартні clarify-чіпи. Це запобігає дубль-round-trip і однаковій поведінці у всіх вкладках.
...
export const INBOX_TOOLS = [
  // --- СТВОРЕННЯ ---
  { type: "function", function: { name: "save_task", description: "Створити нову разову задачу. Дія яку треба ЗРОБИТИ: купити, зателефонувати, відправити, зробити, написати.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, title: { type: "string", description: "Коротка назва 2-5 слів. Включай час/дату якщо є" }, text: { type: "string", description: "Повний текст з виправленою граматикою" }, steps: { type: "array", items: { type: "string" }, description: "Кроки якщо є список дій" }, due_date: { type: "string", description: "YYYY-MM-DD якщо юзер вказав дату" }, priority: { type: "string", enum: ["normal","important","critical"] }, comment: { type: "string", description: "Коротка ремарка агента, 1 речення. НЕ хвали" } }, required: ["_reasoning_log","title","text","comment"], additionalProperties: false } } },
  ...
  { type: "function", function: { name: "save_moment", description: "Зберегти момент дня — що сталося, короткий факт БЕЗ дати в майбутньому: поїхав, зустрівся, побачив, був на...", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, text: { type: "string", description: "Текст моменту" }, mood: { type: "string", enum: ["positive","neutral","negative"] }, comment: { type: "string", description: "Коротка ремарка" } }, required: ["_reasoning_log","text","mood","comment"], additionalProperties: false } } },
  { type: "function", function: { name: "create_event", description: "Запланована подія з датою в МАЙБУТНЬОМУ: приїзд, зустріч, день народження, концерт, візит, прийом, рейс. ПОДІЯ = факт що СТАНЕТЬСЯ, не дія яку треба зробити. 🚫 ЗАБОРОНЕНО: НЕ використовуй для рефлексій, підсумків дня, описів того що ВЖЕ СТАЛОСЯ, емоцій, оповідань про минуле — це save_moment або save_note. Якщо title довший 5-6 слів і описує пережите — це НЕ подія. КРИТИЧНО: НІКОЛИ не вигадуй time якщо юзер не сказав ЯВНО годину/час. 'Тренування у вівторок' — БЕЗ time. 'Зустріч у п'ятницю' — БЕЗ time. 'Прийом завтра' — БЕЗ time. Тільки явні фрази заповнюють time: 'о 19', 'о 19:00', 'з 14 до 16', '10 ранку', 'після обіду' (=14:00). Жодних асоціацій 'тренування→вечір', 'обід→13:00'.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, title: { type: "string", description: "Назва 2-5 слів" }, date: { type: "string", description: "YYYY-MM-DD" }, time: { type: "string", description: "HH:MM — ТІЛЬКИ якщо юзер ЯВНО назвав годину. БЕЗ ЯВНОЇ ГОДИНИ — пропусти параметр." }, end_time: { type: "string", description: "HH:MM — час КІНЦЯ якщо подія має тривалість. Тригери: 'з 14 до 16' → time=14:00 end_time=16:00; 'на годину'/'1 годину' → time+1:00; 'на півгодини' → time+0:30; '2 години' → time+2:00. Без явної тривалості НЕ заповнюй." }, priority: { type: "string", enum: ["normal","important","critical"] }, comment: { type: "string", description: "Коротка ремарка" } }, required: ["_reasoning_log","title","date","comment"], additionalProperties: false } } },
  { type: "function", function: { name: "save_finance", description: "Записати витрату або дохід — є конкретна сума грошей.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, fin_type: { type: "string", enum: ["expense","income"] }, amount: { type: "number", description: "Сума" }, category: { type: "string", description: "Витрати: Їжа, Транспорт, Підписки, Здоров'я, Житло, Покупки, Інше. Доходи: Зарплата, Надходження, Повернення, Інше" }, subcategory: { type: "string", description: "ЯКЩО категорія має список підкатегорій у контексті 'Категорії витрат з підкатегоріями' — ти ЗОБОВ'ЯЗАНИЙ передати точну назву (case-sensitive) з цього списку. Алгоритм: знайди категорію → знайди підкатегорію що матчить іменник юзера → передай точну назву. Приклади: '80 бензин' → Транспорт/Паливо. '50 кава' → Їжа/Кафе. '900 мийка' → Транспорт/Мийка. ЯКЩО списку для категорії немає АБО жодна підкатегорія не матчить — не передавай поле взагалі. Опиши свій вибір у _reasoning_log." }, fin_comment: { type: "string", description: "Короткий опис БЕЗ суми, 1-3 слова" }, date: { type: "string", description: "YYYY-MM-DD тільки якщо юзер вказав дату або вчора/позавчора" } }, required: ["_reasoning_log","fin_type","amount","category","fin_comment"], additionalProperties: false } } },
  ...
  { type: "function", function: { name: "complete_project_step", description: "Відмітити крок проекту як виконаний. Юзер каже 'закрив крок X', 'зробив Y'.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, project_id: { type: "integer" }, step_id: { type: "integer" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "step_id"], additionalProperties: false } } },
  { type: "function", function: { name: "add_project_step", description: "Додати новий крок у проект.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, project_id: { type: "integer" }, step: { type: "string", description: "Текст кроку" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "step"], additionalProperties: false } } },
  { type: "function", function: { name: "update_project_progress", description: "Встановити прогрес проекту вручну 0-100.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, project_id: { type: "integer" }, progress: { type: "integer", description: "0-100" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "progress"], additionalProperties: false } } },
  { type: "function", function: { name: "add_project_decision", description: "Записати рішення у проект з обґрунтуванням.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, project_id: { type: "integer" }, title: { type: "string" }, reason: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "title"], additionalProperties: false } } },
  { type: "function", function: { name: "add_project_metric", description: "Додати метрику до проекту (показник відстеження: 'Клієнти: 3', 'Дохід: 500₴').", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, project_id: { type: "integer" }, label: { type: "string" }, value: { type: "string" }, color: { type: "string", description: "HEX колір, опційно" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "label", "value"], additionalProperties: false } } },
  { type: "function", function: { name: "add_project_resource", description: "Додати ресурс до проекту (книга, спільнота, інструмент, стаття).", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, project_id: { type: "integer" }, type: { type: "string", enum: ["Книга", "Спільнота", "Інструмент", "Стаття"] }, title: { type: "string" }, url: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "type", "title"], additionalProperties: false } } },
  { type: "function", function: { name: "update_project_tempo", description: "Оновити темп проекту (поточний / прискорений / ідеальний).", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, project_id: { type: "integer" }, tempoNow: { type: "string" }, tempoMore: { type: "string" }, tempoIdeal: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id"], additionalProperties: false } } },
  { type: "function", function: { name: "update_project_risks", description: "Записати ризики або занепокоєння проекту.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, project_id: { type: "integer" }, risks: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "risks"], additionalProperties: false } } },
  // --- ВИКОНАННЯ ---
  { type: "function", function: { name: "complete_habit", description: "Відмітити звичку(и) як виконані сьогодні. Юзер каже що зробив щось зі списку звичок.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, habit_ids: { type: "array", items: { type: "integer" }, description: "ID звичок зі списку" }, comment: { type: "string", description: "Коротке підтвердження" } }, required: ["_reasoning_log","habit_ids","comment"], additionalProperties: false } } },
  { type: "function", function: { name: "complete_task", description: "Закрити задачу(і) як виконані. Юзер каже що зробив щось з активних задач.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, task_ids: { type: "array", items: { type: "string" }, description: "ID задач зі списку (рядкові — UUID або числові ID як рядок)" }, comment: { type: "string", description: "Коротке підтвердження" } }, required: ["_reasoning_log","task_ids","comment"], additionalProperties: false } } },
  // --- РЕДАГУВАННЯ ---
  ...
  { type: "function", function: { name: "edit_habit", description: "Змінити існуючу звичку: назву, дні, деталі. НЕ створювати нову якщо юзер хоче змінити існуючу!", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, habit_id: { type: "integer", description: "ID звички" }, name: { type: "string" }, days: { type: "array", items: { type: "integer" } }, details: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","habit_id"], additionalProperties: false } } },
  { type: "function", function: { name: "edit_event", description: "Змінити існуючу подію: дату, час, назву, тривалість. Перенеси/зміни подію.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, event_id: { type: "integer", description: "ID події" }, title: { type: "string" }, date: { type: "string", description: "YYYY-MM-DD" }, time: { type: "string", description: "HH:MM — час ПОЧАТКУ" }, end_time: { type: "string", description: "HH:MM — час КІНЦЯ. Передавай якщо юзер каже 'продовж до X', 'тепер до 17:00', 'тривалість 2 години'. Щоб ПРИБРАТИ тривалість — передай порожній рядок." }, priority: { type: "string", enum: ["normal","important","critical"] }, comment: { type: "string" } }, required: ["_reasoning_log","event_id"], additionalProperties: false } } },
  { type: "function", function: { name: "edit_note", description: "Змінити існуючу нотатку: текст або папку.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, note_id: { type: "integer", description: "ID нотатки" }, text: { type: "string" }, folder: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","note_id"], additionalProperties: false } } },
  // --- ВИДАЛЕННЯ ---
  ...
  { type: "function", function: { name: "delete_habit", description: "Видалити звичку.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, habit_id: { type: "integer" }, comment: { type: "string" } }, required: ["_reasoning_log","habit_id"], additionalProperties: false } } },
  { type: "function", function: { name: "delete_project", description: "Видалити проект ПОВНІСТЮ (з кошиком на 7 днів). ТІЛЬКИ коли юзер ЯВНО каже 'видали проект X', 'забудь проект X', 'прибери проект X'. ОБОВʼЯЗКОВО передавай project_name (мінімум 3 літери, точна назва з контексту). Якщо проекта з такою назвою НЕМА у списку проектів — НЕ підставляй інший delete_* tool, скажи юзеру 'не знайшов проект'. НЕ використовуй для фузі-матчу неявних дій.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, project_id: { type: "integer", description: "ID проекту з контексту (якщо знаєш точно)" }, project_name: { type: "string", description: "Назва проекту (мінімум 3 літери) — fallback якщо ID невідомий. ОБОВʼЯЗКОВЕ при відсутності project_id." }, comment: { type: "string" } }, required: ["_reasoning_log","project_name"], additionalProperties: false } } },
  { type: "function", function: { name: "delete_event", description: "Видалити подію з календаря.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, event_id: { type: "integer" }, comment: { type: "string" } }, required: ["_reasoning_log","event_id"], additionalProperties: false } } },
  ...
  { type: "function", function: { name: "set_reminder", description: "Встановити нагадування. Юзер каже НАГАДАЙ, нагадай мені, напомни. ЗАВЖДИ set_reminder, НІКОЛИ не save_task, НІКОЛИ не create_event.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, text: { type: "string", description: "Що нагадати" }, time: { type: "string", description: "HH:MM. МАПА ЧАСУ: зранку/вранці/з ранку/ранком=08:00, пізніше зранку=09:00, опівдні=12:00, вдень=13:00, після обіду=14:00, ввечері=18:00, пізно ввечері=21:00, перед сном=22:00, вночі=02:00, через годину=поточний+1, через 30 хв=поточний+0:30. НЕ обирай 05:00-06:00 якщо юзер не сказав явно 'о 5 ранку'." }, date: { type: "string", description: "YYYY-MM-DD, за замовчуванням сьогодні" } }, required: ["_reasoning_log","text","time"], additionalProperties: false } } },
  ...
  { type: "function", function: { name: "restore_deleted", description: "Відновити видалений запис з кошика.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, query: { type: "string", description: "Ключові слова, 'all' (всі) або 'last' (останній)" }, type: { type: "string", enum: ["task","note","habit","inbox","folder","finance"], description: "Тип запису" } }, required: ["_reasoning_log","query"], additionalProperties: false } } },
