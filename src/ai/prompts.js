...
export const INBOX_TOOLS = [
  ...
  { type: "function", function: { name: "save_moment", description: "Зберегти момент дня — що СТАЛОСЯ (минулий час), короткий факт БЕЗ дати в майбутньому: поїхав, зустрівся, побачив, був на, жарили, пили. 🚫 ЗАБОРОНЕНО викликати create_event паралельно з save_moment — обери ОДНЕ. Минулий час = save_moment. Майбутнє = create_event.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, text: { type: "string", description: "Текст моменту" }, mood: { type: "string", enum: ["positive","neutral","negative"] }, date: { type: "string", description: "YYYY-MM-DD дата ЯКОЛИ це сталося. Якщо юзер каже «вчора» — передавай вчорашню дату. «позавчора» — два дні тому. Без явного посилання — пропусти (буде сьогодні). НЕ майбутні дати — це create_event, не save_moment." }, comment: { type: "string", description: "Коротка ремарка" } }, required: ["_reasoning_log","text","mood","comment"], additionalProperties: false } } },
