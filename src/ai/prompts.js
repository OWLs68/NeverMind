...
ЯКЩО повідомлення містить ✔️ АБО описує доконану дію («сплатив», «купив», «зробив», «прибрав», без числа-суми):
  • КРОК vs ЗАДАЧА: глянь у контексті «активні: a, b, c» (підказка про активні кроки задачі). Якщо «купив X» де X = один з активних кроків — це complete_step(task_id, step_text=X), НЕ complete_task. Закриваєш цілу задачу тільки коли юзер сказав про ОБ'ЄКТ задачі (повна назва), не один з кроків.
  • Якщо у переліку 1+ ID активних задач/звичок з повним збігом → complete_task / complete_habit (можна паралельно для кількох ID).
