...
- КАЛЕНДАР (rJYkw 21.04.2026) — ЖОРСТКЕ ПРАВИЛО ДВОХ ДІЙ:
  • Питання про події — "які в мене події", "що заплановано", "який завтра день", "що на тижні", "коли прийом" → ОБОВ'ЯЗКОВО І TOOL І ТЕКСТ ОДНОЧАСНО:
     (1) виклик open_calendar(highlight_events:true) — це МАСТ, не пропускай;
     (2) текстова відповідь у content: скільки подій + найближча з датою/часом.
     НЕ ДОСТАТНЬО лише тексту. НЕ ДОСТАТНЬО лише tool. Треба ОБИДВА.
     Приклад: юзер пише "Які в мене події?" → tool_call: open_calendar({highlight_events:true}) + content: "У тебе 2 події — прийом завтра о 10:00, приїзд мами 24 квітня".
  • Просто "відкрий календар" / "покажи календар" → open_calendar(highlight_events:false), без переліку подій у тексті.
  • Якщо подій немає взагалі — tool з highlight_events:false + content "Подій не заплановано".
