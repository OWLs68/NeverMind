...
- 🚫 КРИТИЧНО (MPVly-day2 06.05): set_reminder ТІЛЬКИ якщо повідомлення містить явне слово-тригер: "нагадай" / "нагадайте" / "напомни" / "нагадування" / "remind". ЗАБОРОНЕНО викликати set_reminder якщо юзер просто говорить про свої наміри: "завтра зроблю X", "хочу закінчити Y", "планую Z", "завтра закінчу її". Це НАМІР / ПЛАН, не запит на нагадування. У такому випадку → save_task (якщо це конкретна дія) АБО save_moment (якщо це частина рефлексії дня) АБО просто текстова відповідь без tool. Лінія МАПИ ЧАСУ нижче працює ТІЛЬКИ якщо інтент = set_reminder вже визначено за словом-тригером.
...
- ⚠️ КОЛИ ВИКЛИКАТИ delete_reminder: при коригуванні часу свіжого нагадування АБО коли юзер прямо каже "видали нагадування про X". Коригування — це коли в тому ж чаті щойно був set_reminder і юзер дає НОВИЙ ЧАС БЕЗ НОВОЇ НАЗВИ. НЕ для запиту з НОВОЮ ТЕМОЮ (інша назва — окреме нагадування).
- 🚨 КОРИГУВАННЯ ЧАСУ (B-126 fix MPVly 05.05+06.05): якщо у цьому ж чаті щойно був set_reminder І поточний запит юзера містить ТІЛЬКИ новий час БЕЗ ПОВТОРНОЇ НАЗВИ — це коригування. Патерни (всі = delete_reminder старого + set_reminder нового): "не на 10 а на 9" / "не давай на 9" / "насправді в 09:30" / "ні, на 8" / "переніс на 11" / "Нагадай в 10" / "о 14:00" / "через годину" / "завтра в 8" (без явно нової назви). АЛЕ якщо новий запит містить НОВУ НАЗВУ ("Нагадай купити хліб") — це окреме нагадування, БЕЗ delete_reminder. Правило: дивись чи є у запиті саме слово/словосполучення яке відрізняє його від попереднього reminder. Немає — коригування. Є — нове. ⚠️ ВАЖЛИВО: у новому set_reminder передавай ту саму date що мав СТАРИЙ reminder (не покладайся на дефолт «сьогодні»!). Якщо старий був на завтра — новий теж на завтра. Інакше створиш дубль або зсунеш дату назад. Приклад: щойно set_reminder(text="Помити посуд", time="10:00", date="2026-05-06") → юзер каже "не на 10 а на 9" → 1) delete_reminder(text="Помити посуд", time="10:00") 2) set_reminder(text="Помити посуд", time="09:00", date="2026-05-06"  ← ТА Ж САМА дата).`;
...
export const INBOX_TOOLS = [
  ...
  { type: "function", function: { name: "save_finance", description: "Записати витрату або дохід — є конкретна сума грошей.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, fin_type: { type: "string", enum: ["expense","income"] }, amount: { type: "number", description: "Сума" }, category: { type: "string", description: "Витрати: Їжа, Транспорт, Підписки, Здоров'я, Житло, Покупки, Інше. Доходи: Зарплата, Надходження, Повернення, Інше" }, subcategory: { type: "string", description: "ОПЦІЙНО. Назва підкатегорії з контексту 'Категорії витрат з підкатегоріями'. Має бути ТОЧНО зі списку відповідної категорії (case-sensitive). Не вигадуй власних — якщо немає підкатегорії що ідеально матчить іменник юзера, лиш категорію без subcategory. Приклади: '80 бензин' → category=Транспорт, subcategory=Паливо. '50 кава' → category=Їжа, subcategory=Кафе." }, fin_comment: { type: "string", description: "Короткий опис БЕЗ суми, 1-3 слова" }, date: { type: "string", description: "YYYY-MM-DD тільки якщо юзер вказав дату або вчора/позавчора" } }, required: ["_reasoning_log","fin_type","amount","category","fin_comment"], additionalProperties: false } } },
