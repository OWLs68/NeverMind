...
export const INBOX_TOOLS = [
  ...
  { type: "function", function: { name: "save_finance", description: "Записати витрату або дохід — є конкретна сума грошей.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, fin_type: { type: "string", enum: ["expense","income"] }, amount: { type: "number", description: "Сума" }, category: { type: "string", description: "Витрати: Їжа, Транспорт, Підписки, Здоров'я, Житло, Покупки, Інше. Доходи: Зарплата, Надходження, Повернення, Інше" }, subcategory: { type: "string", description: "ОПЦІЙНЕ поле. Якщо у контексті 'Категорії витрат з підкатегоріями' список Є — спробуй підібрати найближчу за іменником юзера ('80 бензин' → Транспорт/Паливо, '50 кава' → Їжа/Кафе). Якщо у контексті списку немає — використовуй вбудовані підказки: Їжа→(кава/капучино/еспресо=Кафе; хліб/молоко/продукти=Продукти; ресторан/обід/вечеря=Ресторан), Транспорт→(бензин/пальне=Паливо; таксі/Uber/Bolt=Таксі), Підписки→(Netflix/Spotify/Apple=назва сервісу). Якщо іменник однозначно вказує підкатегорію — заповни. Якщо сумнів — НЕ передавай поле і НЕ питай юзера. 🚫 НIКОЛИ не запитуй юзера у content про підкатегорію — це опційна метадата, не блокує save_finance." }, fin_comment: { type: "string", description: "Короткий опис БЕЗ суми, 1-3 слова" }, date: { type: "string", description: "YYYY-MM-DD тільки якщо юзер вказав дату або вчора/позавчора" } }, required: ["_reasoning_log","fin_type","amount","category","fin_comment"], additionalProperties: false } } },
