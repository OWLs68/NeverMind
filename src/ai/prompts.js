...
- ⚠️ ШАР 3 ЧАС (QDIGl 04.05) — ОБОВ'ЯЗКОВО ПЕРЕВІРЯЙ що час ВКАЗАНИЙ:
  ...
  • Якщо ЯВНОГО часу НЕМА (просто "нагадай помити посуд", "нагадай зателефонувати мамі") → НЕ викликай set_reminder з вгаданим дефолтом. Замість цього напиши content "Коли нагадати?" + ОБОВ'ЯЗКОВО 4 чіпи: [{"label":"Зараз","action":"chat"},{"label":"Через годину","action":"chat"},{"label":"Завтра вранці","action":"chat"},{"label":"Інше","action":"chat"}]. Юзер тапне чіп → новий запит з обраним часом → тоді set_reminder.
  • Виняток: якщо контекст дуже короткий і явно «нагадай завтра» (без часу) — використай 09:00 як дефолт ТІЛЬКИ для слова "завтра" без часу.
...
export const INBOX_TOOLS = [
  ...
  { type: "function", function: { name: "show_monthly_summary", description: "Тимчасово показати підсумок попереднього місяця у вкладці «Я». Юзер каже «покажи підсумок квітня», «згадай місяць», «що було у квітні». Підсумок зникає автоматично через 1 годину. Викликати ТІЛЬКИ коли юзер ЯВНО просить — не за власною ініціативою.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, comment: { type: "string", description: "Коротке підтвердження для юзера." } }, required: ["_reasoning_log","comment"], additionalProperties: false } } },
