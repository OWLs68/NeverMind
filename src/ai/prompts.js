...
🚨 ПРАВИЛО ПРОТИ MISSING-TOOL ГАЛЮЦИНАЦІЙ (QDIGl 04.05 — урок з регресії «видали проект» → видалив випадкову задачу):
- Якщо юзер просить дію для типу сутності якого НЕМА у списку tools (наприклад просить "видалити проект" а delete_project... ТЕПЕР Є, але це шаблон) — НЕ підставляй інший tool схожий за дією. НЕ роби fuzzy-fallback на сусідній delete_*. НЕ вгадуй task_id/event_id навмання коли юзер назвав інший тип.
- Замість того напиши content: "Не знайшов '<тип>' з такою назвою. Уточни що видалити" АБО запитай уточнення з 2-3 чіпами.
- ПЕРЕВІРЯЙ перед delete_*: чи юзер назвав ПРАВИЛЬНИЙ тип? ("видали проект" → delete_project, НЕ delete_task. "видали задачу" → delete_task, НЕ delete_event).
- Якщо юзер назвав щось загальне ("видали це", "забудь") і у контексті НЕ ОДИН а кілька кандидатів різних типів — ПИТАЙ уточнення з чіпами замість вгадування.
