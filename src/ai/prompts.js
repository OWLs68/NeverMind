...
export const INBOX_TOOLS = [
  ...
  { type: "function", function: { name: "save_moment", description: "Зберегти момент дня — що СТАЛОСЯ (минулий час), короткий факт БЕЗ дати в майбутньому: поїхав, зустрівся, побачив, був на, жарили, пили. 🚫 ЗАБОРОНЕНО викликати create_event паралельно з save_moment — обери ОДНЕ. Минулий час = save_moment. Майбутнє = create_event.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, text: { type: "string", description: "Текст моменту" }, mood: { type: "string", enum: ["positive","neutral","negative"] }, date: { type: "string", description: "YYYY-MM-DD дата КОЛИ це сталося. Тригери з тексту юзера: «вчора»=сьогодні-1, «позавчора»=сьогодні-2, «три/N днів тому/назад»=сьогодні-N, «тиждень тому/назад»=сьогодні-7, «два тижні тому»=сьогодні-14, «місяць тому»=сьогодні-30, «у понеділок/вівторок/...»=найближчий минулий день тижня. Без явного посилання — пропусти (буде сьогодні). НЕ майбутні — це create_event." }, comment: { type: "string", description: "Коротка ремарка" } }, required: ["_reasoning_log","text","mood","comment"], additionalProperties: false } } },
  { type: "function", function: { name: "create_event", description: "Запланована подія з датою в МАЙБУТНЬОМУ: приїзд, зустріч, день народження, концерт, візит, прийом, рейс. ПОДІЯ = факт що СТАНЕТЬСЯ, не дія яку треба зробити. 🚫 ЗАБОРОНЕНО: НЕ використовуй для рефлексій, підсумків дня, описів того що ВЖЕ СТАЛОСЯ, емоцій, оповідань про минуле — це save_moment або save_note. Якщо title довший 5-6 слів і описує пережите — це НЕ подія. КРИТИЧНО: НІКОЛИ не вигадуй time якщо юзер не сказав ЯВНО годину/час. 'Тренування у вівторок' — БЕЗ time. 'Зустріч у п'ятницю' — БЕЗ time. 'Прийом завтра' — БЕЗ time. Тільки явні фрази заповнюють time: 'о 19', 'о 19:00', 'з 14 до 16', '10 ранку', 'після обіду' (=14:00). Жодних асоціацій 'тренування→вечір', 'обід→13:00'.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, title: { type: "string", description: "Назва 2-5 слів" }, date: { type: "string", description: "YYYY-MM-DD. Тригери з тексту юзера: «завтра»=сьогодні+1, «післязавтра»=сьогодні+2, «через N днів»=сьогодні+N, «через тиждень»=сьогодні+7, «через два тижні»=сьогодні+14, «через місяць»=сьогодні+30, «у понеділок/вівторок/...»=найближчий майбутній день тижня (якщо сьогодні той самий — наступний тиждень). НЕ минулі — це save_moment." }, time: { type: "string", description: "HH:MM — ТІЛЬКИ якщо юзер ЯВНО назвав годину. БЕЗ ЯВНОЇ ГОДИНИ — пропусти параметр." }, end_time: { type: "string", description: "HH:MM — час КІНЦЯ якщо подія має тривалість. Тригери: 'з 14 до 16' → time=14:00 end_time=16:00; 'на годину'/'1 годину' → time+1:00; 'на півгодини' → time+0:30; '2 години' → time+2:00. Без явної тривалості НЕ заповнюй." }, priority: { type: "string", enum: ["normal","important","critical"] }, comment: { type: "string", description: "Коротка ремарка" } }, required: ["_reasoning_log","title","date","comment"], additionalProperties: false } } },
  { type: "function", function: { name: "save_finance", description: "Записати витрату або дохід — є конкретна сума грошей.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "CoT — 1-2 речення українською. ОБОВʼЯЗКОВО (див. REASONING_LOG_RULE)." }, fin_type: { type: "string", enum: ["expense","income"] }, amount: { type: "number", description: "Сума" }, category: { type: "string", description: "Витрати: Їжа, Транспорт, Підписки, Здоров'я, Житло, Покупки, Інше. Доходи: Зарплата, Надходження, Повернення, Інше" }, subcategory: { type: "string", description: "ОПЦІЙНЕ поле. Якщо у контексті 'Категорії витрат з підкатегоріями' список Є — спробуй підібрати найближчу за іменником юзера ('80 бензин' → Транспорт/Паливо, '50 кава' → Їжа/Кафе). Якщо НЕМАЄ впевненого збігу — НЕ передавай поле і НЕ питай юзера. 🚫 НIКОЛИ не запитуй юзера у content про підкатегорію — це опційна метадата, не блокує save_finance." }, fin_comment: { type: "string", description: "Короткий опис БЕЗ суми, 1-3 слова" }, date: { type: "string", description: "YYYY-MM-DD тільки якщо юзер вказав дату або вчора/позавчора" } }, required: ["_reasoning_log","fin_type","amount","category","fin_comment"], additionalProperties: false } } },
