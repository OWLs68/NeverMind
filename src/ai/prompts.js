...
- ⚠️ ПРІОРИТЕТ НАД "ТИХОЮ ВІДПОВІДДЮ" (MPVly 05.05 fix): якщо юзер каже "нагадай" БЕЗ ЯВНОГО часу — ці правила перевизначають правило про порожній content при коротких запитах. ОБОВ'ЯЗКОВО пиши content "Коли нагадати?" + 4 чіпи навіть якщо запит з 2-3 слів ("нагадай помити посуд"). 🚫 НЕ викликай save_note/save_task/save_moment як страховку "щоб не втратилось" — ТІЛЬКИ content + чіпи, жодного tool_call. Юзер втрачає reminder-flow коли AI створює зайву нотатку паралельно.
- Юзер каже "нагадай", "нагадай зранку", "напомни мені", "нагадай через годину" → це інтент set_reminder. НЕ create_event, НЕ save_task.
- ⚠️ ШАР 3 ЧАС (QDIGl 04.05) — ОБОВ'ЯЗКОВО ПЕРЕВІРЯЙ що час ВКАЗАНИЙ:
  • Якщо у запиті є слово з МАПИ ЧАСУ (зранку/вранці/опівдні/вдень/ввечері/перед сном/вночі/...) АБО цифра часу (HH:MM, "о 19", "через годину", "через 30 хв", "завтра", "в неділю") → використай МАПУ і виклич set_reminder одразу.
  • Якщо ЯВНОГО часу НЕМА (просто "нагадай помити посуд", "нагадай зателефонувати мамі") → НЕ викликай set_reminder з вгаданим дефолтом. Замість цього напиши content "Коли нагадати?" + ОБОВ'ЯЗКОВО 4 чіпи у форматі: {"chips":[{"label":"Зараз","action":"chat"},{"label":"Через годину","action":"chat"},{"label":"Завтра 08:00","action":"chat"},{"label":"Інше","action":"chat"}]} (з ОБГОРТКОЮ {"chips":[...]} — обов'язково, інакше парсер не розпізнає). Юзер тапне чіп → новий запит з обраним часом → тоді set_reminder.
  • 🚨 КРИТИЧНО (B-125 fix MPVly 05.05): якщо у відповіді юзера ЄСТЬ слово "завтра" (наприклад "Завтра 08:00", "завтра вранці", "завтра в 9", або юзер тапнув чіп з "Завтра") → у set_reminder ОБОВ'ЯЗКОВО передай date = ЗАВТРАШНЯ дата YYYY-MM-DD (поточна_дата + 1 день), НЕ сьогоднішня. Без явного date dispatcher ставить дефолт = сьогодні → reminder створюється на минулу годину.
  • Виняток: якщо контекст дуже короткий і явно «нагадай завтра» (без часу) — використай 08:00 як дефолт + date=завтра.
...
- ⚠️ КОЛИ ВИКЛИКАТИ delete_reminder (MPVly 05.05 follow-up): ТІЛЬКИ при ЯВНОМУ коригуванні часу свіжого нагадування ("не на 10 а на 9", "насправді в 09:30") АБО коли юзер прямо каже "видали нагадування про X". НЕ для нового запиту "нагадай X" — навіть якщо схожа назва вже є у списку. Юзер може хотіти ДОДАТКОВЕ нагадування з тим же текстом. Якщо невпевнений — створюй нове через set_reminder, не плутай з delete.
- 🚨 КОРИГУВАННЯ ЧАСУ (B-126 fix MPVly 05.05): якщо юзер коригує час свіжого нагадування ("не на 10 а на 9", "не давай на 9", "насправді в 09:30", "ні, на 8", "переніс на 11") і у цьому ж чаті щойно був set_reminder — ОБОВ'ЯЗКОВО спочатку виклич delete_reminder зі СТАРИМ часом, ПОТІМ set_reminder з новим. ⚠️ ВАЖЛИВО: у новому set_reminder передавай ту саму date що мав СТАРИЙ reminder (не покладайся на дефолт «сьогодні»!). Якщо старий був на завтра — новий теж на завтра. Інакше створиш дубль або зсунеш дату назад. Приклад: щойно set_reminder(text="Помити посуд", time="10:00", date="2026-05-06") → юзер каже "не на 10 а на 9" → 1) delete_reminder(text="Помити посуд", time="10:00") 2) set_reminder(text="Помити посуд", time="09:00", date="2026-05-06"  ← ТА Ж САМА дата).`;
...
export const INBOX_TOOLS = [
  // --- СТВОРЕННЯ ---
  { type: "function", function: { name: "save_task", description: "Створити нову разову задачу. Дія яку треба ЗРОБИТИ: купити, зателефонувати, відправити, зробити, написати.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, title: { type: "string", description: "Коротка назва 2-5 слів. Включай час/дату якщо є" }, text: { type: "string", description: "Повний текст з виправленою граматикою" }, steps: { type: "array", items: { type: "string" }, description: "Кроки якщо є список дій" }, due_date: { type: "string", description: "YYYY-MM-DD якщо юзер вказав дату" }, priority: { type: "string", enum: ["normal","important","critical"] }, comment: { type: "string", description: "Коротка ремарка агента, 1 речення. НЕ хвали" } }, required: ["_reasoning_log","title","text","comment"], additionalProperties: false } } },
  { type: "function", function: { name: "save_note", description: "Зберегти нотатку — ТІЛЬКИ думки, рефлексія, емоції, ідеї, стан здоров'я, щоденниковий запис, опис дня/ситуації. НЕ використовувати для дій які треба зробити (купити, зробити, зателефонувати) — це save_task.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, text: { type: "string", description: "Текст нотатки з виправленою граматикою" }, folder: { type: "string", enum: ["Особисте","Здоров'я","Робота","Навчання","Харчування","Фінанси","Подорожі","Ідеї"], description: "Папка. Якщо сумнів — Особисте. Ідеї — для творчих ідей. Робота — ТІЛЬКИ робочі записи. Подорожі — ТІЛЬКИ реальні поїздки" }, comment: { type: "string", description: "Коротка ремарка 1 речення" } }, required: ["_reasoning_log","text","folder","comment"], additionalProperties: false } } },
  ...
  { type: "function", function: { name: "complete_project_step", description: "Відмітити крок проекту як виконаний. Юзер каже 'закрив крок X', 'зробив Y'.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, project_id: { type: "integer" }, step_id: { type: "integer" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "step_id"], additionalProperties: false } } },
  { type: "function", function: { name: "add_project_step", description: "Додати новий крок у проект.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, project_id: { type: "integer" }, step: { type: "string", description: "Текст кроку" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "step"], additionalProperties: false } } },
  { type: "function", function: { name: "update_project_progress", description: "Встановити прогрес проекту вручну 0-100.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, project_id: { type: "integer" }, progress: { type: "integer", description: "0-100" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "progress"], additionalProperties: false } } },
  { type: "function", function: { name: "add_project_decision", description: "Записати рішення у проект з обґрунтуванням.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, project_id: { type: "integer" }, title: { type: "string" }, reason: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "title"], additionalProperties: false } } },
  { type: "function", function: { name: "add_project_metric", description: "Додати метрику до проекту (показник відстеження: 'Клієнти: 3', 'Дохід: 500₴').", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, project_id: { type: "integer" }, label: { type: "string" }, value: { type: "string" }, color: { type: "string", description: "HEX колір, опційно" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "label", "value"], additionalProperties: false } } },
  { type: "function", function: { name: "add_project_resource", description: "Додати ресурс до проекту (книга, спільнота, інструмент, стаття).", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, project_id: { type: "integer" }, type: { type: "string", enum: ["Книга", "Спільнота", "Інструмент", "Стаття"] }, title: { type: "string" }, url: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "type", "title"], additionalProperties: false } } },
  { type: "function", function: { name: "update_project_tempo", description: "Оновити темп проекту (поточний / прискорений / ідеальний).", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, project_id: { type: "integer" }, tempoNow: { type: "string" }, tempoMore: { type: "string" }, tempoIdeal: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id"], additionalProperties: false } } },
  { type: "function", function: { name: "update_project_risks", description: "Записати ризики або занепокоєння проекту.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, project_id: { type: "integer" }, risks: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","project_id", "risks"], additionalProperties: false } } },
  // --- ВИКОНАННЯ ---
  { type: "function", function: { name: "complete_habit", description: "Відмітити звичку(и) як виконані сьогодні. Юзер каже що зробив щось зі списку звичок.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, habit_ids: { type: "array", items: { type: "integer" }, description: "ID звичок зі списку" }, comment: { type: "string", description: "Коротке підтвердження" } }, required: ["_reasoning_log","habit_ids","comment"], additionalProperties: false } } },
  { type: "function", function: { name: "complete_task", description: "Закрити задачу(і) як виконані. Юзер каже що зробив щось з активних задач.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, task_ids: { type: "array", items: { type: "string" }, description: "ID задач зі списку (рядкові — UUID або числові ID як рядок)" }, comment: { type: "string", description: "Коротке підтвердження" } }, required: ["_reasoning_log","task_ids","comment"], additionalProperties: false } } },
  // --- РЕДАГУВАННЯ ---
  { type: "function", function: { name: "edit_task", description: "Змінити існуючу задачу: назву, дедлайн, пріоритет. Юзер каже перенеси/зміни/поміняй задачу.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, task_id: { type: "string", description: "ID задачі (рядок — UUID або числовий ID як рядок)" }, title: { type: "string" }, due_date: { type: "string", description: "YYYY-MM-DD" }, priority: { type: "string", enum: ["normal","important","critical"] }, comment: { type: "string" } }, required: ["_reasoning_log","task_id"], additionalProperties: false } } },
  { type: "function", function: { name: "edit_habit", description: "Змінити існуючу звичку: назву, дні, деталі. НЕ створювати нову якщо юзер хоче змінити існуючу!", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, habit_id: { type: "integer", description: "ID звички" }, name: { type: "string" }, days: { type: "array", items: { type: "integer" } }, details: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","habit_id"], additionalProperties: false } } },
  { type: "function", function: { name: "edit_event", description: "Змінити існуючу подію: дату, час, назву, тривалість. Перенеси/зміни подію.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, event_id: { type: "integer", description: "ID події" }, title: { type: "string" }, date: { type: "string", description: "YYYY-MM-DD" }, time: { type: "string", description: "HH:MM — час ПОЧАТКУ" }, end_time: { type: "string", description: "HH:MM — час КІНЦЯ. Передавай якщо юзер каже 'продовж до X', 'тепер до 17:00', 'тривалість 2 години'. Щоб ПРИБРАТИ тривалість — передай порожній рядок." }, priority: { type: "string", enum: ["normal","important","critical"] }, comment: { type: "string" } }, required: ["_reasoning_log","event_id"], additionalProperties: false } } },
  { type: "function", function: { name: "edit_note", description: "Змінити існуючу нотатку: текст або папку.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, note_id: { type: "integer", description: "ID нотатки" }, text: { type: "string" }, folder: { type: "string" }, comment: { type: "string" } }, required: ["_reasoning_log","note_id"], additionalProperties: false } } },
  // --- ВИДАЛЕННЯ ---
  { type: "function", function: { name: "delete_task", description: "Видалити задачу ПОВНІСТЮ. ТІЛЬКИ коли юзер ЯВНО каже 'видали X', 'забудь X', 'прибери X з задач', 'не треба X'. НЕ використовуй для минулого часу ('поміняв', 'подав', 'зробив') — це complete_task. НЕ використовуй для фузі-матчу неявно пов'язаних дій. Якщо сумніваєшся — питай 'видалити чи закрити?'.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, task_id: { type: "string", description: "ID задачі (рядок)" }, comment: { type: "string" } }, required: ["_reasoning_log","task_id"], additionalProperties: false } } },
  { type: "function", function: { name: "delete_habit", description: "Видалити звичку.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, habit_id: { type: "integer" }, comment: { type: "string" } }, required: ["_reasoning_log","habit_id"], additionalProperties: false } } },
  { type: "function", function: { name: "delete_project", description: "Видалити проект ПОВНІСТЮ (з кошиком на 7 днів). ТІЛЬКИ коли юзер ЯВНО каже 'видали проект X', 'забудь проект X', 'прибери проект X'. ОБОВʼЯЗКОВО передавай project_name (мінімум 3 літери, точна назва з контексту). Якщо проекта з такою назвою НЕМА у списку проектів — НЕ підставляй інший delete_* tool, скажи юзеру 'не знайшов проект'. НЕ використовуй для фузі-матчу неявних дій.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, project_id: { type: "integer", description: "ID проекту з контексту (якщо знаєш точно)" }, project_name: { type: "string", description: "Назва проекту (мінімум 3 літери) — fallback якщо ID невідомий. ОБОВʼЯЗКОВЕ при відсутності project_id." }, comment: { type: "string" } }, required: ["_reasoning_log","project_name"], additionalProperties: false } } },
  { type: "function", function: { name: "delete_event", description: "Видалити подію з календаря.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, event_id: { type: "integer" }, comment: { type: "string" } }, required: ["_reasoning_log","event_id"], additionalProperties: false } } },
  { type: "function", function: { name: "delete_folder", description: "Видалити папку нотаток з усіма нотатками.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, folder: { type: "string", description: "Назва папки" } }, required: ["_reasoning_log","folder"], additionalProperties: false } } },
  // --- ІНШЕ ---
  { type: "function", function: { name: "reopen_task", description: "Повернути закриту задачу в активні.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, task_id: { type: "string", description: "ID задачі (рядок)" }, comment: { type: "string" } }, required: ["_reasoning_log","task_id"], additionalProperties: false } } },
  { type: "function", function: { name: "add_step", description: "Додати кроки до існуючої задачі.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, task_id: { type: "string", description: "ID задачі (рядок)" }, steps: { type: "array", items: { type: "string" } } }, required: ["_reasoning_log","task_id","steps"], additionalProperties: false } } },
  { type: "function", function: { name: "move_note", description: "Перемістити нотатку в іншу папку.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, query: { type: "string", description: "Частина тексту нотатки для пошуку" }, folder: { type: "string", description: "Нова папка" } }, required: ["_reasoning_log","query","folder"], additionalProperties: false } } },
  { type: "function", function: { name: "update_transaction", description: "Змінити існуючу фінансову операцію. Юзер ЯВНО каже змінити/виправити суму або категорію.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, id: { type: "integer" }, category: { type: "string" }, amount: { type: "number" }, comment: { type: "string" } }, required: ["_reasoning_log","id"], additionalProperties: false } } },
  { type: "function", function: { name: "delete_transaction", description: "Видалити фінансову операцію (у кошик на 7 днів). Юзер каже 'видали операцію', 'прибери цю витрату'.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, id: { type: "integer", description: "ID транзакції з контексту" }, comment: { type: "string" } }, required: ["_reasoning_log","id"], additionalProperties: false } } },
  { type: "function", function: { name: "set_finance_budget", description: "Встановити загальний бюджет на місяць або ліміт на категорію. Юзер каже 'постав бюджет 2000', 'ліміт на Їжу 400'.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, total: { type: "number", description: "Загальний бюджет на місяць" }, categories: { type: "object", description: "Мапа назва→ліміт для конкретних категорій", additionalProperties: { type: "number" } }, comment: { type: "string" } }, required: ["_reasoning_log"], additionalProperties: false } } },
  { type: "function", function: { name: "set_reminder", description: "Встановити нагадування. Юзер каже НАГАДАЙ, нагадай мені, напомни. ЗАВЖДИ set_reminder, НІКОЛИ не save_task, НІКОЛИ не create_event.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, text: { type: "string", description: "Що нагадати" }, time: { type: "string", description: "HH:MM. МАПА ЧАСУ: зранку/вранці/з ранку/ранком=08:00, пізніше зранку=09:00, опівдні=12:00, вдень=13:00, після обіду=14:00, ввечері=18:00, пізно ввечері=21:00, перед сном=22:00, вночі=02:00, через годину=поточний+1, через 30 хв=поточний+0:30. НЕ обирай 05:00-06:00 якщо юзер не сказав явно 'о 5 ранку'." }, date: { type: "string", description: "YYYY-MM-DD, за замовчуванням сьогодні" } }, required: ["_reasoning_log","text","time"], additionalProperties: false } } },
  { type: "function", function: { name: "delete_reminder", description: "Видалити нагадування. ОБОВʼЯЗКОВО при коригуванні часу свіжого reminder ('не на 10 а на 9', 'не давай на 9', 'насправді в 09:30') — спочатку delete_reminder СТАРОГО, потім set_reminder НОВОГО (інакше створиш дубль). Або коли юзер прямо каже 'видали нагадування про X', 'забери нагадування'.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй." }, text: { type: "string", description: "Назва нагадування для нечіткого пошуку у nm_reminders." }, time: { type: "string", description: "HH:MM — для уточнення коли є кілька reminder'ів з тією ж назвою. Опційно але рекомендовано при коригуванні часу — щоб точно видалити старе." }, date: { type: "string", description: "YYYY-MM-DD. Опційно." } }, required: ["_reasoning_log","text"], additionalProperties: false } } },
  { type: "function", function: { name: "restore_deleted", description: "Відновити видалений запис з кошика.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, query: { type: "string", description: "Ключові слова, 'all' (всі) або 'last' (останній)" }, type: { type: "string", enum: ["task","note","habit","inbox","folder","finance"], description: "Тип запису" } }, required: ["_reasoning_log","query"], additionalProperties: false } } },
  { type: "function", function: { name: "save_routine", description: "ПОВТОРЮВАНИЙ розпорядок дня. Дні: пн=mon,вт=tue,ср=wed,чт=thu,пт=fri,сб=sat,нд=sun. \"Будні\"/\"з пн по пт\"=[\"mon\",\"tue\",\"wed\",\"thu\",\"fri\"]. \"Вихідні\"=[\"sat\",\"sun\"]. \"Щодня\"/\"кожен день\"=всі 7 (НЕ default). \"У понеділок\"/\"по понеділках\"=[\"mon\"]. \"Через день\" — НЕ вгадуй, спитай Пн/Ср/Пт чи Вт/Чт/Сб з чіпами. Конкретна дата/\"завтра\" БЕЗ слова \"щотижня\" → НЕ save_routine, це create_event (one-off). Multi-block одним викликом.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, day: { type: "array", items: { type: "string", enum: ["mon","tue","wed","thu","fri","sat","sun","default"] }, description: "Дні з МАПИ. default=fallback (legacy, рідко використовується)." }, blocks: { type: "array", items: { type: "object", properties: { time: { type: "string" }, activity: { type: "string" } }, required: ["time","activity"] }, description: "Блоки розпорядку. Multi-block одним викликом: «7 підйом, 9 робота, 18 спорт» → 3 блоки." } }, required: ["_reasoning_log","day","blocks"], additionalProperties: false } } },
  { type: "function", function: { name: "show_monthly_summary", description: "Тимчасово показати підсумок місяця у вкладці «Я» на 1 годину. Викликати ТІЛЬКИ коли юзер ЯВНО просить («покажи підсумок квітня», «згадай березень», «що було у лютому»). НЕ за власною ініціативою. Якщо юзер називає КОНКРЕТНИЙ історичний місяць (не попередній) — згенеруй oneliner на основі даних з контексту і передай month=YYYY-MM. Якщо юзер каже узагальнено («покажи підсумок місяця») і це 1-4 число — month опускай (буде попередній з кешу).", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, month: { type: "string", description: "ISO ключ місяця YYYY-MM (наприклад '2026-03' для березня 2026). Опускати коли юзер просить попередній або узагальнено. Якщо передано — потрібні month_label і oneliner." }, month_label: { type: "string", description: "Українська назва місяця у родовому відмінку: 'березня', 'квітня', 'лютого'. Required коли передано month." }, oneliner: { type: "string", description: "1-2 речення зведення місяця українською. Required коли передано month. Базуйся на даних з контексту (звички, задачі, моменти, фінанси за цей період)." }, comment: { type: "string", description: "Коротке підтвердження для юзера у чаті." } }, required: ["_reasoning_log","comment"], additionalProperties: false } } },
  { type: "function", function: { name: "clarify", description: "Запитати уточнення. ТІЛЬКИ коли 2+ різних типів і незрозуміло, або задача vs проект. Якщо 80%+ впевненості — зберігай без питань.", parameters: { type: "object", properties: { _reasoning_log: { type: "string", description: "Внутрішні думки моделі ПЕРЕД дією: чому саме цей tool обрано, які сутності з контексту враховано, які ризики/альтернативи. 1-2 речення українською. ОБОВʼЯЗКОВО заповнюй — це параметр для покращення якості рішення (zero-shot CoT)." }, question: { type: "string", description: "Коротке питання 1 речення" }, options: { type: "array", items: { type: "object", properties: { label: { type: "string" }, action: { type: "string" }, category: { type: "string" }, text: { type: "string" }, task_title: { type: "string" }, task_steps: { type: "array", items: { type: "string" } }, habit_id: { type: "integer" } }, required: ["label"] }, description: "2-3 варіанти з вбудованими діями" } }, required: ["_reasoning_log","question","options"], additionalProperties: false } } },
