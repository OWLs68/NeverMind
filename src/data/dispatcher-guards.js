...
// Початкова версія з inbox.js 64CXo. Дієслівна гілка мала /\b(кирилиця)/ →
// МЕРТВА з народження (JS \b не знає кирилиці) → детект минулого часу для
// дієслів не працював (тільки слова вчора/назад). Оживлено v3pexs:
// кирилично-безпечна межа (?:^|[\s,.:;-]) … (?=[\s,.:;-]|$).
const PAST_INDICATORS_RE = /(вчора|позавчора|минулого|тому\s|назад)|(?:^|[\s,.:;\-])(гуля|жари|їл|пил|зустрі|сходи|створи|купи|зроби|написа|закінчи|поми|поча|відкри|приготува|пройш|по[бг]ачи|зустрі)(в|ла|ло|ли|вся|лася|лися|лось)(?=[\s,.:;\-]|$)/i;

// Увага: НЕ використовувати \b з кирилицею — у JS \b рахує межу лише по
// латиниці/цифрах (\w), тому /\bмомент/ НІКОЛИ не матчить українське «момент»
// (баг знайдено контракт-тестом gfrvu5 20.06). Просто підрядок — достатньо.
const MOMENT_KEYWORD_RE = /момент/i;
