...
| 04.05.2026 | NpBmN | **`add_health_history_entry` — stale-card detection.** Раніше тихий fallback на загальну «Здоровʼя» коли `card_id` невалідний — юзер думав що зберіг куди хотів. Тепер `tool-dispatcher.js:258-294` розрізняє два кейси: (a) `card_id` не передано → тихий fallback як раніше; (b) `card_id` передано але картки нема (юзер видалив поки тримав чіп) → префікс «⚠️ Картку видалено — записав у "<fallback>":». Без зміни schema tool. Також у `clarify-guard.js` SUSPICIOUS_TOOLS розширено до 8 елементів (додано `add_health_history_entry`, `create_health_card`, `set_reminder` як creative-hallucination ризик; `save_finance` НЕ додано — мертвий патерн через HAS_NUMBER cut). Перевірка ВСІХ tool_calls (`some()`) замість тільки `firstName`. Шар 2 Dynamic chips «Лікарі»: `DOCTOR_MENTION_RE` детектор + `_buildDoctorChips()` генерує chips з реальних `card.doctor` через `getHealthCards()` (з v2 міграцією). |
