...
| `src/ui/checklist.js` | **Спільний рендер чеклісту (v3pexs 27.06, DRY)** — `renderChecklist(items, opts)` → HTML прогрес-бар + квадратики. Винесено з tasks.js; 2 споживачі: задачі (task.steps) і списки (list.items). Параметризується через opts (tapAction + назви data-атрибутів). |
...
| `src/tabs/habits.js` | Звички + quit-звички, лог виконання, стріки, processUniversalAction (_splitReply) + create_list/delete_list (v3pexs) |
| `src/tabs/lists.js` | **Списки-чеклісти в Inbox (v3pexs 27.06)** — окрема сутність `nm_lists` (НЕ задача). `getLists/saveLists` (канонічний сеттер, payload `nm-data-changed:'lists'`) + `toggleListItem(listId,itemId)` (тап галочки → done + re-render) + `regTouch('toggle-list-item')`. Картка-чекліст рендериться у стрічці Inbox (renderInbox гілка `category:'list'`). |
...
| `src/data/clarify-decision.js` | **Чиста логіка clarify-рішення (v3pexs 28.06)** — `decideClarify(text, toolCalls)` (bareNoun/GREETING_STOPLIST/COMMAND_RE/SUSPICIOUS_TOOLS, два режими). Винесено з clarify-guard (browser-залежний) → node-тест `check-clarify-decision.js` 28/28. Guard = презентація (i18n-чіпи+UUID). |
| `src/core/execute-action.js` | **UNIVERSAL ACTION PROCESSOR (v3pexs D1)** — processUniversalAction + _fuzzyFindFolder, 835 рядків з habits.js (був 1985). Один executor для всіх барів (= Refactor Сесія 4, план §7). habits.js ре-експортує (strangler). |
| `src/core/migrations.js` | **18 boot-міграцій (v3pexs D3)** — runMigrations з boot.js (був 1450→616). Лишаються для one-time імпорту localStorage (SQL-міграції з 1 дня Supabase). |
| `src/tabs/inbox-feed.js` | **Стрічка Inbox (v3pexs D2)** — мапи категорій, getInbox/saveInbox, toggleListItem+regTouch, navigateInboxItem, renderInbox+свайп. З inbox.js (1459→1150), односторонній (не циклічний). inbox.js ре-експортує. |
| `src/tabs/notes-view.js` | **Note-view модалка + note-chat (v3pexs D4)** — openNoteView/closeNoteView/чат, з notes.js (1408→1109). getActiveNoteViewId getter (ESM module-var). getFolderColor ЛИШИВСЯ у notes.js (біля своїх констант — урок E2E #39). |
| `src/data/tool-filter.js` | **Фільтр інструментів під намір (v3pexs 27.06)** — винесено чистою функцією з `core.js` + контракт-тест `check-tool-filter.js`. `selectRelevantTools(userText, fullTools)` звужує повний набір tool-схем до релевантної підмножини за `TOOL_CATEGORIES` (економія ~30-40% токенів на схеми у кожен запит). `BASE_TOOL_NAMES` — завжди включені. **Корінь токен-бага:** межі категорій раніше через `\b` (JS `\b` не матчить кирилицю) → фільтр був мертвий, летіли всі tools. Споживач: `src/ai/core.js`. Категорія `list` додана v3pexs (інакше save_list відкидався). |
| `src/data/list-detector.js` | **Детермінований парсер списків-чеклістів (v3pexs 27.06, правило 12)** — `parseListIntent(text)` → `{tool:'save_list', args:{title, items:[string]}}` або null. Тригер «список/купити:» + перелік ≥2 пунктів. Bail на 1 пункт / нагадування (проти over-trigger). Реюз кирилично-безпечних меж `BL/BR` з intent-router. Споживачі: `core.js` fast-path + guard `dropTaskOnList`. Контракт-тест `check-list-detector.js` (19/19). |
