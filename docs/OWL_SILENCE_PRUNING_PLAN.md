...
>
> **Статус:** ✅ **ВСІ 3 ФАЗИ ЗАВЕРШЕНО** (UVKL1 26.04 → C8uQD 27.04 → xHQfi 30.04.2026). Перенесено у [`ROADMAP_DONE.md`](../ROADMAP_DONE.md). Документ зберігається як архітектурна довідка.
...
## Фаза 2 — Pruning Engine ✅ ВИКОНАНО (C8uQD 27.04.2026)

**Реалізовано у комітах** `3d5a465` (filter messages on UI render path), `d9d3edf` (one-time wipe of legacy board history), `d17b769` (CACHE_NAME bump). Файл `src/owl/board-utils.js` створено з `isEntityRelevant()` + `isMessageRelevant()`. `src/owl/board.js:4,22,118` фільтрує. `src/owl/proactive.js:781,861-867` має промпт + збереження `entityRefs`. `src/core/boot.js:323-338` робить wipe.
...
## Фаза 3 — Розділення Контексту ✅ ВИКОНАНО (C8uQD 27.04.2026)

**Реалізовано у комітах** `baf91bc` (Phase 3.1 silence flag у chat AIContext), `68a2674` (Phase 3.2 remove recentlyDone from board contexts), `3e39418` (CACHE_NAME bump). `src/ai/core.js:56` має блок `[ВАЖЛИВО — РЕЖИМ ТИШІ]`. `src/owl/proactive.js:44-47` блок `[ФАКТ] Нещодавно ЗАКРИТІ` видалено з tab-board контексту.
