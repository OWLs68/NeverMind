...
## 2026-04-18 — 📝 Новий скіл `/finish` + ротація документації (сесія FMykK)

### Контекст
Роман попросив спростити старт нової сесії — прибрати з активних файлів те що застаріло, зменшити час читання `/start`. BUGS (314 рядків) і SESSION_STATE (291) виросли через накопичення історії.
...
- Ідемпотентний: перевіряє чи є блок поточної сесії у SESSION_STATE. Якщо нема → CREATE режим з ротацією. Якщо є → UPDATE режим без ротації архіву.
- Архіви ротуються ТІЛЬКИ при першому виклику у сесії.

**2. Ручна підготовка документації (перевірка алгоритму скіла)**
...
- `_ai-tools/SESSION_STATE.md`: ~290 → ~180 рядків. Винесено KTQZA/gHCOh/cnTkD блоки + Safari zombie-SW довідку + список попередніх сесій у `_archive/SESSION_STATE_archive.md` (~125 рядків).
- `_ai-tools/SKILLS_PLAN.md`: додано `/finish` у індекс.
- `START_HERE.md`: додано `/finish` у таблицю базових скілів.

**3. Додано правило ротації у шапки BUGS + SESSION_STATE**
Правило динамічне ("останні 2 активні сесії"), не хардкодить імена. При `/finish` файли переписуються автоматично.

### Обговорено (без виконання)

- Стиснення `CLAUDE.md` (657 рядків) — відкладено на окрему сесію. Ризиковано бо це правила, не нотатки.
- Кандидати на стиснення у CLAUDE.md: блок "⚠️ ОБОВ'ЯЗКОВО" (80→40 рядків), деталі "Що не можна змінювати" (5-10→2-3 рядки на пункт).
- ROADMAP.md (624 рядки) — переглянути разом, зараз перенесено у наступний крок.
...
- `/finish` включає Фазу 6 "Умовні оновлення" (CLAUDE.md/CONCEPTS_ACTIVE/ARCHITECTURE/DESIGN_SYSTEM/SKILLS_PLAN/START_HERE) — Роман підтвердив що краще автоматизувати.
- `/obsidian` і `/finish` — різні скіли: `/obsidian` пише у Roman's brain, `/finish` оновлює репо.

### Інциденти

- **Stream idle timeout** кілька разів під час написання скіла і підготовки архівів. Обхід через skeleton+Edit (правило №1 з CLAUDE.md). Прогрес не втрачено завдяки checkpoint-комітам після кожної фази (правило №2).
- **Roman feedback — "кілометрові повідомлення".** Нагадав правило "середній розмір + людське пояснення". Самоперевірка посилена.
...
- `_archive/SESSION_STATE_archive.md` (новий, 125)
- `docs/CHANGES.md` (цей запис)
- `_ai-tools/SKILLS_PLAN.md` (додано `/finish`)
- `START_HERE.md` (додано `/finish` у таблицю скілів)

### Метрики

- **Гілка:** `claude/start-session-FMykK`
- **Код не чіпано** — тільки документація. CACHE_NAME не мінявся, деплою не було.
- **Коміти:** `da410ba` (feat skill) → `7f1c0f8` (docs bugs) → `a22b055` (docs session-state) → наступні
