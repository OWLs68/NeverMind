...
## 04.05.2026 — сесія NpBmN

Шар 2 Dynamic chips «Лікарі» (Active ROADMAP) — `DOCTOR_MENTION_RE` у `clarify-guard.js` (~25 спеціальностей+клініки) + `_buildDoctorChips()` через `getHealthCards()` → до 3 реальних `card.doctor` + «Інший лікар». Інтерв'ю проектів 3→5 питань (команда + дедлайн + метрика успіху) + активовано виклик `maybeAskGuideQuestion` (мертвий імпорт у `inbox.js:26` з минулого, через що інтерв'ю показувало тільки Q1). Council-аудит у 2 раунди (5+5 паралельних агентів) знайшов 22 знахідки — 18 закрито у 5 фазах, 1 свідомо пропущена (G — set_reminder без COMMAND-тригера це захист, не баг). Phase 1 (6 фіксів: zombie-interview cleanup при swipe-delete, ієрархія промпту ПРІОРИТЕТ-1 Лікар > ПРІОРИТЕТ-2 Створити проект, виняток Health-чату+focused, зламані лапки, SUSPICIOUS_TOOLS усі+set_reminder, card_id мертвий вантаж). Phase 2 (3 фікси: race nm_guide_waiting_topic, cooldown skip, stale card_id ⚠️). Phase 3 (6 фіксів: `tab` параметр активовано+getFocusedHealthCard, промпт `лікар:` замість `doctor`, об'єктивний маркер ФОКУС, DOCTOR ПЕРЕД HAS_NUMBER, setItem ПІСЛЯ key check, save_finance прибрано). Phase 4 (4 фікси: undo restore через snapshot, `_bumpGuideCooldown` helper, NM_KEYS.cache+7 guide-state, getHealthCards у `_buildDoctorChips`). Phase 5 самоперевірка знайшла власну неповноту (focused-card check переведено на getHealthCards для consistency).

**Коміти:** `e136951` feat Шар 2 + інтерв'ю 5 питань · `be37491` audit-phase1 (6 фіксів) · `3ae0510` audit-phase2 (3 фікси) · `288bc52` audit-phase3 (6 фіксів) · `230114f` audit-phase4 (4 фікси) · `edc62e9` audit-phase5 (#L consistency) · `a5f7101` SESSION_STATE NpBmN + archive rC4TO · `0690cfb` BUGS rotate UvEHE · `d7de50a` ROADMAP Шар 2 done
**Метрики:** 6 фіча/фікси-комітів + 3+ finish, v627→v631+, CACHE `nm-20260504-0005` → `nm-20260504-0325` (5 bumps), гілка `claude/start-session-NpBmN`. Закриті баги: 0 формальних B-XX (18 audit-знахідок документовані у lessons.md). Нові фічі: Шар 2 Dynamic chips (Лікарі) + інтерв'ю проектів 5 питань. Відкриті борги: Dynamic chips Шари 3-6, архітектурний борг (8 копій addXChatMsg, 11 fetch openai, Date.now → UUID), smoke-test на iPhone ввечері.
