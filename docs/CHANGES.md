...
## 2026-04-20 — сесія EY55M — "Один мозок" #1 + #2A

**Зроблено (17 комітів `abbeafd → 5078a21`):**

**Один мозок #1** (8 фаз) — UI tools (навігація) у всі 7 tab-чатів. Створено спільний `UI_TOOLS_RULES` у `prompts.js`, переключив me/habits/notes/finance-chat/health/projects з `fetch()` на `callAIWithTools(UI_TOOLS)`. Evening-chat отримав `handleUITool` dispatch перед `dispatchEveningTool`. CACHE_NAME bump `nm-20260419-1951` → `nm-20260419-2027`.

**Один мозок #2 варіант A** (7 фаз) — повний INBOX_TOOLS у всі 7 чатів. Створено `_toolCallToUniversalAction` (мапа 20 tools → action) і `dispatchChatToolCalls` (UI tool OR CRUD via universal action) у `habits.js`. Промпти скоротились на ~30 рядків кожен. Локальні fallback залишено для специфічних actions. CACHE_NAME bump `→ nm-20260420-0430`.

**Регресії #2A виправлено** (коміт `5078a21`):
- Фінанси: "50 на каву" обирав switch_tab → жорстке правило з прикладами save_finance.
- Задачі: галюцинація "Пити воду вже існує" → правило "перевіряй ЗВИЧКИ СЬОГОДНІ список". CACHE_NAME `→ nm-20260420-0441`.

**Обговорено/прийнято:**
- Варіант A vs B для #2: Роман обрав A швидко, я рекомендував B поетапно. Виявились регресії — підтверджено правило "поетапно з тестом безпечніше".
- Спільна мапа замість 6 копій dispatch.

**Відкриті після сесії:** B-84 Здоров'я промпт OWL "вибачається". B-85 Проекти/Здоров'я switch_tab замість CRUD (та сама регресія що у Фінансах, треба аналогічний промпт-фікс).

**Файли:** `src/ai/prompts.js`, `src/tabs/habits.js`, `src/tabs/me.js`, `src/tabs/notes.js`, `src/tabs/finance-chat.js`, `src/tabs/health.js`, `src/tabs/projects.js`, `src/tabs/evening-chat.js`, `sw.js`, `NEVERMIND_BUGS.md`.

**Метрики:** 17 комітів + 2 build, v309→v312+ після мержу, CACHE_NAME `nm-20260419-1951` → `nm-20260420-0441` (3 bump'и), 2 нових відкритих баги (B-84/B-85).

**Додатково після першого /finish (коміт `a3721d5`):** recalibrated `.claude/hooks/context-warning.sh` під реальний ліміт Opus 4.7 [1m] ≈1.6M. Нові пороги: WARN 75% (~1.2M) жовте з 🟡⚠️ трикутниками, CRITICAL 85% (~1.36M) червоне з 🔴⚠️. Помітніший візуал (рамка-рядок емодзі, bordered block, ⚡ action hint). Hook через UserPromptSubmit — попередження у кожному наступному повідомленні після переходу порога.
