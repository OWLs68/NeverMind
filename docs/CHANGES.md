...
## 2026-04-11 — 🧠 Стратегія мозку OWL: обговорення з Gemini, синхронізація ROADMAP, GLOSSARY
...
- **BYOK (Bring Your Own Key) vs Підписка** — критичне бізнес-питання ДО Supabase. Не вирішено, відкладено на окреме обговорення. Без вирішення — ризик банкрутства при масштабі.
...
**Коміт 4 (цей) — docs: синхронізація ROADMAP + GLOSSARY**
- **`FEATURES_ROADMAP.md`** — додано Judge Layer і BroadcastChannel у "Що мозок вже вміє". Перефор
мульовано 4.2 (структуровані факти замість наративу). 4.19 позначено як ✅ з відкритим завданням уніфікації. 4.20 деталізовано.
- **`CLAUDE.md`** — оновлено секцію про `localStorage.setItem` override (BroadcastChannel вже поруч, не "крихкий самотній hack").
- **Новий файл `docs/GLOSSARY.md`** — 10 термінів простою мовою з аналогіями: RAG, Embedding, pgvector, Tool Calling, SyncEngine, BroadcastChannel, RLS, Edge Functions, BYOK, Multi-tenant. Формат: назва → "просто" → "аналогія" → "чому важливо для NeverMind". Читається на старті кожної сесії.
- **`_ai-tools/SESSION_STATE.md`** — новий порядок кроків Jarvis Architecture, секція "ДЛЯ НОВОГО ЧАТУ" оновлена з правилом розміру і GLOSSARY.
