...
## 2026-04-18 — 🪝 Хук попередження про заповнення контексту (сесія Vydqm)
...
1 feature-коміт (`07d5136`) + 2 docs-коміти (`d1cf45c`, `1def70d`) + коміти від `/finish`. v238. CACHE_NAME не мінявся (конфіг хуків). Код у `src/` не чіпано.
