...
- Між 13.04 і 15.04 → `git reset --hard` (причина невідома — можливо відкат зламаного деплою, можливо слідування інструкції "Екстрений скид" з `CLAUDE.md`)
- 15.04 18:16 → перший деплой після reset = **v3** (source `claude/start-session-6v2eR`)
- 16.04 12:28 → v22 (перший деплой який зберігся у git history після reset — старіші v1-v21 garbage collected)
- 16.04-17.04 → v22 → v48 → v53
- 17.04 (сесія cnTkD) → виявлення проблеми
