...
### Зроблено (усе запушено, E2E зелений, задеплоєно)

**1. Фікс CI-блокера (`971c3b4`):** коміт dc272b6 додав `@playwright/test` у package.json, але не оновив `package-lock.json` → `npm ci` падав → **авто-злиття всіх гілок і E2E падали, нічого не деплоїлось**. Фікс: `npm install` регенерував lock. + влито dependabot PR #17 (gitleaks v3) та #18 (setup-node v6); #19 закрився сам.

**2. Новий E2E-тестер (Playwright у GitHub Actions) — замінив Hetzner.** Council 5 агентів + веб-дослід («золота середина»: автоматично+безкоштовно, без сервера). `tests/e2e/` (`helpers.js`: boot глушить OpenAI, seedState, mockAI, gotoTab через switchTab, прапор `__NM_TEST_SEED__` під Supabase; `smoke`/`tasks`/`modals` specs), `playwright.config.js` (Mobile Safari WebKit + Desktop Chrome, toHaveScreenshot anti-flaky). **12 тестів зелені.** CI зловив 3 реальні пастки дорогою (lock, дубль data-tab, вітальний тур що перекривав кліки). Реальні iOS-quirks headless НЕ ловить — лишається ручний смоук (Red Team).

**3. Hetzner AI-тестер → `_archive/hetzner-tester/`** (scripts + workflow + tester-*.json/md + setup-доки + README). Оновлено 6 операційних місць: CLAUDE.md правило 13, START_HERE, INDEX, /start Крок 1.5, /finish Фаза 3.5, ROADMAP. Гак pre-commit-screenshot.js лишено (PHI-захист). **Сервер 94.130.25.22 Роман вимикає вручну** (поза репо; поки живий — пушить старі tester-status коміти).

**4. Supabase Фаза-1 розслідування (Council×2 + web).** Ключові знахідки: (а) `makeProject`+конверт **уже зроблено** (qpzj7k), нотатка «лишилось» застаріла — ледь не переробив; (б) «критичний finance.projectId баг» від агентів — **порожній** (числових projectId у даних нема, прив'язка постала після v14); (в) **UUID v7** (`9722e2a`): `generateUUID()` тепер час-сортований v7 (RFC 9562) замість v4 — Postgres вставляє у кінець індексу, не фрагментує. Старі v4 валідні поряд (ніде не сортуємо за id).

### Відкрите / далі
- **Supabase Фаза 1 лишилось:** Ворота 2 (структурний `nm-data-changed` — потребує iPhone-смоуку) + структуровані чіпи.
- **Рішення Романа:** кредити Anthropic для Claude Security Scan (зараз тихо пропускає при вичерпаних кредитах).
- **iPhone-смоук:** календар-баг (закрити якщо ок), B-198/B-199 фінмодалки, CSP.
- Нарощувати E2E-покриття (нотатки/звички/фінанси) + знімок-еталон «вигляду».

### Документація (за прямим запитом Романа в кінці сесії)
- **Дрейф CHANGES.md виправлено:** бекфілнуто 11 пропущених сесій (e9t3N 15.05 → v1d9eo 13.06) стислими записами (Council Sonnet-агент дістав з SESSION_STATE/архіву).
- **Корінь дрейфу:** forward-нотатки («ДЛЯ НОВОГО ЧАТУ», ROADMAP Active) накопичують «✅ done» але не стирають «⏳ todo» → ледь не переробив готову фабрику проектів. Виправив застарілу секцію + 3 уроки в lessons.md.
- **Правило в `/finish` Фаза 4:** для кожного ✅ закритого — ВИДАЛИ ⏳ у ROADMAP+SESSION_STATE, не лише познач done. (Хук свідомо НЕ додавали — це судження, не механіка.)

### Метрики
- Гілка `claude/new-session-foyz2r`. Коміти `971c3b4`→`1c714e3` (~11): lock, E2E `9fe0aff`→`52d729e`, архів `8f5e002`+`afab369`, UUID v7 `9722e2a`, доки `b5f8a24`/`706404f`/`1c714e3`. CACHE: `nm-20260617-0930`. Деплой v1072.
- Council: 5 агентів (тести) + 4 (Supabase проекти) + 2 doc-агенти, усі Sonnet + web. E2E прогін #11 зелений (12/12).

---

## 🔧 Сесія v1d9eo — Голос: дослідження + 2 фікси + новий план (13.06.2026)

### Зроблено (коміт `632a10c`, запушено)

**Дослідження (Council 3 Sonnet-агенти + web):** Realtime vs альтернативи vs Supabase-кут. Два незалежні агенти зійшлись: OpenAI Realtime — **погано для iOS-PWA** (Apple вбиває веб-голос у фоні; cost-bomb re-білінг історії ~×100 за 20хв; VAD ламає function-calling; echo на iOS Safari). **Рішення Романа: КОНВЕЄР** «почув(Deepgram)→GPT-4o-mini мозок→сказав(ElevenLabs стрім)» — 4-12× дешевше, реюз «одного мозку».

**Brain/Fable Романа незалежно зійшовся з агентами** (конвеєр, не Realtime) → впевненість висока.

**4 інтерим-зміни `voice-output.js` + `index.html`:** «голос не той» (список у Налаштуваннях = джерело правди + міграція `ttsEngineMigrated`); затримка (`_chunkText` + prefetch); Web Speech fallback ПРИБРАНО; STT → push-to-talk; дозвіл мікрофона на зеленій — СПРОБА+ВІДКАТ (iOS getUserMedia ламає аудіо-маршрут). **🗄️ ГОЛОСОВИЙ РЕЖИМ АРХІВОВАНО** (рішення Романа — TTS на iOS-вебі ненадійний); лишився нижній мік (`voice-input.js`) для диктування. Голос повернеться конвеєром після Supabase (`docs/VOICE_REALTIME_PLAN.md`).

### Відкрите / смоук за Романом
- Засмоукати голос; **велика збірка конвеєра** (Deepgram+ElevenLabs+Edge) — після Supabase; **Календар-баг (A)** — перевірити на чистому старті.

### Метрики
- Коміт `632a10c`, гілка `claude/new-session-v1d9eo`. CACHE_NAME `nm-20260613-1955`. Council 3 Sonnet.

---

## 🔧 Сесія qpzj7k — Проекти + Голос OWL + Фото-vision (13.06.2026)

### Зроблено — ~37 commits (`125ccd6` → `02038bd`), усе запушено

**Вкладка Проекти (велика робота, Council × 2):**
- `makeProject` фабрика + stampEntity (Ворота 3 закриті на 6/6 сутностях). +поля `brief/targetAudience/currentStage/deadline/images`.
- **Адаптивне інтерв'ю**: раніше OWL ставив 5 фіксованих питань і ВИКИДАВ відповіді (корінь — `saveGuideTopicAnswer` не знав тему `project_*`). Тепер питання генеруються під контекст, відповіді ловляться (`captureProjectInterviewAnswer`), наповнюють проект (steps/tempo/risks/budget/brief).
- **«Про проект» + детермінований мозок повноти** (`src/data/project-completeness.js`, Council-затверджено): 7 вимірів читаються з ІСНУЮЧИХ полів (не дубль-структура); `canAdvise` (суть+для кого) гейтить поради КОДОМ, не здогадом моделі. UX — живий абзац + мʼякі «уточнити: …», без форми/лічильника.
- Наповнення воркспейсу: OWL-інсайт, наступний крок з ✓, швидкі дії, лінк-стати. `set_project_budget`/`set_project_brief` tools. Бюджет=агрегат з Фінансів за `projectId`. Крок↔Задача безпечний (точний збіг). Кнопка «назад» (окремий контейнер). OWL=проектний стратег у промпті.

**Фото-vision (один мозок):** `src/ui/chat-image.js` — кнопка камери зліва у КОЖНОМУ барі → фото → gpt-4o-mini vision → опис → звичайний потік чату обробляє. Фото не зберігаємо (памʼять), лише текст. `images` поле про запас під Supabase Storage.

**Голос OWL (TTS, Web Speech):** `src/ui/voice-output.js` — кнопка 🎙 у шапці (живий діалог: озвучка + авто-мік), блок «Голос Агента» в Налаштуваннях (вибір голосу tts-1 + ключ ElevenLabs опц.), єдиний замок мовлення (табло/чат не перебивають), таймер тиші для швидкого надсилання, стоп у фоні, голос за `getLang()`. Озвучка табло коли чат закритий.

**Інфраструктура:** розбивка витрат usage-meter за способом (голос/фото/текст + TTS логування); esbuild 0.28.0→0.28.1 (новий advisory блокував auto-merge — деплой стояв); Return=новий рядок у всіх барах; Аналітика свайп-закриття (scrollGuard).

### Обговорено / вирішено

- **Голос — фундамент слабкий.** Web Speech (STT) + окремий TTS + ручна петля = крихко на iOS, звідси весь клас багів. Системне рішення — **OpenAI Realtime API (WebRTC)**: план у `docs/VOICE_REALTIME_PLAN.md`, робити окремою сесією.
- **Голос НЕ персистить між запусками** (фікс зависання на вході — сам ліз до мікрофона). Opt-in щосесії.
- **ElevenLabs** для голосу без акценту — потребує ключа Романа (стоп-ґеп до Supabase Edge).

### Інциденти

- **Деплой стояв** — новий esbuild security-advisory валив `npm audit` у auto-merge → жодна гілка не зливалась. Фікс `e738022` (0.28.0→0.28.1).
- **Завис застосунок на вході** (таббар/барабан мертві) — персистентний voice mode авто-активував мікрофон/петлю. Фікс `02038bd` (voice off on load).
- Кілька smoke-bypass `pre-push: ok` — з хмари iPhone-smoke неможливий, чек-ліст Роману.

### Метрики

- Коміти: `125ccd6` → `02038bd` (~37), гілка `claude/new-session-qpzj7k`, запушено
- CACHE_NAME: `nm-20260613-1840`
- Council: 2× (Проекти 5 агентів, голос 3 агенти) + веб-дослід (TTS, Realtime API)
- Build: node --check + check-factories(72) + guards + escape + imports + i18n — чисте
...
2. **statusLine у Claude Code з реальним % контексту** (`3ad07bf`). Створено `.claude/hooks/lib/compute-context-pct.sh` — спільна функція: парсить .jsonl через python3, бере останнє `assistant.message.usage`, ділить на 1M. Виводить `<percent> <tokens>`. Створено `.claude/hooks/statusline.sh` — формат `📊 NN% · XXXK/1M`. Переписано `.claude/hooks/context-warning.sh` — тепер бере цифру з lib (не `wc -c` файлу). Додано `statusLine` у settings.json з `refreshInterval: 10`.
3. **Архівація 4xJ7n у Phase 0 /finish** (`8889c74`) — 2+1=3 активних, винос найстарішого. Норма ≤2 збережена.

### Обговорено (без виконання)
- **Варіанти A/B/C з повідомлення Брейн** — нічого не зроблено цієї сесії. A: B-120+B-121 модалка Health (15-20 хв). B: pre-commit-i18n хук (30 хв) — потребує перевірки чи `check-i18n.js` уже не у `pre-push-check.js`. C: iPhone smoke-test 17 сценаріїв Health AI-інтерв'ю v568+.
- **PreToolUse хук-блокер для Read CLAUDE.md** — обговорено як 100% гарантія. Не реалізовано — спершу пробуємо найдешевший фікс (інструкція + нагадування).
- **Дочитати ROADMAP.md повністю + хвіст SESSION_STATE.md** — НЕ зроблено цієї сесії.

### Ключові рішення
- **Видалити самотест** замість лагодити — бо він не перевіряв реального читання.
- **statusline через `assistant.message.usage`** — бо це те саме що `/context`, і коректно віддзеркалює auto-compaction.
- **Спільна функція `lib/compute-context-pct.sh`** — щоб statusline і context-warning брали ОДНУ цифру.
- **Локальне обчислення (bash + python3)** — НЕ споживає Anthropic API лімітів.
- **`refreshInterval: 10` сек + формат `📊 NN% · XXXK/1M`** — обрано Романом з 2 варіантів.

### Інциденти
- **Stop hook нагадав про uncommitted changes** після першого блоку — спричинило вчасний коміт `924ba3c`. Це правильна поведінка хука.
- Без `git reset` / `git push --force` / skip hooks. Усі коміти першою спробою.

### Конфлікти/суперечності
- **Я склав самотест механічно, цитуючи правила з тренування.** Рома спіймав: «ти прочитав чи переглянув?». Визнав чесно: переглянув. Це призвело до видалення самотесту і впровадження прямої вимоги Read CLAUDE.md.
- **A → C → B план не підтверджено Романом.** Я запропонував порядок, Рома НЕ дав ОК — питав про правила, потім дав нову задачу (statusline). Усі A/B/C відкладені.

### Метрики
- Коміти: `924ba3c` → `3ad07bf` → `8889c74` = 3 коміти + фінальні /finish коміти
- Версії: v570 (start) → v570 (без зміни — не зачіпало `src/`)
...
## 🔧 Сесія LW3j8 — HOT_RULES + самотест-хук + i18n finance-modals/notes (01.05.2026)
...
## 🔧 Сесія bOqdI — Council механізм + 3 архітектурні фікси + Cleanup-аудит (02.05.2026)

**✅ Зроблено (7 комітів):** Council механізм у CLAUDE.md (`8cc73af`) — 5 паралельних Agent викликів. Архівація `COUNCIL_CONCEPT.md` (`891cee2`). 3 архітектурні прогалини закриті: `evening.js`/`health.js`/`proactive.js` (`19e112f` `25e60da` `8c3fe8d`). Cleanup-аудит → `CLEANUP_PLAN_bOqdI.md` (`b33e9ff`).

**Ключові рішення:** Council як механізм у CLAUDE.md, не скіл (після 5-ї ітерації з Gemini Роман: «Ми ускладнили лишнє»). Реальні паралельні агенти через Agent tool. Тригер активації автоматичний.

**Інциденти:** 3 агенти-верифікатори у rate limit (`1:10pm UTC`). Версія в git log v549 — реальна v551 з iPhone. Pre-push хук заблокував перший пуш — забув bump CACHE. Edit без Read на `sw.js` (швидко виправлено). Без reset/force/skip.

**Конфлікти:** Council 4 ітерації Gemini → ускладнення → відкат. Я схилявся не запускати Council через rate limit, Роман: «Перевіряй з агентами».

**Метрики:** `8cc73af` → `b33e9ff` (7 комітів), v549 → v551, CACHE `nm-20260502-1200` → `nm-20260502-1235`.
...
1. **Видалено хук `quick-dialogue-detector.sh` + правило ⚡ Швидкий діалог** (`6a8443d`) — детектор 1-10 слів / закритих питань / тригер-слів («коротко / швидко / по суті») змушував мене відповідати ≤3 рядки без преамбули. Засмічувало контекст: правило поверхневе, давало хибні позитиви. Замінено на одне просте правило «реагуй на сигнали болю Романа» («простіше / коротше / не розумію / простирадло») — переписати простіше + БЕЗ преамбули. Видалено з `CLAUDE.md`, `_ai-tools/HOT_RULES.md` (пункт 4), `.claude/hooks/`, `.claude/settings.json`.

2. **Розширено whitelist + спрощено `rules-reminder.sh`** (`c5d1226` + `c28fe81`) — у `check-response-violations.js` додано константи проекту (HOT_RULES, INBOX_TOOLS, nm_user_patterns тощо) і назви хуків у snake_case. `rules-reminder.sh` тепер реагує **тільки на сигнали болю**, не «кожне 5-те повідомлення» (це був шум коли все нормально).

3. **Видалено хук `check-response-violations.js` ПОВНІСТЮ** (`f4a1517`) — Stop-хук після кожної моєї відповіді сканував коди (snake_case/camelCase) без пояснень → блокував через `exit 2` → я переписував → Роман отримував **дублі**. Stop-хук не може скасувати вже відправлене повідомлення, тільки змусити написати нове. Корінь — у відсутності контекстного розуміння (regex не бачить що я **цитую** слово як приклад чи **використовую** як назву). Видалено: сам скрипт, `show-violations.sh`, `last-violations.txt`, `block-counter.json`, `violations-log.txt`, всі згадки у `settings.json`. Дисципліну пояснень тепер тримає сам Роман — смикне коли побачить що сиплю кодом без дужок.

4. **Рефакторинг CLAUDE.md 561 → 94 рядки** (`3fbf4ca` + `8e496a6`) — корінь проблеми «правила розкидані всюди + я читаю поверхнево». Стара версія 561 рядка з 11 секціями в архіві (`_archive/CLAUDE_pre_refactor_2026-05-01.md`). Нове ядро містить: 10 поведінкових правил що діють завжди, карту «куди йти за чим» (посилання на 4 нові вузькі файли під задачу), архітектурний принцип «Один мозок», файлову структуру коротко, карту документації.

5. **4 нові `RULES_*.md` під задачу** (`3fbf4ca`):
   - `_ai-tools/RULES_COMMUNICATION.md` (154р) — поведінкові: пояснення в дужках з колонками ✅/❌, UI ВИГЛЯД замість коду, розмір, brain-фідбек, код-блок у чаті
   - `_ai-tools/RULES_WORKFLOW.md` (152р) — процес: читання коду, чек-лісти архітектурних задач, smoke test після міграцій, edit/delete/cleanup, ROADMAP флоу, корінь vs симптом
   - `_ai-tools/RULES_UI.md` (98р) — UI: опис ВИГЛЯД, mockup перед кодом, посилання на DESIGN_SYSTEM, anti-patterns
   - `_ai-tools/RULES_TECH.md` (182р) — skeleton+Edit, checkpoint-коміти, CACHE_NAME bump, i18n, AI tools синхронізація, аудит скілів, документація після змін

6. **Архівовано 5 блоків SESSION_STATE** (`ff8f013`) — Ph8ym, xHQfi, EhxzJ, H0DxS, TdIqO винесено у `_archive/SESSION_STATE_archive.md` з конвертацією `<details>/<summary>` у `## 🔧 Сесія` заголовки. У живому файлі — 5 stub-рядків з посиланнями. Лишилось 2 активних: 6ANWm + LW3j8 (правило `/finish` ≤2 активних).

**Принцип нової архітектури:** CLAUDE.md повністю на старті (94р, реально читаю), точкові RULES_*.md під задачу (читаю прицільно перед відповіддю). Менше = справді читаю.

### Ключові рішення сесії

- **Видалити хук, не лагодити** — після 4 ітерацій (whitelist → поріг ≥3 → варіанти A/B → видалення) Роман сам сказав «то якась хуйня трохи. я вже на грані шоб все нахуй скасувати». Корінь дефекту — Stop-хуки не можуть скасувати вже відправлене повідомлення. Лагодити косяк такої архітектури = ставити патч на патч.
- **CLAUDE.md ≤100 рядків як ядро + посилання на вузькі RULES_*.md** — корінь поверхневого читання правил був у розмірі (561 рядок я сканував, не читав). Менше = справді читаю.
- **Поведінкові правила (тон, корінь vs симптом, сигнали болю) — у ядрі.** Якщо винести у RULES_*.md за тригером — я знов проґавлю, бо ці правила діють постійно.
- **Архівувати, не видаляти.** Все що міняється з CLAUDE.md і SESSION_STATE — копія у `_archive/`. Прямий запит Романа.
- **Пояснення англ слів — при КОЖНІЙ згадці, не «тільки при першій».** Цитата Романа: «Якщо треба два рази написати пояснення в дужках — просто напиши два рази. Я ж не робот щоб запамʼятати з першого разу».

### Інциденти

- **Дублі повідомлень через `exit 2` хука `check-response-violations`** — Роман отримував по 2 версії моєї відповіді (першу заблокували, другу я переписував). 3 такі епізоди до видалення хука. Без `git reset` / `force push` / skip hooks.

### Конфлікти/суперечності

- **Я не запропонував архітектурне рішення на початку** — слідував todo-списку (полагодити whitelist хука) замість запитати «а чи проблема та?». Роман прямо звернув увагу: «Чого ти не пропонував це коли ми мудрили з хуками?». Записано як урок «корінь vs симптом» у нове правило 5 ядра CLAUDE.md.
- **«Їдемося з робочою архітектурою замість того щоб займатися проектом»** — справедливий докір Романа на середині сесії. Цілу сесію витратили на мета-роботу замість продукту. Окреслює пріоритет наступного чату — повертатись до NeverMind фічей.

### Метрики 6ANWm

- Коміти: `6a8443d` → `ff8f013` (8 комітів, всі docs/chore — без коду у `src/`)
- Версії: НЕ змінювалась (остання v535 з LW3j8). 6ANWm нічого не деплоїла.
- CACHE_NAME: `nm-20260501-0428` (без bump — інфраструктурна сесія).
- Build: чистий — `bundle.js` не торкали.

## 🔧 Сесія LW3j8 — HOT_RULES + i18n finance-modals/notes (01.05.2026)

**✅ ЗРОБЛЕНО У LW3j8 01.05 (10 комітів):**

1. **HOT_RULES + самотест-хук після `/start`** (`cc81434`) — створено `_ai-tools/HOT_RULES.md` (8 найболючіших правил з ❌/✅ прикладами: пояснення в дужках, опис UI вигляду а не коду, розмір, швидкий діалог *(пункт «швидкий діалог» згодом скасовано 6ANWm 01.05)*, без «Роби», глибина, CACHE_NAME bump, skeleton+Edit) + `.claude/hooks/start-self-test.sh` (3 питання що Claude мусить відповісти своїми словами у чаті ПЕРЕД першою реплікою) + оновлено `/start` (HOT_RULES читається ОСТАННІМ + Крок 3 «запустити самотест»). Корінь: систематичне поверхневе читання правил між сесіями розкладається — потрібна активна обробка через відповідь на питання, не лише пасивне читання.

2. **`finance-modals.js` закритий повністю** (`10db385` + `92b56a3`) — 24 обгортки у 3 модалках (дата/бюджет/категорія) + cache bump. Перевикористано існуючі ключі `finance.date.today/yesterday/day_before_yesterday`, `finance.tx.toggle_expense/income`, `common.{cancel,save,delete,close,create}`. 1 false-positive детектора лишився (regex-bug з вкладеним `t()` у placeholder).

3. **`notes.js` — 2 батчі i18n обгорток** (`040c30f` + `27f1ef0`) — 30 рядків (запасна назва папки 17 разів через `replace_all` + написи модалки нотатки + повідомлення чату нотатки + 8 назв кольорів папок). Baseline 88 → 72.

4. **Рефакторинг категорій папок нотаток** (`03f70c7` + `446f848`) — створено `src/data/notes-categories.js` (19 категорій з канонічним англ ID + назва через `t()` + двоступеневий пошук `findCategoryByFolder` з нормалізацією апострофів). Видалено з `notes.js` дві старі мапи з українськими ключами (`FOLDER_ICON_MAP` 8 рядків + `FOLDER_ICONS` 3 + `FOLDER_COLORS` 12 з дублями `Здоров'я`/`Здоровя`). Переписано 3 функції-споживачі. Baseline 72 → 43 (-29). Лишилось 43 — це 2 системних AI промпти що передаються моделі (юзер не бачить).

5. **TESTING_LOG секція v530+** (`5eb7cc5`) — додано 5 груп тестів для рефакторингу нотаток (категорії папок з legacy `Здоровя` без апострофа), модалок фінансів, picker'а кольорів папки, повідомлень чату нотатки, нижньої AI-плашки вкладки Нотатки. Особливий фокус — категорії папок (відкат можливий через `446f848` + `03f70c7`).

6. **Brain-фідбек оброблено двічі** — від іншої сесії прийшло 2 повідомлення з порадами. Обидва оцінені як 30% сигнал / 70% шум: більшість пунктів — дублі того що Ph8ym уже зробив (brain-Клод не звірив SESSION_STATE), мета-спостереження без дії, посилання на файли brain-репо. Корисні: 3 конкретні швидкі перевірки + патерн «не питай Романа про стратегію». Передано формат для покращення (звіряти SESSION_STATE перед написанням, 3-5 пунктів макс, кожен — конкретна дія + тригер + наслідок).

7. **Аудит `/audit`** — 8 пунктів пройдено. 0 проблем, 0 виправлень. `notes.js` 1184 → 1160 рядків після рефакторингу (наближається до порогу 1200, варто винести AI-чат логіку у `notes-chat.js` або переписати на tool calling як інші чати).

**⚠️ ФЛАГНУТО — НЕ ВИРІШЕНО (на момент архівації):**
- **Ризик рефакторингу категорій** — потрібно перевірити рукою на iPhone що legacy папки (без апострофа) показують іконки. Якщо ламається — відкат `446f848` + `03f70c7`.
- **Попередні мої коміти `040c30f` і `27f1ef0`** додавали `t()` без явного імпорту у notes.js. Працювало через esbuild IIFE bundle (всі модулі в одному scope), тепер імпорт явний у `03f70c7`. Якщо v529 на проді мав ReferenceError — критичний інцидент. Перевірити консоль.
- **AI-чат `notes.js` рядки 689-815 + 1020-1170** — legacy text-JSON dialect, треба переписати на `callAIWithTools` + `INBOX_TOOLS` як у Вечорі/Фінансах/Проектах.
- **Лог `nm_reasoning_log`** (Фаза 1 EhxzJ) — перегляд через 2 тижні.
- **Лог `nm_user_patterns`** (Фаза 6 xHQfi) — перевірити через тиждень.
- **payload `nm-data-changed` без уніфікації** — 37 call-sites (Підсесія 3).

## 🔧 Сесія Ph8ym

**✅ ЗРОБЛЕНО У Ph8ym 30.04 (6 комітів):**

1. **Жива перевірка `pre-commit-testing-log.js` хука** — 3 кейси: `feat:` чіпає `src/` без `TESTING_LOG.md` → блок (exit 2 з повним текстом помилки); `fix:` будь-що → пропуск; `feat:` з `TESTING_LOG.md` у staged → пропуск. **Підтверджено: третя ітерація — справжній закон, не нагадування.** Це розриває ланцюг 3 декларативних ітерацій (lesson-reminder → xHQfi заявив автоматичне додавання → d6Fgh виявив що це ще нагадувач).

2. **Фікс детектора i18n** (`3768b9b`) — false-positive у template literals: `${t('key', 'value')}` усередині backtick-рядка більше не рахується як необгорнутий. Додано balanced `stripInterpolations()` що вирізає `${...}` блоки перед перевіркою кирилиці. Baseline 1314 → 1296 (-18).

3. **41 обгортка UI у 3 файлах:**
   - **projects.js** (`95cd468`) — 17: заголовки секцій (Бюджет/Метрики/Хронологія/Лог рішень/Корисна інфа/Ризики), кнопки, плейсхолдери, AI-інтервʼю при створенні з {name} плейсхолдером.
   - **evening.js** (`edfb995`) — 10: плейсхолдер моментів, кнопки рескедулу (На завтра/На тиждень), quit-звички (Тримався 💪/Зірвався/новий старт), модалка моменту. **Виправлено пастку:** у `.map(t => ...)` локальна `t` шедовила функцію перекладу — переніс мітки у константи (`lblTomorrow`/`lblWeek`/`lblHold` etc) ПЕРЕД `.map()`, параметр map'а перейменував на `task`.
   - **finance-modals.js** (`04a5ab2`) — 14: модалка транзакції цілком (заголовки динамічні, тогл Витрата/Дохід, дати, кнопки Скасувати/Зберегти/Видалити, toast'и).

4. **Урок + патерн + прекондиція** (`7f20f42`) — у `lessons.md`:
   - Доповнено антипатерн «Декларативне правило без автоматичного контролю» ланцюгом 3 ітерацій з d6Fgh.
   - Новий урок «Урок про точність звітів сесій» — звіт має підкріплюватись `echo` у хук, не просто читанням коду.
   - Новий патерн «Обробка brain-фідбеку з іншої сесії» — доведений робочий цикл lRnXU→m4Q1o→d6Fgh, формат для повтору.
   - У `ROADMAP.md` до V3 Фази 3 додано прекондицію: звірити логи 4 виконаних фаз (`nm_owl_cooldowns`/`nm_user_patterns`/`nm_reasoning_log`/`nm_tool_filter_log`) на iPhone ПЕРЕД реалізацією.

5. **Brain-фідбек оброблено** — з 5 пунктів втілено 4 (тест хука, фікс детектора, урок, прекондиція). 1 (перевірка V3 логів) чекає iPhone-даних Романа. Перевірка через сховище браузера локально неможлива.

6. **Файл нотаток підозрілих моментів** (`_ai-tools/SUSPICIOUS_NOTES_Ph8ym.md`) — баг детектора з regex-літералами (`/[&<>"']/g` у usage-meter.js:182 ламає state machine парсера). НЕ виправлено — потребує справжнього JS-лексера. usage-meter.js повністю обгорнутий, але baseline=2 через ці false-positive.

**📋 ПРІОРИТЕТНИЙ ПОРЯДОК ДЛЯ НОВОГО ЧАТУ:**

1. **🚨 iPhone smoke-test v527+** — БОРГ 13+ сесій. Ph8ym додав 41 i18n-обгортку — формально UI той самий, але якщо локалізація десь зламалась — побачимо тільки на iPhone. Особливо перевірити: вкладку Проекти (workspace + AI-інтервʼю при створенні), вкладку Вечір (свайп на завтра/тиждень для задач, quit-звички Тримався/Зірвався, модалка моменту), модалку транзакції фінансів (тогл Витрата/Дохід, кнопки Скасувати/Зберегти/Додати).
2. **🌍 Продовжити i18n обгортання** (baseline 1264, реально менше через regex false-positive). Наступні файли середнього розміру: `me.js` (50), `notes.js` (88), друга частина `finance-modals.js` (модалки бюджету/категорії/дати, ~13 рядків UI).
3. **🛡️ Pre-Migration Hardening Підсесія 3** — починається з бекапа `nm_backup_v*`. 37 call-sites зі string detail у `nm-data-changed`. Передумова Supabase.
4. **🐛 Фікс детектора regex-bug** — JS-лексер для `check-i18n.js`. Дасть чесну baseline (-N false-positive). Альтернатива: жити з ним, коли впритул потрібна правда — використовувати спеціальний debug-скрипт.
5. **⏸️ OWL V3 Фаза 3** — відкладена, чекає Supabase. Прекондиція у ROADMAP.
...
- **Лог `nm_user_patterns`** (Фаза 6 xHQfi) — перевірити через тиждень.
- **payload `nm-data-changed` без уніфікації** — 37 call-sites (Підсесія 3).
- **Детектор `check-i18n.js` плутається з regex-літералами** з лапками — деталі у `_ai-tools/SUSPICIOUS_NOTES_Ph8ym.md`.
...
## 🔧 Сесія d6Fgh — i18n обгортання 5 батчами + pre-commit-testing-log хук + бекап у Підсесії 3 (30.04.2026)

### Зроблено

1. **`pre-commit-testing-log.js` хук — закон, не нагадування** (`efd1461`). PreToolUse Bash хук блокує `git commit` з `feat:` що чіпає `src/|index.html|style.css` якщо `TESTING_LOG.md` НЕ серед staged файлів. Bypass-фраза `testing-log: ok` для інфраструктури без UI. Створено за прямим запитом Романа («Ти ігноруєш правила») після діагностики що xHQfi мав 4 `feat:` коміти у src/ але приріст у TESTING_LOG = 1 рядок (ручний).

2. **5 i18n батчів обгортань — ~55 рядків** (baseline 1426→1314):
   - **Batch 1** (`b005843`): utils.js (3 — formatTime), swipe-delete.js (1 — aria-label), finance-insight.js (1 — placeholder)
   - **Batch 2** (`d83aa8c`): voice-input.js (6 — aria + 5 toast), usage-meter.js (5 — метрики Налаштувань + toast)
   - **Batch 3** (`b95e64a`): evening-actions.js (7 — закриття дня + fallback titles), evening-chat.js (6 — no-key/розкажи/мережева)
   - **Batch 4** (`d4a47b9`): finance-chat.js (7 — бюджетні warning з параметрами), tasks.js (12 — модалка/undo/task-chat)
   - **Batch 5** (`497bad2`): logger.js (7 UI — секція Логи помилок у Розробник)
   - AI-промпти у `tasks.js`/`evening-chat.js`/`finance-insight.js` навмисно НЕ обгорнуто (whitelisted concept — системні інструкції моделі). Текст для юзера НЕ змінився.

3. **Бекап `nm_backup_v*` як перший крок Підсесії 3** (`f0a255e`). У `ROADMAP.md` Підсесія 3 тепер починається з: «копія всього сховища з номером версії ПЕРЕД запуском міграції payload». Закриває пропуск з Підсесії 1 (Gemini-патерн з C8uQD 27.04).

4. **Brain-фідбек оброблено за правилом «не складувати raw»** — з 6 пунктів іншої сесії втілено 3 валідних з конкретним аргументом, 3 відкинуто (повтор, тертя, не наша територія).

### Обговорено (без виконання)

- **Діагностика автоматичності `lesson-reminder.sh`** — підтверджено що це нагадувач, не writer. xHQfi мав 4 `feat:` у src/, TESTING_LOG отримав 1 рядок руками.
- **Тренд порушень у `.claude/violations-log.txt`** — неможливо порахувати, хук свіжий з xHQfi (тільки 2 рядки за d6Fgh).
- **i18n детектор false-positive у template literals** — `${t('key', 'value')}` усередині backticks рахується як необгорнутий. Не блокує, але цифра `1314` нижча за реальну на ~7-10 рядків.

### Ключові рішення

- **Pre-commit хук блокує, не нагадує** — після того як lesson-reminder з 30.04 не змусив Claude дописувати TESTING_LOG. Закон через exit-code 2 надійніше за текстовий нагадник.
- **AI-промпти у tabs/* НЕ обгортати** — whitelisted concept. Тільки UI-видимі рядки. Granular підхід замість whitelist цілих файлів.
- **Cache bump після кожного i18n батчу** — формально текст не змінився, але байти бандлу різні; PWA міг би видати старі файли. 5 bump'ів за сесію.
- **Чекпоінт-коміт після кожного батчу + cache bump окремо** — обриви потоку при `git add && commit && push` блокували коміт. Розділення на окремі команди надійніше.

### Рішення Романа

- **«Роби» × 5+** — швидке схвалення pre-commit хука, бекапу у ROADMAP, продовження i18n батчів («Якщо є контекст то роби», «Роби поки маєш контекст сесії»).
- **«Ладно, проїхали»** — відмова від детального розбору моєї криптичної відповіді про bypass-фразу. Тон: «не лізь у заклинання, дай людську версію».
- **«Бекап це шо?»** — попросив пояснити термін людською. Я відповів «копія всіх твоїх даних ПЕРЕД ризикованою зміною — щоб відкотити одним натиском».
- **«ЦЕ ЗАКОНИ!!!»** (через накопичений тиск) — Stop-хук тепер блокує криптичні відповіді. Корінь у моїх формулюваннях, не лише у хуку.
- **«Окрім тестів нічого нема шо можемо зробити?»** — обрав i18n обгортання замість iPhone-тесту. Рішення на основі трьох опцій (Підсесія 3 / Проекти-Фінанси / i18n).

### Інциденти

Без інцидентів. Жодного `git reset` / `git push --force` / skip hooks. Всі коміти першою спробою. Pre-push-check активувався 2 рази через CACHE_NAME bump check — bypass-фразою `pre-push: ok` пропускав false-positive (документація без UI або bump пройшов окремим комітом).

### Конфлікти/суперечності

- **Stop-хук заблокував мою відповідь з `cancel_quiet`** на ранній фазі — Roman «ЦЕ ЗАКОНИ!!!». Я переписав без коду-як-заклинань.
- **Pre-push hook двічі вимагав CACHE_NAME bump** при i18n обгортаннях. Конфлікт: формально текст не міняється (обгортка нейтральна), але правило 6 прав. Розв'язано: bump кожного разу — bytes бандлу різні.
- **xHQfi заявляв «авто-додавання у TESTING_LOG»** — я визнав у відповіді на brain-фідбек що насправді це нагадувач. Сформулював правильніше у звіті d6Fgh.

### Відкладене

- **iPhone smoke-test v508+** — Роман відклав («Окрім тестів нічого нема шо можемо зробити?»). 12+ сесій борг — критично росте, але поточна сесія його не зачепила (тільки i18n обгортки що формально не міняють UI).
- **Повне обгортання у `t()`** — лишилось 1314 рядків у 21 файлі. Найбільші непочатi: `inbox.js` (179), `onboarding.js` (192), `habits.js` (158).
- **Логи `nm_reasoning_log` / `nm_tool_filter_log` / `nm_user_patterns`** — перевірка через 1-2 тижні (не зараз).

### Метрики

- **Коміти:** `f0a255e` (backup ROADMAP) → `efd1461` (testing-log hook) → `b005843` (i18n batch1+cache) → `d83aa8c` (i18n batch2+cache) → `b95e64a` (i18n batch3+cache) → `d4a47b9` (i18n batch4+cache) → `497bad2` (i18n batch5/logger) → `a25140b` (archive EhxzJ) — 8 робочих + finish-коміти
- **Версії:** v517 (xHQfi останній deploy) → v518+ після auto-merge d6Fgh
- **CACHE_NAME:** `nm-20260430-1226` → `1321` → `1325` → `1330` → `1333` → `1334` (5 bumps)
- **i18n baseline:** 1426 → 1314 (-112; цей сесія обгорнула ~55, решта різниці — попередні сесії)
- **Hooks:** +1 новий блокувальний (`pre-commit-testing-log.js`), settings.json оновлено
- **Build:** node build.js не запускався локально (документаційні зміни + i18n не ламає синтаксис)

### Спостереження стилю Романа цю сесію

- **Швидкі реакції на криптичні відповіді** — «Це заклинання на алхімії?» / «ЦЕ ЗАКОНИ!!!». Тон загострений коли я порушую правила пояснень навіть один раз.
- **Чергування питань-уточнень і «Роби»** — «Бекап це шо?» / «Шо далі?» / «Роби». Швидкий ритм. Не любить простирадла.
- **Делегує обрання batch'у мені** — «Якщо є контекст то роби», «Роби поки маєш контекст». Довіряє коли бачить що план зрозумілий.
- **Відмова заглиблюватись у складне** — «Ладно, проїхали» коли я довго пояснював зайве.

### Відкриті питання

- **Чи `pre-commit-testing-log` хук не дасть false-positive у naturals workflow** — наприклад refactor що випадково попадає у `feat:` — буде заблоковано без TESTING_LOG. Контроль перші 1-2 сесії.
- **i18n детектор у template literals** — фікс не критичний, але цифра baseline нижча за реальну. Виправити коли будемо рефакторити `scripts/check-i18n.js`.
- **Чи після auto-merge d6Fgh стане v518 чи більше** — залежить від CI deploy lag. Перевірити.
...
2. **V3 Фаза 1 — `_reasoning_log` у всіх 60 tools** (`a5556df`) — закриває B-97 «Прийом у лікаря відміни». Кожен tool має обовʼязкове перше поле `_reasoning_log:string` (zero-shot CoT). Покриття: 50 INBOX_TOOLS + 9 UI_TOOLS + 1 BRAIN_TOOLS (post_chat_message). Dispatcher (`src/ai/tool-dispatcher.js`, `src/tabs/inbox.js`, `src/owl/brain-pulse.js`) стрипить поле і логує у localStorage `nm_reasoning_log` (50 останніх). Інструкція через `REASONING_LOG_RULE` блок інжектиться у `UI_TOOLS_RULES` (покриває Inbox + 5 tab-chats) + `getBrainPulseSystemPrompt`. Автоматизація через `/tmp/add_reasoning_log.js` скрипт.

3. **V3 Фаза 1.5 — Dynamic Tool Loading** (`33fd846`) — економія 30-40% токенів. Функція `selectRelevantTools(userText, fullTools)` у `src/ai/core.js` з 12 regex-категоріями. 8 базових tools завжди включаються. Fallback на повний набір при 0 або >4 матчах. `callAIWithTools` авто-фільтрує — zero-touch для 8 call sites. Лог `nm_tool_filter_log` для моніторингу.

### Ключові рішення

- **Автоматизація через node-скрипт замість 50 ручних правок** — `/tmp/add_reasoning_log.js` з регексом, ~1 год економії і zero-error.
- **Класифікатор через regex замість AI-виклику** — економія 1-2 сек на запит.
- **Fallback на повний набір при 0 або >4 матчах** — амбівалентні запити краще обробляти повним набором.
- **Bump `INSIGHTS_VERSION` 3→4 і 4→5** — старі кеші Романа невалідні, AI перегенерує одразу.

### Інциденти

Без інцидентів. Жодного `git reset` / `git push --force` / skip hooks. CACHE_NAME bump 3 рази.

### Метрики

- **Коміти:** 12 робочих + finish-коміти. Версії v494→v497+. CACHE_NAME 3 bumps.
- **i18n baseline:** 1363→1364. **Закриті баги:** 7 (B-97 + B-109..B-114). **Tools:** 60 з `_reasoning_log`, 12 категорій regex.

### Відкриті питання

- Чи `_reasoning_log` дійсно покращить рішення моделі (контроль через 2 тижні).
- Чи Dynamic Tool Loading не пропускає важливі tools (контроль через `nm_tool_filter_log`).
- Чи debounce 5 сек у B-113 не надто короткий.
...
## 🔧 Сесія JMQuT — Health AI Isolation + Event Delegation +4 + EU Compliance (17.05.2026)
