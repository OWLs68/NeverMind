...
### Активація мертвого імпорту = відкриває нові race conditions (NpBmN 04.05)

**Що сталось:** `maybeAskGuideQuestion` імпортувалась у `inbox.js:26`, але виклику ніде не було → інтерв'ю проектів показувало тільки Q1, далі мертво. Активував виклик у `sendToAI` — і одразу відкрилися 3 нові баги: (1) zombie-interview якщо юзер видалив проект під час інтерв'ю, (2) race з `nm_guide_waiting_topic` (подвійне питання за один обмін AI), (3) cooldown skip після фіналу Q5 → миттєвий random tip без 3хв паузи.

**Корінь:** активація мертвого коду = тестування ВСІЄЇ підлеглої системи яка раніше ніколи не виконувалась. Race conditions, stale state, edge cases — все це накопичується ТИХО поки виклику нема, і вибухає одразу.

**Правило:** перед активацією мертвого імпорту — простежити весь call chain (що буде викликатись далі) + перевірити cleanup для всіх локально зашитих state (`nm_*` ключів). НЕ робити «активую і подивимось».

### firstName check у guard ловить тільки first tool (NpBmN 04.05)

**Що сталось:** `clarify-guard.js shouldClarify` перевіряв тільки `toolCalls[0]?.function?.name`. AI на «був у дерматолога» міг видати batch `[save_finance, save_note]` (creative hallucination) — first=save_finance не у SUSPICIOUS, guard повертав null, doctor-чіпи не показувались.

**Правило:** для guard-перевірок з масивом — `toolCalls.some(...)` замість `toolCalls[0]`. AI часто видає batch коли не впевнений у одному tool — ловити треба ПЕРЕТИН з підозрілими.

### Тихий fallback у dispatcher = юзер думає що зберіг куди хотів (NpBmN 04.05)

**Що сталось:** `add_health_history_entry` мав «зручний» fallback: якщо `card_id` не знайдено → пише у загальну «Здоровʼя» без жодного попередження. Кейс: юзер тапнув чіп лікаря → за 3 сек видалив картку → дані пішли НЕ туди → юзер впевнений що зберіг у потрібну картку.

**Правило:** розрізнити **(a) AI не передав ID** (тихий fallback OK) vs **(b) ID передано але невалідний** (stale, треба видимий ⚠️ префікс «Картку видалено — записав у X»). Друге — НЕ silent. Юзер має знати що його очікування не справдились.

### Промпт що каже AI генерувати ID = мертвий вантаж якщо JS-guard сам збирає payload (NpBmN 04.05)

**Що сталось:** CLARIFY_INLINE_RULES блок про лікарів казав AI «згенеруй `card_id:<ID картки>` з контексту». Але `_buildDoctorChips` у guard сам збирав payload з localStorage → AI-згенерований card_id ніколи не використовувався. ~150 токенів × 5 чатів × тисячі запитів — мертвий вантаж.

**Правило:** якщо safety-net код САМ добудовує payload — промпт має казати «залиш payload `{}`, код добудує», а не диктувати конкретні поля. Інакше AI робить роботу яка одразу викидається.

### Consistency-фіксу мало одного callsite — перевір ВСІ повторення патерна (NpBmN 04.05)

**Що сталось:** Phase 4 #K замінив прямий `JSON.parse(localStorage.getItem('nm_health_cards'))` у `_buildDoctorChips` на `getHealthCards()` (для v2 міграції). У ТОМУ Ж файлі `clarify-guard.js` була ще одна гілка (focused-card check, 30 рядків вище) з тим самим прямим читанням — її пропустив. Phase 5 #L окремий коміт виправив. Потенційно edge case (raw card.doctor без міграції) на двох гілках замість одної.

**Правило:** при reuse-фіксі (заміна прямого LS на helper) — `grep` ВСІХ входжень патерну у файлі (`grep -n "localStorage.getItem.*nm_health_cards"` тощо) перш ніж commit. Один callsite = напівфікс.
...
_Останнє оновлення: 2026-05-04 (NpBmN — 5 нових анти-патернів з Council-аудиту 18 знахідок: активація мертвого імпорту, firstName check у guard, тихий fallback у dispatcher, мертвий вантаж промпту, consistency-фікс одного callsite)._

_Попереднє оновлення: 2026-04-29 (SK6E2 — патерн «створення/зміна хука у `.claude/hooks/` → штучний тригер → перевірка блокування → false positive перевірка → коміт». Інцидент oknnM з PostToolUse→PreToolUse у `settings.json` як підтверджений кейс)._
