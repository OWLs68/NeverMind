...
|---|------|------|----------------|
| B-94 | `src/tabs/health.js` (промпт) | "Алергія на пил" у чаті Здоров'я → AI викликав `set_owl_mode(Наставник)` замість `add_allergy` | Додав блок "🚫 ЖОРСТКЕ ПРАВИЛО UI-TOOLS У ЗДОРОВ'Ї" на початок промпту (одразу після "OWL НЕ ЛІКАР"). Явний список дозволених UI-тригерів (відкрий/покажи/перейди/переключись на Тренера/експортуй) + явна заборона: "Алергія на X" → ЗАВЖДИ `add_allergy`, слово X — алерген не тригер. |
| B-95 | `src/tabs/health.js` (промпт) | "Завтра прийом у лікаря на 2" у чаті Здоров'я → AI викликав `switch_tab(calendar)` замість `create_event` | Той самий блок покриває: "Завтра/сьогодні прийом о HH" → ЗАВЖДИ `create_event`, "прийом/лікар/календар" у цьому контексті — НЕ тригер `switch_tab`. Плюс правило сумніву: якщо фраза не починається з явної команди навігації — це CRUD, не UI. |
