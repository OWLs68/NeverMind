...
| ~~**B-201**~~ | ~~`src/tabs/habits.js:968,1049`~~ | ✅ **ЗАКРИТО 26yz5s 03.07** (`f8ba815`). Регресія батчу D: `export { processUniversalAction } from ...` (чистий re-export) НЕ створює локального імені → виклик у `_processOne` (чат Задач, текстовий JSON-fallback) падав ReferenceError → юзер бачив «Мережева помилка». На проді з v1082 (у bundle виклик «голий», верифіковано). **Фікс:** import + export окремо (локальна доступність + проброс для 6 імпортерів збережено). **Системно:** новий ESLint-сторож no-undef у CI (`460fc5b`) — саме він і знайшов баг при оцінці; клас E2E#39/B-201 тепер падає статично до деплою. |
