...
| # | Файл | Опис | Деталі |
|---|------|------|--------|
| B-98 | `src/owl/board.js` (функція `generateBoardMessage` + прапорець `_boardGenerating`) | OWL табло не оновлюється попри тригери — залип прапорець генерації. iPhone Роман v370 24.04.2026: діагностика повідомила «OWL табло: Не оновлюється 8 год. Можливо прапорець генерації залип». У логах 5+ викликів `[OWL board] stale message detected, forcing generation` за день, але 0 API-запитів до OpenAI. Порівняння з v368 у друга: там табло оновлювалось (4 API-запити за день) → регрес у v369-370 або раніше. | **Гіпотеза:** проміс генерації впав з помилкою (можливо abort через глобальний `_boardAbortController` з rJYkw Ф2, або stream-таймаут, або мережа). `.catch()` не скинув прапорець, `finally` не спрацював. На новий тригер функція бачить `_boardGenerating=true` → мовчить назавжди до перезапуску. **Фікс:** (1) обгорнути генерацію у `try/finally` щоб прапорець скидався завжди; (2) додати watchdog-timeout — якщо прапорець true довше 60 сек, примусово скинути; (3) логувати причину скидання для діагностики. **Пріоритет:** ДО V3 — це ламає ініціативу сови, головну фічу проекту (всі тести проактивності будуть фальшиві). Виявлено 8bSsE 24.04.2026. |
...
| # | Файл | Опис | Деталі |
|---|------|------|--------|
| B-99 | `src/owl/brain-pulse.js` (функція логу skip) | У логах з iPhone стрінг `[brain-pulse] skip:` іноді з пустою причиною після двокрапки. Іноді з нормальною причиною (`silent-phase`). Баг логгера — не впливає на функціональність, але заважає діагностувати чому мозок мовчить. Виявлено у обох iPhone-логах (друг 22.04, Роман 24.04). | **Фікс:** переглянути всі місця виклику `skip(reason)` — можливо десь передається `undefined`/`null`/`''`. Додати fallback `reason ?? 'unknown'` у самому логгері щоб підстрахувати. Опціонально: додати структуроване поле у лог-JSON (`{event:'skip', reason:'silent-phase', tab:'tasks'}`) замість вільного тексту. Виявлено 8bSsE 24.04.2026. |
