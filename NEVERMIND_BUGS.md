...
| **B-197** | `src/tabs/notes.js:458,530` `data-folder` на `.folder-item-wrap` | 🟡 **ВIДКРИТО vdlyeg 10.06** (security-аудит). `data-folder` рендериться через `escapeJsArg` (JS-string escaper для `onclick`), а не `escapeHtml`. Свайп-видалення читає `wrap.dataset.folder` → отримує JS-екрановане `O\'Brien`, а у сховищі `O'Brien` → не збігається → нотатки папки з апострофом/лапкою НЕ видаляються (лишаються «осиротілими»). 3-й escape-дефект у notes.js поспіль (раніше :186 stored XSS, :355). **Фікс (системний):** `escapeJsArg` → `escapeHtml` у рядках 458/530 (після vdlyeg КРОК 1 escapeHtml екранує лапки + round-trip через dataset цілий). Правило: `data-*` атрибути → завжди escapeHtml, ніколи escapeJsArg. ~10 хв. |
