...
| # | Файл | Опис | Деталі |
|---|------|------|--------|
| B-94 | `src/tabs/health.js` (промпт + UI_TOOLS_RULES) | "Алергія на пил" у чаті Здоров'я → AI викликав `set_owl_mode(Наставник)` замість `add_allergy` | iPhone-тест v325: юзер написав "Алергія на пил" → сова "Характер OWL: Наставник." Замість додавання алергії перемкнула свій характер. Корінь: UI-tools (`set_owl_mode`) перекривають CRUD у промпті Здоров'я — слабкий тригер "переключись на X" зіштовхнувся зі словом "пил". Треба жорсткий пріоритет `add_allergy` на слово "алергія" + обмежити `set_owl_mode` явним "переключись на Тренера/Партнера/Ментора". Виявлено 20.04.2026 NRw8G. |
| B-95 | `src/tabs/health.js` (промпт Здоров'я) | "Завтра прийом у лікаря на 2" у чаті Здоров'я → AI викликав `switch_tab(calendar)` замість `create_event` | iPhone-тест v325: юзер написав речення про новий прийом → сова "Відкрив Календар." Замість створення події перемкнула вкладку. Те саме що B-86 але з іншим UI-tool (`switch_tab`). B-86 фікс заборонив `set_finance_period`, забув `switch_tab`. Потрібно жорстке правило у промпті Здоров'я: **"UI-tools дозволені ТІЛЬКИ на явне 'відкрий X / покажи X / переключись на X'. Усе інше — CRUD."** Виявлено 20.04.2026 NRw8G. |
...
| # | Файл | Опис | Деталі |
|---|------|------|--------|
| B-93 | `src/tabs/inbox.js` (`.chat-chips-row` rendering) або `style.css` (chat window overflow) | Чіпи у Inbox chat обрізані знизу — видно тільки верхній край | iPhone-тест v325: після відповіді сови "Коли саме почалось?" чіпи-кнопки рендеряться, але вилазять за межі чат-вікна — видно тільки верх жовтих прямокутників. Перша пара чіпів (для першого питання) видима, для другого — обрізана. Гіпотеза: `.chat-chips-row` рендериться після бульбашки, але висота чат-вікна не враховує додатковий рядок; авто-скрол `msgs.scrollTop = scrollHeight` скролить до бульбашки, не до чіпів. Або `overflow:hidden` ріже. Виявлено 20.04.2026 NRw8G. |
