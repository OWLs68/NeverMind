...
| # | Файл | Опис | Деталі |
|---|------|------|--------|
| B-103 | `src/tabs/inbox.js:579` + `src/tabs/habits.js:1090` + `src/tabs/tasks.js` + `src/tabs/notes.js` + `src/tabs/health.js` (5 місць) | Дублікати подій у Календарі. iPhone 24.04 v376: у Календарі зверху два «Концерт 30 квітня» + два у секції «Події · квітень». Юзер просив одну — створились дві. | **Корінь:** код створення події просто робить `events.unshift(ev); saveEvents(events);` без перевірки на схожу подію. Модель може повернути 2 `create_event` tool_calls у одній відповіді (або юзер двічі тапнув до відповіді, або AbortController + повторний запит). Правило 4.12 «антидублювання» у промпті є але прохальне — модель ігнорує свої ж попередні tool_calls у тій же відповіді. **Фікс:** винести у `calendar.js` helper `addEventDedup(ev)` який перевіряє: чи вже є подія з `date+time+fuzzy-title-match` за останні 60 сек → якщо так, не додавати і повернути «exists». Всі 5 call-sites делегують сюди. Виявлено v2vYo 24.04.2026. |
