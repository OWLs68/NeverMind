...
|---|------|------|----------------|
| B-93 | `style.css` (`.ai-bar-messages` mask-image) | Чіпи-кнопки у Inbox chat обрізані знизу — видно тільки верх | Корінь: нижній fade у `mask-image` на контейнері повідомлень (`linear-gradient ... transparent 100%`) робив останні 16px прозорими. Чіпи останнім елементом попадали у цю зону. Фікс: прибрано нижню точку fade (мазка тепер тільки згори, знизу чорна до кінця), плюс `padding-bottom` 20→28px для комфортного запасу. Глобально для всіх 8 чатів. |
| B-94 | `src/tabs/health.js` (промпт + history detox) | "Алергія на пил" у чаті Здоров'я → AI викликав `set_owl_mode(Партнер)` замість `add_allergy` | **Два підходи у ЕWxjG.** Перший (коміт `379f13e`): жорсткий блок "🚫 ПРАВИЛО UI-TOOLS У ЗДОРОВ'Ї" на початку промпту. На iPhone v337 все одно відтворювалось 3 рази поспіль → знайдено глибшу причину. Другий (коміт `[hash]`): прибрано дубль `UI_TOOLS_RULES` з кінця промпту (конфлікт з новим блоком — старий допускав `set_owl_mode` на слово "переключись" окремо) + додано **history detox** — UI-tool acknowledgements (типу "Характер OWL: Партнер") більше не попадають у `healthBarHistory`, тільки в UI/storage. Інакше 3 помилкові відповіді поспіль у history навчали AI продовжувати той самий патерн (self-reinforcement). |
| B-95 | `src/tabs/health.js` (промпт) | "Завтра прийом у лікаря на 2" у чаті Здоров'я → AI викликав `switch_tab(calendar)` замість `create_event` | Той самий пакет що B-94. Мій блок "🚫 ПРАВИЛО UI-TOOLS У ЗДОРОВ'Ї" на початку + history detox. "Завтра/сьогодні прийом о HH" → ЗАВЖДИ `create_event`, "прийом/лікар/календар" — НЕ тригер `switch_tab`. Правило сумніву: якщо фраза не починається з явної команди навігації — це CRUD, не UI. |
