> 🔒 **ЗАБЛОКОВАНО (14.07.2026)** — не запускати. Активувати ТІЛЬКИ коли Рома явно скаже
> що проект на фінішній прямій / поліровці перед релізом. Причина блокування: застосунок ще
> активно будується, повний аудит зараз занадто дорогий (тижневий ліміт токенів) відносно
> користі — тестовий прогін 11.07 спалив ~2,64М токенів і вичерпав сесійний ліміт за ~40 хв.
> Файл винесено з `.claude/commands/` у `.claude/commands-archived/`, щоб команда `/fullaudit`
> фізично НЕ реєструвалась. Дизайн з фіксами (бюджет-гейт на `budget.remaining()`, severity-verify
> з effort:medium, чанкінг по вимірах хвилями+паузи, fail-open verify-null → under-verified,
> нічний Routine) **готовий і лишається актуальним** — деталі нижче + `_ai-tools/FULLAUDIT_TESTRUN_2026-07-11.md`.
> Розблокувати: `git mv .claude/commands-archived/fullaudit.md .claude/commands/` + прибрати цей банер.
