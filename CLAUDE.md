...
OWL — це єдиний мозок застосунку. Він має знати про **будь-яку** зміну даних на **будь-якій** вкладці і реагувати миттєво. Табло, чати, чіпи — все це один агент який бачить повну картину.

**Технічна реалізація:** кожна save-функція (`saveTasks`, `saveHabits`, `saveHabitLog`, `saveNotes`, `saveFinance`, `saveProjects`, `saveMoments`, `saveHealthCards`, `saveHealthLog`) dispatch'ає подію `window.dispatchEvent(new CustomEvent('nm-data-changed'))`. Debounce-listeners у `proactive.js` і `inbox-board.js` чекають 3 секунди і тригерять оновлення табло.

**Правило:** при додаванні нової save-функції або нового типу даних — **обов'язково** додати `window.dispatchEvent(new CustomEvent('nm-data-changed', { detail: 'тип' }))`. Якщо OWL не знає про зміну — це баг.
