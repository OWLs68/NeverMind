...
- **При кожному пуші в `main`** — повідомити Роману що має з'явитись на лічильнику: `v[версія] · ДД.ММ ГГ:ХХ` (версія з `id="deploy-version"` в index.html, час — поточний Amsterdam `TZ='Europe/Amsterdam' date '+%d.%m %H:%M'`)
