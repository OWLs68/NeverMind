...
## Обов'язкове правило: завжди консультуватись з Gemini

**КОЖНЕ** повідомлення від користувача потребує консультації з Gemini перед відповіддю.

### Алгоритм для кожного запиту:

1. **Виклич `ask_gemini`** — передай питання користувача + релевантний контекст (код, файл, задача)
2. **Порівняй** свою думку з думкою Gemini — знайди де збігаємось, де розходимось
3. **Дай користувачу відповідь** у форматі:

```
💬 Обговорили з Gemini:

[Коротко що думає Gemini]

[Коротко де погоджуємось / де розходимось]

✅ Рішення: [фінальна рекомендація з урахуванням обох думок]
```

### Винятки (коли Gemini не потрібен):
