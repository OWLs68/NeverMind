...
## Обов'язкове правило: повний цикл з Gemini для кожної задачі

**КОЖНА** задача проходить три етапи разом з Gemini. Без винятків.

---

### Етап 1 — Планування (до написання коду)

1. Виклич `ask_gemini` — **обов'язково передавай у `context`**:
   - Поточна задача (що саме треба зробити)
   - Релевантний код або вміст файлів
   - Обмеження: ванільний JS, без фреймворків, localStorage, GitHub Pages
   - Поточний стан з SESSION_STATE.md якщо доречно
   Gemini вже знає загальний контекст проекту через системний промпт — передавай специфіку.
2. Порівняй підходи — свій і Gemini
3. Покажи користувачу варіанти і поясни різницю:

```
📋 Обрали підхід разом з Gemini:

Gemini пропонує: [підхід A] — [чому]
Я пропоную:      [підхід B] — [чому]

Збігаємось у: [що]
Розходимось у: [що]

➡️ Рекомендую [підхід X] тому що [причина з урахуванням обох думок]
   Погоджуєшся? Чи хочеш інший варіант?
```

4. **Не пиши код поки користувач не підтвердив підхід.**

---

### Етап 2 — Написання коду

5. Пиши код згідно погодженого підходу
6. Одразу виклич `gemini_review_code` з написаним кодом
7. Якщо Gemini знайшов проблеми — виправ до того як показати користувачу

```
🔍 Код перевірено Gemini:

[якщо знайшов проблеми]: Виправив X — Gemini вказав що [причина]
[якщо все ок]: Gemini підтвердив що код коректний
```

---

### Етап 3 — Фінальна відповідь

```
💬 Рішення (погоджено з Gemini):

[готовий код або зміни]

✅ Що зроблено: [коротко]
⚠️ На що звернути увагу: [якщо є нюанси]
```

---

### Винятки (Gemini не потрібен):
- Просте читання файлу без аналізу
- Уточнюючі питання до користувача
- Коли користувач явно каже "без Gemini"

---

### Протокол незгоди між Claude і Gemini

Якщо Claude і Gemini пропонують **принципово різні підходи** — не обирай сам.
Покажи користувачу обидва варіанти чітко і запитай:

```
⚡ Ми з Gemini не погоджуємось:

Варіант A (Gemini): [підхід] — плюси: X, мінуси: Y
Варіант B (Claude): [підхід] — плюси: X, мінуси: Y

Який обираємо? (або я поясню детальніше будь-який)
```

Це навчає користувача розуміти компроміси, а не сліпо приймати рішення AI.

---

### Журнал рішень

Кожне **архітектурне рішення** (вибір підходу, структура даних, важлива зміна логіки)
додавай в кінець файлу `_ai-tools/decisions.md` у форматі:

```
## [дата] [коротка назва рішення]
**Задача:** що вирішували
**Обрали:** який підхід
**Чому:** причина (включно з думкою Gemini якщо різнились)
```

---

### Автоматичне збереження стану сесії

Сесія може обірватись несподівано — без попередження. Тому стан зберігається **автоматично**.

#### Правило: оновлювати SESSION_STATE.md як ПЕРШУ дію кожної відповіді

Перш ніж читати файли, писати код, або відповідати — одразу оновити `_ai-tools/SESSION_STATE.md`.
Так навіть якщо сесія обірветься посередині — попередній стан вже збережено.
...
## Зараз робимо
[поточна задача одним реченням]

## Зроблено в цій сесії
- [файл або дія] — [що саме зроблено]

## Наступний крок
[конкретна наступна дія]

## Важливий контекст
[рішення, нюанси, попередження]
```

Після оновлення файлу — зроби `git add _ai-tools/SESSION_STATE.md && git commit -m "session: update state" && git push`.

#### На початку нової сесії

Перша дія — прочитати `_ai-tools/SESSION_STATE.md` і повідомити користувача:
```
📌 Підхопив контекст з попередньої сесії:
Робили: [що]
Зроблено: [що]
Наступний крок: [що]
Продовжуємо?
