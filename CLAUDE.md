...
2. `_ai-tools/LAST_SESSION.md` — авто-генерований git-стан після попередньої сесії
3. `_ai-tools/SESSION_STATE.md` — контекст попередньої сесії (якщо є)
4. `docs/CHANGES.md` — що змінювалось в попередніх сесіях
...
- **Після кожного пушу** — оновити і закомітити `_ai-tools/SESSION_STATE.md` з поточним станом: що робимо, що зроблено, наступний крок. Формат:

```markdown
# Стан сесії
**Оновлено:** YYYY-MM-DD HH:MM (Amsterdam)
## Зараз робимо
[поточна задача]
