...
- **НІКОЛИ не створювати правила самостійно.** Завжди спочатку запитати Романа. Тільки якщо він дає згоду — створити правило і записати в документ.
- **Після КОЖНОЇ зміни** (баг, фікс, планування, будь-що) — одразу оновити `docs/CHANGES.md` і `_ai-tools/SESSION_STATE.md` і запушити. Не чекати кінця сесії. Роман може перейти в новий чат в будь-який момент без попередження — документація завжди має бути актуальною.
...
- **При кожному деплої** — змінити `CACHE_NAME` у `sw.js` (формат: `nm-YYYYMMDD-HHMM`) **локально, перед пушем** — CI не чіпає sw.js. Використовуй `date` в терміналі щоб отримати правильний час.
